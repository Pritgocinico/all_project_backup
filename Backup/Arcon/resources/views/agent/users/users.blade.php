...
<div class="houmanity-card">
    <div class="card-body card-head">
        <div class="d-md-flex gap-4 align-items-center bg-white p-3">
            <div class="d-none d-md-flex">All Dealers</div>
            ...
            <div class="ms-auto d-flex">
                <form class="user-inner-search" action="">
                    <div class="row">
                        <div class="col-md-12">
                ...
                {{-- <a href="{{ route('admin.add_user') }}" class="ms-2 btn btn-primary">
            ...
            <tbody>
                @if (!blank($users))
                    @foreach ($users as $user)
                        <tr>
                            <td data-th="Name"><a href="{{ route('agent.view_user',$user->id) }}" class="text-primary">{{$user->name}}</a></td>
                            <td data-th="Email">
                                {{$user->email}}
                            </td>
                                ...
                                <div class="status-inner-div">
                                    <div class="btn-group ml-5">
                                        ...
                                        <ul class="dropdown-menu user-status">
                                            <li><a class="dropdown-item status @if($user->agent_status == 1) hide @endif" href="#" data-status="1" data-user="{{ $user->id }}">Verify</a></li>
                                            <li><a class="dropdown-item status @if($user->agent_status == 0) hide @endif" href="#" data-status="0" data-user="{{ $user->id }}">Not Verified</a></li>
                                        </ul>
                                    </div>
                                    <p class="mb-0">
                                        @if ($user->status == 1)
                                            <span class="text-success">Verified by Arcon</span>
                                        @else
                                            <span class="text-danger">Not Verified by Arcon</span>
                                        @endif
                                    </p>
                                </div>
                            </td>
...
<script>
    $(document).ready(function(){
        $('.user-status li a').on('click', function(){
            ...
            $.ajax({
                url : "{{ route('change_user_agent_status') }}",
                type : 'POST',
