...
@section('content')
    <div
                    ...
                    <div class="hstack gap-2 ms-auto">
                        @if (collect($accesses)->where('menu_id', '8')->first()->status == 2)
                            ...
                            <a href="#" class="btn btn-sm btn-dark" data-bs-toggle="modal"
                                ...
                                Bulk Import</a>
                            {{-- <a href="#" class="btn btn-sm btn-dark" data-bs-toggle="modal"
                                data-bs-target="#village_import" data-bs-toggle="modal"><i
                                    class="fa-solid fa-file-import"></i>
                                Village Import</a> --}}
                        @endif
                    </div>
        ...
        <div class="modal fade" id="village_import" tabindex="-1" aria-labelledby="depositLiquidityModalLabel"
            style="display: none;" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content overflow-hidden">
                    <div class="modal-header pb-0 border-0">
                        <h1 class="modal-title h4" id="depositLiquidityModalLabel">Import Village</h1>
                        <button type="reset" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form action="{{ route('import.excel') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer">
                            <button type="reset" class="btn btn-neutral" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-primary">Import Village</button>
                        </div>
                    </form>                    
                </div>
            </div>
...
@section('script')
    <script>
        ...
        function submitForm() {
            ...
            if (Holidaydate == '' && Holidayname == '' && Holidaydesc == '') {
                $('#date_error').html('Please Select Date.')
                $('#name_error').html('Please Enter Name.')
                $('#description_error').html('Please Enter Description.')
                    return false;
            }else if(Holidaydate == '' && Holidayname == ''){
                $('#date_error').html('Please Select Date.')
                $('#name_error').html('Please Enter Name.')
                return false;
            }else if(Holidayname == '' && Holidaydesc == ''){
                $('#name_error').html('Please Enter Name.')
                $('#description_error').html('Please Enter Description.')
                return false;
            }else if(Holidaydesc == '' && Holidaydate == ''){
                $('#description_error').html('Please Enter Description.')
                $('#date_error').html('Please Select Date.')
                return false;
            }else if(Holidayname == ''){
        ...
        $('#holiday_name').on('keyup', function() {
            if ($(this).val() != '') {
                $('#name_error').html('');
            }
        });
        ...
        function updateHoliday() {
            ...
            if (Editdate == '' && Editname == '' && Editdesc == '') {
                ...
                editHoliday(id);
                    return false;
            }else if(Editname == '' && Editdesc == ''){
                $('#edit_name_error').html('Please Enter Name.')
                $('#edit_description_error').html('Please Enter Description.')
                editHoliday(id);
                    return false;
            }else if(Editdesc == '' && Editdate == ''){
                $('#edit_date_error').html('Please Select Date.')
                $('#edit_description_error').html('Please Enter Description.')
                editHoliday(id);
                    return false;
            }else if(Editname == '' && Editdate == ''){
                $('#edit_date_error').html('Please Select Date.')
                $('#edit_name_error').html('Please Enter Name.')
                editHoliday(id);
                    return false;
        ...
        $('#edit_holiday_date').on('change', function() {
            if ($(this).val() != '') {
                $('#edit_date_error').html('');
            }
        });

        $('#edit_holiday_name').on('keyup', function() {
            if ($(this).val() != '') {
                $('#edit_name_error').html('');
            }
        });
