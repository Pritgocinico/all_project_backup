...
class LeaveController extends Controller
{
    ...
    {
        $page = "Leave";
        $this->leave = resolve(Leave::class)->with('userDetail', 'userDetail.roleDetail', 'userDetail.departmentDetail');
    ...
    public function leaveAjaxList(Request $request)
    {
        ...
        $date = $request->date;
        $leaveList = $this->leave->when($status, function ($query) use ($status) {
            $query->where('status', $status);
        })->when($date, function ($query) use ($date) {
            $date1 = explode('/', $date);
            $query->whereBetween('leave_from', [Carbon::parse($date1[0])->format('Y-m-d'), Carbon::parse($date1[1])->format('Y-m-d')]);
        });
    ...
    public function changeLeaveStatus(Request $request)
    {
        ...
        if ($update) {
            ...
            $user = User::where('status', 1)->where('id', $leave->user_id)->first();
            Notification::send($user, new SendNotification($offerData));
            $pusher->trigger('notifications', 'new-notification', $offerData);
            Log::create([
                'user_id' => auth()->user()->id,
