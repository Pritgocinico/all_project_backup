...
class HolidayController extends Controller
{
    ...
    public function exportFile(Request $request)
    {
        ...
        if ($request->format == "pdf") {
            ...
            $pdf = PDF::loadView('admin.pdf.holiday', ['holidayList' => $holidayList,'setting' =>$setting]);
    ...
    public function import(Request $request)
    {
        ...
        $request->validate([
            'file' => 'required|mimes:xlsx,xls,csv|max:100000', // Adjust max size if needed
        ]);

        // Import the Excel file
        Excel::import(new LargeDataImport, $request->file('file'));

        return redirect()->route('holiday.index')->with('success', 'Data imported successfully!');
    }
