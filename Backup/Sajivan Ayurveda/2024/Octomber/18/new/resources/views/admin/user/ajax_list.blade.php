            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="form-check form-switch">
                        @if (collect($accesses)->where('menu_id', '5')->where('edit', 1)->first())
                            <input class="form-check-input" type="checkbox" data-id="{{ $user->id }}"
                                data-status="{{ $status }}" name="status" id="status"
                                onchange="changeUserStatus(this)" {{ $user->status == 1 ? 'checked' : '' }}>
                        @else
                            <input class="form-check-input" type="checkbox" data-id="{{ $user->id }}"
                                data-status="{{ $status }}" name="status" id="status"
                ...
                <td class="text-end">
                    <div class="icon-td">
                        ...
                        @endif
                        @if (collect($accesses)->where('menu_id', '5')->where('delete', 1)->first())
