                ...
                <main class="container-fluid px-6 pb-10">
                    <form action="{{ route('leave.update', $leave->id) }}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="row align-items-center g-3 mt-6 position-relative">
                            ...
                            <div class="col-md-4 col-xl-4">
                                <select name="leave_type" class="form-control" id="leave_type_dropdown">
                                    ...
                                    <option value="paid"  @if ($total_leftLeave <= 0) disabled @endif @if ($leave->leave_type == 'paid') selected @endif>Paid
                                    </option>
                                    <option value="unpaid" @if ($leave->leave_type == 'unpaid') selected @endif>Unpaid</option>
                                </select>
                                <div class="text-danger paid_count" id="paid_count_div">@if($total_leftLeave <= 0)You've used up all your paid leaves. Leave left : 0 @else Leave Left : {{$total_leftLeave}} @endif</div>
                                @error('leave_type')
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            <div class="col-md-2" id="leave_from_label"><label class="form-label mb-0">Leave From</label></div>
                            ...
                            <div class="col-md-2" id="leave_to_label"><label class="form-label mb-0">Leave To</label></div>
                            ...
                            <div class="col-md-4 d-none half_time_div">
                                <select name="half_time" id="half_time" class="form-control">
                                    ...
                                    <option value="first_half" {{$leave->half_time == 'first_half'? "selected" : ""}}>First Half ( 9:30 to 2 )</option>
                                    <option value="second_half" {{$leave->half_time == 'second_half'? "selected" : ""}}>Second Half ( 2 to 7 )</option>
                                </select>
                                @error('half_time')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-4 col-xl-4">
                                <select name="leave_reason" class="form-control" id="leave_reason_dropdown">
                                    ...
                                    <option value="Family Function" @if ($leave->reason == 'Family Function') selected @endif>Family Function
                                    </option>
                                    <option value="Sick Leave" @if ($leave->reason == 'Sick Leave') selected @endif>Sick Leave
                                    </option>
                                    <option value="Emergency Leave" @if ($leave->reason == 'Emergency Leave') selected @endif>Emergency Leave
                                    </option>
                                    <option value="other" @if ($leave->reason == 'other') selected @endif>Other
                                    </option>
                                </select>
                                @error('leave_reason')
                                    <div class="text-danger">{{ $message }}</div>
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-4 col-xl-4 d-none" id="other_div">
                                <textarea name="other_reason" class="form-control" placeholder="Enter Leave Reason">{{ $leave->other_reason }}</textarea>
                                @error('other_reason')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
...
@section('script')
    <script>
        ...
        $(document).ready(function(){
            if($('#leave_type_dropdown').val() == "unpaid"){
                $('#paid_count_div').addClass('d-none');
            }else{
                $('#paid_count_div').removeClass('d-none');
            }
            // Check the value of leave feature on page load (for edit)
            ...
            // Check the value of leave reason on page load (for edit)
