        ...
        <main class="container-fluid p-0">
            <div class="px-6 px-lg-7 pt-8 border-bottom">
                <div class="row d-flex align-items-center mb-5">
                    <h1 class="col-md-4">Leave</h1>
                    <div class="col-md-8">
                        <div class="row attendance-list-btn">
                            @if (collect($accesses)->where('menu_id', '7')->where('view', 1)->first())
                                <div class="col-md-2"></div>
                                <div class="col-md-3">
                                    <select name="leave_status" id="leave_status" class="form-control"
                                        onchange="leaveAjax(1)">
                                ...
                                <div class="col-md-1">
                                    <a class="btn btn-sm btn-dark" id="search_date" onclick="clearFilter()"
                                        value="Clear">Clear</a>
                                </div>
                            @endif
                            @if (collect($accesses)->where('menu_id', '7')->where('export', 1)->first())
                                <div class="col-md-1">
                            ...
                            @endif
                            @if (collect($accesses)->where('menu_id', '7')->where('add', 1)->first())
                                <div class="col-md-1">
                                    <a href="{{ route('leave.create') }}" class="btn btn-sm btn-dark"><i
            ...
            <div class="px-6 px-lg-7 pt-6">
                <div id="leave_table_ajax" class=" custom-scrollbar"></div>
            </div>
...
@section('script')
    <script>
        ...
        function exportCSV() {
            ...
            window.open(exportFile + '?format=' + format + '&search=' + search + "&date=" + date + "&status=" + status + "",
                '_blank');
        }
