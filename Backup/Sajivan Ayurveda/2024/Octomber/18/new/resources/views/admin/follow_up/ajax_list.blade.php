<table class="table table-hover table-sm table-scrolling table-nowrap table-responsive mt-6 border" id="follow_up_table">
    <thead>
        <tr>
            <th>No</th>
            <th>Event Name</th>
    ...
    <tbody>
        @foreach ($followUpList as $key => $follow)
            <tr>
                <td>{{ $key + 1 }}</td>
                <td>
                    @if (collect($accesses)->where('menu_id', '16')->where('view', 1)->first())
                        <a href="{{ route('follow-up.show', $follow->id) }}">{{ $follow->event_name }}</a>
                    @else
                        {{ $follow->event_name }}
                    @endif
                </td>
                ...
                <td class="text-end">
                    <div class="icon-td">
                        @if (collect($accesses)->where('menu_id', '16')->where('edit', 1)->first())
                            <a href="{{ route('follow-up.edit', $follow->id) }}" class="text-dark" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="Edit Follow Up"><i
                                    class="fa-solid fa-pen-to-square"></i></a>
                        @endif
                        @if (collect($accesses)->where('menu_id', '16')->where('delete', 1)->first())
