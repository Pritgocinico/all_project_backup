@if (collect($accesses)->where('menu_id', '5')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '6')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '7')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '8')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '10')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '11')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '12')->where('disable',1)->first() ||
        collect($accesses)->where('menu_id', '13')->where('disable',1)->first())
    ...
    <a class="nav-link d-flex align-items-center {{request()->routeIs('user.*') || request()->routeIs('holiday.*') || request()->routeIs('leave.*') || request()->routeIs('attendance.*') || request()->routeIs('certificate.*') || request()->routeIs('salary-slip.*') || request()->routeIs('daily_attendance') ? 'active' : '' }}"
        ...
        <ul class="nav nav-sm flex-column mt-1 inner-ul {{ request()->routeIs('incentive.*') || request()->routeIs('holiday.*') || request()->routeIs('user.*') || request()->routeIs('leave.*') || request()->routeIs('attendance.*') || request()->routeIs('certificate.*') || request()->routeIs('salary-slip.*') || request()->routeIs('daily_attendance') ? ' ' : '' }}"
            id="sidebar-account-hr">
                    ...
                    @endif --}}
                    @if (collect($accesses)->where('menu_id', '7')->where('disable',1)->first())
                    ...
                    @if (collect($accesses)->where('menu_id', '10')->where('disable',1)->first())
                    ...
                    @endif --}}
                    @if (collect($accesses)->where('menu_id', '13')->where('disable',1)->first())
