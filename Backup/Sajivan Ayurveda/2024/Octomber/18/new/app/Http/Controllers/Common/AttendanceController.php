...
class AttendanceController extends Controller
{
    ...
    public function dailyAttendance()
    {
        ...
        $employees = User::where('status',1)->where('role_id', '!=', 1)->latest()->get();
        if (Auth()->user()->role_id == 2) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $employees = User::where('status',1)->whereIn('id', $managerMember)->get();
            } else {
                $employees = User::where('status',1)->where('id', Auth()->user()->id)->get();
            }
        }
    ...
    public function ajaxList(Request $request)
    {
        ...
        if ($request->start_date != null) {
            $date1 = explode('/', $request->start_date);
            $startDate = Carbon::parse($date1[0]);
            $endDate = Carbon::parse($date1[1])->format('Y-m-d');
            if($startDate->eq($endDate)){
                $type = 1;
            }
            $startDate = Carbon::parse($date1[0])->format('Y-m-d');
            $endDate = Carbon::parse($date1[1])->format('Y-m-d');
        } else {
        ...
        return view('admin.attendance.table', compact('userList', 'attendanceData','type'));
    }
    ...
    public function dailyAttendanceExport(Request $request)
    {
        ...
        if ($user_id == 1) {
            ...
            $employees = User::where('status',1)->where('id', $user_id)->get();
        }
    ...
    public function changeStatus(Request $request){
        ...
        $attendance = $this->attendance->where('user_id',$request->id)->where('attendance_date',Carbon::today())->first();
        if($attendance){
            $attendance->status = $request->status;
            $attendance->save();
            return response()->json(['message' => 'Attendance status changed successfully.'],200);
        } else {
