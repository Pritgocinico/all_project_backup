...
use App\Imports\BulkImportData;
...
class LeadController extends Controller
{
    ...
    {
        $type = request('type');
        $employeeList = User::where('status', 1)->where('role_id', "!=", '1')->get();
        // if(Auth()->user()->role_id == 8){

        //     return view('sale_approval.lead.index', compact('type', 'employeeList'));
        // }
        return view('admin.lead.index', compact('type', 'employeeList'));
    }
    ...
    public function leadAjaxList(Request $request)
    {
        ...
        if (Auth()->user()->role_id != 1 && Auth()->user()->role_id != 8) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $leads = $leads->whereIn('created_by', $managerMember)->orWhereIn('assigned_to', $managerMember);
            } else {
                $leads = $leads->where('created_by', Auth()->user()->id)->orWhere('assigned_to', Auth()->user()->id);
            }
        }
        // if(Auth()->user()->role_id == 8){
    ...
    public function edit($id)
    {
        ...
        $customerDetail = Customer::find($lead->customer_id);
        if ($customerDetail) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        $lead->lead_date = $request->lead_date;
        $lead->assigned_to = $request->assigned_to;
        $leadChangeColumns = $lead->getDirty();
        if (!empty($leadChangeColumns)) {
            foreach ($leadChangeColumns as $column => $newValue) {
                $originalValue = $lead->getOriginal($column);
                $originalValue = $originalValue ? $originalValue : '';
                $newValue = $newValue ? $newValue : '';
                if ($column == 'assigned_to') {
                    $originalName = User::find($originalValue);
                    $newName = User::find($newValue);
                    $originalValue = $originalName->name;
                    $newValue = $newName->name;
                }
                $columnName = str_replace('_', ' ', ucfirst($column));
                if ($originalValue != $newValue) {
                    $modifiedLog = new Remark();
                    $modifiedLog->lead_id = $lead->id;
                    $modifiedLog->title = "Lead Change";
                    $modifiedLog->created_by = auth()->user()->id;
                    $modifiedLog->remark = "Lead Has Been '{$columnName}' to '{$originalValue}' to '{$newValue}'";
                    $modifiedLog->remark_icon = 'fa-solid fa-location-dot';
                    $modifiedLog->save();
                }
            }
        }
    ...
    public function leadRemarks(Request $request)
    {
        ...
        if ($request->title == "Call Back/Follow Up") {
            $lead_remark->next_call_date_time = $request->next_call;
        }
    ...
    public function updateLeadCustomerData(Request $request)
    {
        ...
        if ($lead) {
            ...
            $lead->collectable_amount = $request->collect_amount;
            $lead->payment_type = $request->payment_type;
            $lead->discount_amount = $request->discount_amount;
            ...
            if (!empty($leadChangeColumns)) {
                foreach ($leadChangeColumns as $column => $newValue) {
                    ...
                    if ($originalValue != $newValue) {
                        ...
                        $modifiedLog->created_by = auth()->user()->id;
                        $modifiedLog->remark = "Lead Has Been '{$columnName}' to '{$originalValue}' to '{$newValue}'";
                        $modifiedLog->remark_icon = 'fa-solid fa-database';
                        $modifiedLog->save();
                    }
                }
            }
            ...
            if ($customer) {
                ...
                $customerChangesColumn = $customer->getDirty();
                if (!empty($customerChangesColumn)) {
                    foreach ($customerChangesColumn as $column => $newValue) {
                        $originalValue = $customer->getOriginal($column);
                        if ($originalValue != $newValue) {
                            $columnName = str_replace('_', ' ', ucfirst($column));
                            $modifiedLog = new Remark();
                            $modifiedLog->lead_id = $lead->id;
                            $modifiedLog->title = "Customer Change";
                            $modifiedLog->created_by = auth()->user()->id;
                            $modifiedLog->remark = "Customer Has Been '{$columnName}' to '{$originalValue}' to '{$newValue}'";
                            $modifiedLog->remark_icon = 'fa-solid fa-user';
                            $modifiedLog->save();
                        }
                    }
                }
                ...
                if ($request->cust_product_id !== null) {
                    foreach ($request->cust_product_id as $key => $cusPro) {

                        $productExist = CustomerProduct::where('id', $cusPro)->first();
                        if (!isset($productExist)) {
                            $custProduct = new CustomerProduct();
                        } else {
                            $custProduct = CustomerProduct::where('id', $cusPro)->first();
                        }
                        $custProduct->product_id = $request->product_id[$key];
                        $custProduct->quantity = $request->quantity[$key];
                        $custProduct->amount = $request->amount[$key];
                        $custProduct->tax = $request->tax[$key];
                        $custProduct->product_id = $request->product_id[$key];
                        $custProduct->product_id = $request->product_id[$key];
                        if (isset($request->invoice[$key]) && $request->invoice[$key] !== null) {
                            $file = $request->invoice[$key];
                            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
                            $invoiceCertificatePath = $file->storeAs('lead/invoice', $newFilename, 'public');
                            $custProduct->invoice = $invoiceCertificatePath;
                        }
                        $custProduct->customer_id = $lead->customer_id;
                        $custProduct->lead_id = $lead->id;
                        CustomerProduct::where('id', $cusPro)->first();
                        if (!isset($productExist)) {
                            $modifiedLog = new Remark();
                            $modifiedLog->lead_id = $lead->id;
                            $modifiedLog->title = "New Product Add";
                            $modifiedLog->created_by = auth()->user()->id;
                            $modifiedLog->remark = "New Customer Product Add.";
                            $modifiedLog->remark_icon = 'fa-solid fa-cart-shopping';
                            $modifiedLog->save();
                        } else {
                            $custProductChangedColumns = $custProduct->getDirty();
                            if (!empty($custProductChangedColumns)) {
                                foreach ($custProductChangedColumns as $column => $newValue) {
                                    $originalValue = $custProduct->getOriginal($column);
                                    if ($originalValue != $newValue) {
                                        $columnName = str_replace('_', ' ', ucfirst($column));
                                        $modifiedLog = new Remark();
                                        $modifiedLog->lead_id = $lead->id;
                                        $modifiedLog->title = "Customer Product Change";
                                        $modifiedLog->created_by = auth()->user()->id;
                                        $modifiedLog->remark = "Customer Product Has Been '{$columnName}' to '{$originalValue}' to '{$newValue}'";
                                        $modifiedLog->remark_icon = 'fa-solid fa-phone';
                                        $modifiedLog->save();
                                    }
                                }
                            }
                ...
                if ($request->alt_num_id !== null) {
                    foreach ($request->alt_num_id as $key => $numberId) {
                        if ($request->cust_alt_num[$key] !== null) {
                            ...
                            if (!$customer->exists()) {
                                $alterExist = CustAlternateNumber::where('id', $numberId)->first();
                                if (!isset($alterExist)) {
                                ...
                                if (!isset($alterExist)) {
                                    $modifiedLog = new Remark();
                                    $modifiedLog->lead_id = $lead->id;
                                    $modifiedLog->title = "New Alternate Number Add";
                                    $modifiedLog->created_by = auth()->user()->id;
                                    $modifiedLog->remark = "New Alternate Number Add.";
                                    $modifiedLog->remark_icon = 'fa-solid fa-phone';
                                    $modifiedLog->save();
                                } else {
                                    $altNumberChangedColumns = $altNumberExist->getDirty();
                                    if (!empty($altNumberChangedColumns)) {
                                        foreach ($altNumberChangedColumns as $column => $newValue) {
                                            $originalValue = $altNumberExist->getOriginal($column);
                                            if ($originalValue != $newValue) {
                                                $columnName = str_replace('_', ' ', ucfirst($column));
                                                $modifiedLog = new Remark();
                                                $modifiedLog->lead_id = $lead->id;
                                                $modifiedLog->title = "Alternate Number Change";
                                                $modifiedLog->created_by = auth()->user()->id;
                                                $modifiedLog->remark = "Alternate Number Has Been '{$columnName}' to '{$originalValue}' to '{$newValue}'";
                                                $modifiedLog->remark_icon = 'fa-solid fa-phone';
                                                $modifiedLog->save();
                                            }
                                        }
                ...
                if ($request->address_id !== null) {
                    foreach ($request->address_id as $key => $addressId) {
                        $addExist = CustAddress::where('id', $addressId)->first();
                        if (!isset($addExist)) {
                        ...
                        $addressExist->village = isset($request->village) ? $request->village[$key] : "";
                        $addressExist->office_name = isset($request->office_name) ? $request->office_name[$key] : "";
                        $addressExist->dist_city = isset($request->dist_city) ? $request->dist_city[$key] : "";
                        $addressExist->dist_state = isset($request->dist_state) ? $request->dist_state[$key] : "";
                        if (!isset($addExist)) {
                            $modifiedLog = new Remark();
                            $modifiedLog->lead_id = $lead->id;
                            $modifiedLog->title = "New Address Add";
                            $modifiedLog->created_by = auth()->user()->id;
                            $modifiedLog->remark = "New Customer Address Add.";
                            $modifiedLog->remark_icon = 'fa-solid fa-location-dot';
                            $modifiedLog->save();
                        } else {
                            $addressChangeColumns = $addressExist->getDirty();
                            if (!empty($addressChangeColumns)) {
                                foreach ($addressChangeColumns as $column => $newValue) {
                                    $originalValue = $addressExist->getOriginal($column);
                                    if ($originalValue != $newValue) {
                                        $columnName = str_replace('_', ' ', ucfirst($column));
                                        $modifiedLog = new Remark();
                                        $modifiedLog->lead_id = $lead->id;
                                        $modifiedLog->title = "Column {$columnName} Change";
                                        $modifiedLog->created_by = auth()->user()->id;
                                        $modifiedLog->remark = "Column '{$columnName}' has changed from '{$originalValue}' to '{$newValue}'";
                                        $modifiedLog->remark_icon = 'fa-solid fa-location-dot';
                                        $modifiedLog->save();
                                    }
                                }
    ...
    public function viewEdit($id)
    {
        ...
        if (isset($customer)) {
            $custAltnumbers = $customer->getAlternativeNumber;
            $custAddresses = $customer->customerAddress;
            $whomList = Lead::select('for_whom')->whereNotNull('for_whom')->groupBy('for_whom')->pluck('for_whom')->toArray();
            $languageList = Lead::select('customer_language')->whereNotNull('customer_language')->groupBy('customer_language')->pluck('customer_language')->toArray();
            $leadRemarks = Remark::with('userDetail', 'userDetail.departmentDetail')->where('lead_id', $id)->latest()->get();
            $latestRemark = Remark::where('lead_id', $id)->latest()->first();
            $diseases = Disease::get();
            $leadProduct = CustomerProduct::with('leadProduct')->where('lead_id', $id)->get();
            return view('admin.lead.show_edit', compact('languageList', 'whomList', 'latestRemark', 'leadProduct', 'products', 'diseases', 'lead', 'customer', 'custAltnumbers', 'custAddresses', 'leadRemarks'));
        }
        return redirect()->back()->with('error', 'Customer Not Found.');
    }
    public function bulkImport(Request $request)
    {
        ...
        $request->validate([
            'import_file' => 'required|mimes:xlsx,xls,csv|max:100000',
        ]);
        try {
            $import = new BulkImportData();
            Excel::import($import, $request->file('import_file'));
            $importedLeads = $import->getImportedLeads();
            foreach ($importedLeads as $key => $import) {
                $customerData = Customer::where('id', $import->customer_id)->first();
                $customerData['disease_detail'] = Disease::where('id', $customerData->cust_disease)->first();
                $import['customer_detail'] = $customerData;
            }
            $employees = User::where('status', 1)->where('role_id', "!=", '1')->get();
            $data['leads'] = $importedLeads;
            $data['users'] = $employees;
            return response()->json(['message' => 'Lead Imported Successfully.', 'data' => $data], 200);
        } catch (\Throwable $e) {
            return response()->json(['message' => $e->validator->errors()->first('missing_parameter')], 500);
        }
    }
    public function bulkAssign(Request $request)
    {
        if (isset($request->lead_id)) {
            foreach ($request->lead_id as $key => $leadID) {
                $lead = Lead::find($leadID);
                $lead->assigned_to = $request->assign_to[$key];
                $lead->save();
            }
    ...
    public function saleApproval(Request $request)
    {
        $id = $request->lead_id;
        $lead = Lead::find($id);
        if ($lead) {
            ...
            if ($update) {
                if ($request->status == 3) {
                    ...
                    $remark->lead_id = $lead->id;
                    $remark->created_by = auth()->user()->id;
                    $remark->title = 'Lead Rejected';
                    $remark->remark = $request->reject_reason;
                    $remark->remark_icon = 'fa-solid fa-location-dot';
                    $remark->save();
                } else if ($request->status == 2) {
                    $remark = new Remark();
                    $remark->lead_id = $lead->id;
                    $remark->created_by = auth()->user()->id;
                    $remark->title = 'Lead Approved';
                    $remark->remark = "Lead Approved";
                    $remark->remark_icon = 'fa-solid fa-location-dot';
                }
                return response()->json(['message' => 'Lead Status Change Successfully.'], 200);
            }
