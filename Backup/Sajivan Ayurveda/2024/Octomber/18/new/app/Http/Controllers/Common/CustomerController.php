...
class CustomerController extends Controller
{
    ...
    public function store(CreateCustomerRequest $request)
    {
        ...
        foreach ($request->add_type as $index => $add_type) {
            ...
            $custAddress->taluka_tehsil = $request->taluka_tehsil[$index];
    ...
    public function checkNumberCustomer(Request $request){
        $number = $request->alt_num;
        $customer = Customer::where('mobile_number', $number)->orWhereHas('getAlternativeNumber', function($query) use ($number) {
            $query->where('cust_alt_num', $number);
        })->first();
    ...
    public function removeCustProduct(Request $request){
        $id = $request->id;
        $customerProduct = CustomerProduct::find($id);
        if ($customerProduct) {
            $lead = Lead::where('customer_id', $customerProduct->customer_id)->first();
            if ($lead) {
                $productAmount = $customerProduct->amount;
                $productQuantity = $customerProduct->quantity;
                $productTotalAmount = $productAmount * $productQuantity;
                $lead->total_amount = $lead->total_amount - $productTotalAmount;
                $lead->collectable_amount = $lead->collectable_amount - $productTotalAmount;
                $lead->save();
            }
            $customerProduct->delete();
            return response()->json(['data' => '', 'message' => 'Customer Product Removed.', 'status' => 1], 200);
        }
        return response()->json(['data' => '', 'message' => 'Something Went To Wrong.', 'status' => 1], 500);
    }   
}
