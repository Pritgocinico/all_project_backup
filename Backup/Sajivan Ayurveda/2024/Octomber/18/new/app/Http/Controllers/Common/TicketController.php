...
class TicketController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.ticket.index', compact('departmentList'));
    }
    ...
    public function ticketAjaxList(Request $request)
    {
        ...
        if (Auth()->user()->role_id == 2) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $ticketList = $ticketList->whereIn('emp_id', $managerMember);
            } else {
                $ticketList = $ticketList->where('emp_id', Auth()->user()->id);
            }
        }
        $ticketList = $ticketList->when($search, function ($query, $search) {
    ...
    public function exportFile(Request $request)
    {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            $headers = array(
                ...
                "Content-Disposition" => "attachment; filename=help Export.csv",
                "Pragma" => "no-cache",
        ...
        if ($request->format == "pdf") {
            ...
            $pdf = PDF::loadView('admin.pdf.ticket', ['ticketList' => $ticketList, 'setting' => $setting]);
    ...
    public function store(CreateTicketRequest $request)
    {
        ...
        $ticketId = 'CA-HELP-' . substr("00000{$lastId}", -6);
        $data = [
            'ticket_id' => $ticketId,
        ...
        if ($ticket) {
            $userList = User::where('status',1)->where('role_id', 1)->get();
            ...
            foreach ($userList as $key => $user) {
                $offerData = [
                    'user_id' => $user->id,
                    'type' => 'Help',
                    'title' => "Help is Created For subject " . $request->subject,
                    'text' => "Help Create",
            ...
            Log::create([
                ...
                'module' => 'Help',
                'description' => auth()->user()->name . " created a help named '" . $request->subject . "'"
            ]);
            return response()->json(['status' => 1, 'message' => 'Help created successfully.'], 200);
        }
     ...
     */
    public function show(Ticket $help)
    {
        return view('admin.ticket.show', compact('help'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(Ticket $help)
    {
        return response()->json(['status' => 1, 'data' => $help], 200);
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(CreateTicketRequest $request, Ticket $help)
    {
        $update = $help->update([
            'department_id' => $request->department,
            'ticket_type' => $request->ticket_type,
        ...
        if ($update) {
            ...
            Log::create([
                'user_id' => auth()->user()->id,
                'module' => 'Help',
                'description' => auth()->user()->name . " updated a help named '" . $request->subject . "'"
            ]);
            return response()->json(['status' => 1, 'message' => 'Help updated successfully.'], 200);
        }
        return response()->json(['status' => 1, 'message' => 'Something Went to Wrong.'], 500);
    }
     ...
     */
    public function destroy(Ticket $help)
    {
        $delete = $help->delete();
        if ($delete) {
            Log::create([
                'user_id' => auth()->user()->id,
                'module' => 'Help',
                'description' => auth()->user()->name . " deleted a help"
            ]);
            return response()->json(['status' => 1, 'message' => 'Help deleted successfully.'], 200);
        }
    ...
    public function addCommentTicket(Request $request)
    {
        ...
        if ($ticket) {
            $userList = User::where('status',1)->where('role_id', 1)->where('id', '!=', Auth()->user()->id)->get();
            $config = config('services')['pusher'];
            $pusher = new Pusher($config['key'],  $config['secret'], $config['app_id'], [
                'cluster' => env('PUSHER_APP_CLUSTER', 'ap2'),
            ]);
            foreach ($userList as $key => $user) {
                $offerData = [
                    ...
                    'text' => "Help Comment",
                    'url' => route('help.show', $request->ticket_id),
                ];
                Notification::send($user, new SendNotification($offerData));
                $pusher->trigger('notifications', 'new-notification', $offerData);
            }
            ...
            Log::create([
                'user_id' => auth()->user()->id,
                'module' => 'Help',
                'description' => auth()->user()->name . " added a comment in help"
            ]);
    ...
    public function changeTicketStatus(Request $request, $id)
    {
        ...
        if ($update) {
            $userList = User::where('status',1)->where('role_id', 1)->where('id', '!=', $ticket->emp_id)->get();
            $config = config('services')['pusher'];
            $pusher = new Pusher($config['key'],  $config['secret'], $config['app_id'], [
                'cluster' => env('PUSHER_APP_CLUSTER', 'ap2'),
            ]);
            foreach ($userList as $key => $user) {
                $offerData = [
                    'user_id' => $user->id,
                    'type' => 'Help',
                    'title' => "Help Status changed to " . $statusName,
                    'text' => "Help Status",
                    'url' => route('help.index'),
                ];
                Notification::send($user, new SendNotification($offerData));
            ...
            Log::create([
                'user_id' => auth()->user()->id,
                'module' => 'Help',
                'description' => auth()->user()->name . " changed a help status with " . $ticket->ticket_id,
            ]);
            return response()->json(['data' => $ticket, 'message' => "Help Status Changed Successfully.", 'status' => 1], 200);
        }
        return response()->json(['data' => $ticket, 'message' => 'Something went to wrong.', 'status' => 1], 500);
    }
}
