...
class UserController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->users = resolve(User::class)->with('teamMemberList', 'teamMemberList.memberData', 'officeNumber', 'stateDetail', 'countryDetail', 'cityDetail', 'roleDetail', 'designationDetail', 'departmentDetail');
    }
    ...
    public function exportFile(Request $request)
    {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $columns = array('Name', 'Role Name', 'Email', 'Phone Number', 'Department Name', 'Designation Name', 'Salary', 'Created BY', 'Created At');
            $callback = function () use ($userList, $columns) {
                ...
                foreach ($userList as $user) {
                    ...
                    fputcsv($file, array($user->name, $roleDetail, $user->email, $user->phone_number, $department, $designation, $user->employee_salary, $userName, $date));
                }
                fclose($file);
            };
        ...
        if ($request->format == "pdf") {
            ...
            $pdf = PDF::loadView('admin.pdf.employee', ['userList' => $userList, 'setting' => $setting])->setPaper('a4', 'landscape');
    ...
    public function create(Request $request)
    {
        ...
        $roleList = Role::where('active', 1)->get();
        $userList = $this->users->where('status', 1)
            ->where(function ($query) {
                $query->whereNull('is_manager')
                    ->orWhere('is_manager', 'no');
            })
            ->where('role_id', '!=', 1)
            ->latest()
            ->get();
        return view('admin.user.create', compact('userList', 'countryList', 'roleList', 'departmentList'));
    }
    ...
    public function store(CreateUserRequest $request)
    {
        ...
        $userCode = 'CA-' . $result . '-' . substr("00000{$lastId}", -6);
        $user = new User;
        ...
        if ($request->hasFile('address_proof')) {
            ...
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $addressProof = $file->storeAs('address_proof', $newFilename, 'public');
            $user->address_proof = $addressProof;
        }
        if ($request->hasFile('passport_photo')) {
            $file = $request->file('passport_photo');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $passport_img = $file->storeAs('passport_photo', $newFilename, 'public');
            $user->passport_photo = $passport_img;
        }
        if ($request->hasFile('marksheet')) {
            $file = $request->file('marksheet');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $marksheet_img = $file->storeAs('marksheet', $newFilename, 'public');
            $user->marksheet = $marksheet_img;
        }
        if ($request->hasFile('bank_details')) {
            $file = $request->file('bank_details');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $bankDetail = $file->storeAs('bank_details', $newFilename, 'public');
            $user->bank_details = $bankDetail;
        }
        $insert = $user->save();
        if ($request->office_calling_number) {
            foreach ($request->office_calling_number as $index => $officeNumber) {
                if ($officeNumber !== null) {
                    $emp_officeNum = new OfficeNumber();
                    $emp_officeNum->user_id = $user->id;
                    $emp_officeNum->office_calling_number = $officeNumber;
                    $emp_officeNum->save();
                }
            }
        }
        if ($insert) {
            ...
            if (isset($request->user_id)) {
                foreach ($request->user_id as $key => $id) {
                    $manager = new ManagerMemberDetail();
    ...
    public function edit($id)
    {
        ...
        $managerMember = ManagerMemberDetail::where('manager_id', $id)->pluck('user_id')->toArray();
        
        $employeeList = User::where(function($query) {
        $query->whereNull('is_manager')
              ->orWhere('is_manager', 'no');
    })
    ->where('status', 1)
    ...
    ->latest()
    ->get();
        return view('admin.user.edit', compact('employeeList', 'managerMember', 'user', 'countryList', 'roleList', 'departmentList', 'deductionDetail'));
    }
    ...
    public function update(CreateUserRequest $request, $id)
    {
        ...
        if ($request->hasFile('passport_photo')) {
            $file = $request->file('passport_photo');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $passportPhoto = $file->storeAs('passport_photo', $newFilename, 'public');
            $user->passport_photo = $passportPhoto;
        }
        if ($request->hasFile('marksheet')) {
            $file = $request->file('marksheet');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $markSheet = $file->storeAs('marksheet', $newFilename, 'public');
            $user->marksheet = $markSheet;
        }
        if ($request->hasFile('bank_details')) {
            $file = $request->file('bank_details');
            $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
            $bankDetails = $file->storeAs('bank_details', $newFilename, 'public');
        ...
        if ($request->has('office_num_ids')) {
            foreach ($request->office_num_ids as $key => $id) {
                $office_num = OfficeNumber::find($id);
                $office_num->office_calling_number = $request->office_calling_number[$key];
                $office_num->save();
            }
        }

        if ($request->has('office_calling_number')) {
            foreach ($request->office_calling_number as $key => $number) {
                if (!isset($request->office_num_ids[$key])) {
                    if (!empty($number)) {
                        // dd($number, $user->id);
                        OfficeNumber::create([
                            'user_id' => $user->id,
                            'office_calling_number' => $number
                        ]);
                    }
                }
            }
        ...
        if ($update) {
            if (isset($request->deduction_type)) {
                foreach ($request->deduction_id as $key => $deductionId) {
                    ...
                    if ($deduction) {
                        ...
                        $deduction->amount = $request->amount[$key];
                        $deduction->status = isset($request->deduction_status[$key]) && $request->deduction_status[$key]  == "on" ? 1 : 0;
                        $deduction->save();
                    } else {
            ...
            ManagerMemberDetail::where('manager_id', $user->id)->delete();
            if (isset($request->user_id)) {
    ...
    public function removeUsersCard(Request $request)
    {
        $id = $request->id;
        $column = $request->type;
    ...
    public function office_numDelete(Request $request)
    {
        $id = $request->id;
        $delete = OfficeNumber::where('id', $id)->delete();
        if ($delete) {
    ...
    public function userStatus(Request $request)
    {
        ...
        $update = $user->save();
        if ($update) {
            return response()->json(['status' => 1, 'message' => 'Status updated successfully.', 'data' => []], 200);
        }
        return response()->json(['status' => 0, 'message' => 'Something went to wrong.'], 500);
    }
    public function userLeadData(Request $request)
    {
        $id = $request->id;
        $data['lead'] = Lead::with('customerDetail', 'customerDetail.custDisease')->where('assigned_to', $id)->get();
        $data['users'] = User::where('status', 1)->where('id', '!=', $id)->where('role_id', '!=', 1)->where('status', 1)->get();
        return response()->json(['status' => 1, 'message' => 'Status updated successfully.', 'data' => $data], 200);
    }
    public function deActiveUserData(Request $request)
    {
        $id = $request->user_id;
        $user = User::find($id);
        ...
        if ($update) {
            if ($request->lead_id) {
                foreach ($request->lead_id as $key => $lead) {
                    $lead = Lead::find($lead);
        ...
        return redirect()->route('users.index')->with('error', 'Something went to wrong.');
    }
}
