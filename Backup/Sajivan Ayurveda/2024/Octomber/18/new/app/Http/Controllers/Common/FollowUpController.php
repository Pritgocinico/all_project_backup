...
class FollowUpController extends Controller
{
    ...
    {
        $employees      = User::where('status',1)->where('role_id', 2)->get();
        ...
        if($insert){
            if(isset($request->assigned_to)){
                foreach ($request->assigned_to as $key => $assign) {
                    ...
                    $followUpMember->save();
                    $userDetail = User::where('status',1)->where('id',$assign)->first();
                    $email = $userDetail->email;
                    $data = [
                        'userDetail' => $userDetail,
                        'followUpEvent' => $followUpEvent,
                        'createdUser' => User::where('status',1)->where('id',Auth()->user()->id)->first(),
                    ];
                    try {
    ...
    public function followCommentStore(Request $request){
        ...
        if ($ticket) {
            ...
            foreach ($followList as $key => $follow) {
                $user = User::where('status',1)->where('id',$follow->user_id)->first();
                $offerData = [
                    'user_id' => $user->id,
                    'type' => 'Follow Up',
