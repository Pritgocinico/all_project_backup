...
class SettingController extends Controller
{
    ...
    public function settingUpdate(UpdateSettingRequest $request)
    {
        ...
        if (isset($settingData)) {
            ...
            $update = Setting::create($settingArray);
        }
        if ($update) {
    ...
    public function AllLogs()
    {
        ...
        if($id != 2){
            if(Auth()->user()->is_manager == "yes"){
                $managerMember = ManagerMemberDetail::where('manager_id', $userId)->pluck('user_id')->toArray();
                array_push($managerMember, $userId);
                $logList = $logList->whereIn('user_id', $managerMember);
            } else {
                $logList = $logList->where('user_id', $id);

            }
        }
        $logList = $logList->latest()->get();
        return view('admin.log.index', compact('logList','page'));
    }

    public function logAjaxList(Request $request)
    {
        ...
        if (Auth()->user()->role_id == 2) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $logList = $logList->where('user_id', Auth()->user()->id);
            }
        }
            $logList= $logList->when($search, function ($query, $search) {
            ...
            ->latest()
            ->get();
        return view('admin.log.ajax_list', compact('logList'));
    }
