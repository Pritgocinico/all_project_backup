...
class EmployeeDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth()->user()->is_manager == "yes") {
            ...
            array_push($managerMember, $id);
            $lead = $lead->whereIn('created_by', $managerMember)->orWhereIn('assigned_to', $managerMember);
        } else {
            $lead = $lead->where('created_by', $id)->orWhere('assigned_to', $id);
        }
        $data['leads'] = $lead = $lead->get();
        $statuses = ['1', '2', '3', '4', '5', '6'];

        // $leadStatusData = Lead::latest();
        // if (Auth()->user()->is_manager == "yes") {
        //     $managerMember = ManagerMemberDetail::where('manager_id', $id)->pluck('user_id')->toArray();
        //     array_push($managerMember, $id);
        //     $leadStatusData = $leadStatusData->whereIn('created_by', $managerMember)->orWhereIn('assigned_to',$managerMember);
        // } else {
        //     $leadStatusData = $leadStatusData->where('created_by', $id)->orWhere('assigned_to',$id);
        // }
        // $leadStatusData = $leadStatusData->selectRaw('lead_status, COUNT(*) as count')
        // ->groupBy('lead_status')
        // ->pluck('count', 'status')
        // ->toArray();
        ...
        $followUpList = FollowUpEvent::latest();
        if (Auth()->user()->is_manager == "yes") {
            $managerMember = ManagerMemberDetail::where('manager_id', $id)->pluck('user_id')->toArray();
            array_push($managerMember, $id);
            $followUpList = $followUpList->whereIn('created_by', $managerMember)
                ->orWhereHas('followUpMemberDetail', function ($query) use($managerMember) {
                    $query->orWhereIn('user_id', $managerMember);
                });
        } else {
            $followUpList = $followUpList->where('created_by', $id)->orWhereHas('followUpMemberDetail', function ($query) use($id) {
                $query->where('user_id', $id);
            });
        }
        
        $data['followUpList'] = $followUpList = $followUpList->get();
        $followStatusData = FollowUpEvent::where('created_by', $id)->orWhereHas('followUpMemberDetail', function ($query) use($id) {
            $query->where('user_id', $id);
