...
use App\Models\CompleteLeadCheckList;
...
class LeadController extends Controller
{
    ...
    public function store(CreateLeadRequest $request)
    {
        ...
        if ($insert) {
            if ($request->invest_type == "investments") {
                ...
                $travelLead->document_country = $request->country_document;
                $travelLead->doc_check_list_id = $request->doc_check_list_id;
            ...
            if ($request->child_type !== null) {
                foreach ($request->child_type as $key => $child) {
                    ...
                    if (isset($request->member_doc[$key]) && $request->member_doc[$key] != null) {
                        $fileArray = array();
    ...
    public function show(string $id)
    {
        ...
        if ($lead) {
            $travelData = TravelLead::where('lead_id', $id)->first();
            $docId = isset($travelData) ? $travelData->doc_check_list_id : "";
            $documentCheckList = DocumentCheckList::where('id', $docId)->first();
            $leadCheckListItem = LeadCheckListItem::get();
            $completeCheckList = CompleteLeadCheckList::with('checkListItem', 'userDetail')->where('lead_id', $id)->get();
            return view('admin.lead.show', compact('completeCheckList', 'leadCheckListItem', 'lead', 'userList', 'documentCheckList'));
        }
    ...
    public function leadStatusChange(Request $request)
    {
        ...
        if ($request->lead_status == "3") {
            ...
            $lead->hold_complete_date_time = $request->hold_complete_date_time;
        ...
        if ($update) {
            ...
            if ($request->completed_lead_id != null) {
                foreach ($request->completed_lead_id as $key => $checkId) {
                    $checkListItemId = $checkId;
                    if ($checkId == "other") {
                        $leadCheckList = new LeadCheckList();
                        $leadCheckList->type = $lead->invest_type;
                        $insert = $leadCheckList->save();
                        if ($insert) {
                            $checkListItem = new LeadCheckListItem();
                            $checkListItem->lead_check_list_id = $leadCheckList->id;
                            $checkListItem->name = $request->other_option[$key];
                            $checkListItem->save();
                            $checkListItemId = $checkListItem->id;
                        }
                    }
                    $completedLeadCheckList = new CompleteLeadCheckList();
                    $completedLeadCheckList->lead_id = $lead->id;
                    $completedLeadCheckList->checklist_id = $checkListItemId;
                    $completedLeadCheckList->created_by = Auth()->user()->id;
                    $completedLeadCheckList->save();
                }
            }
            Log::create([
                'user_id' => auth()->user()->id,
