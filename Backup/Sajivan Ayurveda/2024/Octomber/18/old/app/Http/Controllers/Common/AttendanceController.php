...

    {
        ...
        if(Auth()->user()->role_id == 2){
            if(Auth()->user()->is_manager == "yes"){
                ...
                $employees = User::whereIn('id', $managerMember)->latest()->get();
            } else {
                $employees = User::where('id', Auth()->user()->id)->latest()->get();
            }
        }
        
        $dailyAttendanceListsPaginated = $employees->map(function ($employee) use ($status) {
            $attendance = $this->attendance->when($status,function($query)use($status){
                $query->where('status',(string) $status);
            })->whereDate('attendance_date', Carbon::today())->where('user_id', $employee->id)->first();
            ...
            $meetingBreakData = BreakLog::whereDate('date', Carbon::now())->where('user_id', $employee->id)->where('break_type', "meeting")->get();
            $meetingBreakTime = 0;
            if (isset($meetingBreakData)) {
                foreach ($meetingBreakData as $key => $meeting) {
                    $meetingBreakTime += UtilityHelper::getDiffInSecond($meeting->break_start, $meeting->break_over);
                }
            }
            $lunchBreakData = BreakLog::whereDate('date', Carbon::now())->where('user_id', $employee->id)->where('break_type', "lunch")->get();
            $lunchBreakTime = 0;
            if (isset($lunchBreakData)) {
                foreach ($lunchBreakData as $key => $lunch) {
                    $lunchBreakTime += UtilityHelper::getDiffInSecond($lunch->break_start, $lunch->break_over);
                }
            }

            $teaCoffeeBreakData = BreakLog::whereDate('date', Carbon::now())->where('user_id', $employee->id)->where('break_type', "tea_coffee")->get();
            $teaCoffeeBreakTime = 0;
            if (isset($teaCoffeeBreakData)) {
                foreach ($teaCoffeeBreakData as $key => $teaCoffee) {
                    $teaCoffeeBreakTime += UtilityHelper::getDiffInSecond($teaCoffee->break_start, $teaCoffee->break_over);
                }
            }
            ...
            return (object) [
                ...
                'meeting_break_time' => UtilityHelper::formatSecondsToTime($meetingBreakTime),
                'tea_coffee_break_time' => UtilityHelper::formatSecondsToTime($teaCoffeeBreakTime),
                'lunch_break_time' => UtilityHelper::formatSecondsToTime($lunchBreakTime),
...

    {
        ...
        if(Auth()->user()->role_id == 2){
            if(Auth()->user()->is_manager == "yes"){
                $managerMember = ManagerMemberDetail::where('manager_id',Auth()->user()->id)->pluck('user_id')->toArray();
                array_push($managerMember,Auth()->user()->id);
                $userList = $this->users->whereIn('id', $managerMember)->get();
            } else {
                $userList = $this->users->where('id', Auth()->user()->id)->get();
            }
        }
...

    {
        ...
        if(Auth()->user()->role_id == 2){
            if(Auth()->user()->is_manager == "yes"){
                $managerMember = ManagerMemberDetail::where('manager_id',Auth()->user()->id)->pluck('user_id')->toArray();
                array_push($managerMember,Auth()->user()->id);
                $employees = User::whereIn('id', $managerMember)->get();
            } else {
