...
@section('content')
                    ...
                    <div class="hstack gap-2 ms-auto">
                        @if (collect($accesses)->where('menu_id', '21')->first()->status == 2)
                            <a href="#" class="btn btn-sm btn-dark" data-bs-toggle="modal"
                                ...
                                Export</a>
                            <a href="{{ route('lead-checklist.create') }}" class="btn btn-sm btn-dark"><i class="fa-solid fa-plus"></i>
                                Create {{$page}}</a>
                        @endif
                    </div>
                </div>
    ...
    <div class="modal fade" id="export_document_checklist" tabindex="-1" aria-labelledby="depositLiquidityModalLabel"
            ...
            <div class="modal-content overflow-hidden">
                ...
                <form id="" class="form" action="#">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-dark" id="submitBtn" onclick="exportCSV()">Submit</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('script')
    <script>
        ...
        function exportCSV() {
            var exportFile = "{{ route('lead-checklist-export') }}";
        ...
        function deleteDepartment(id) {
            Swal.fire({
                title: 'Are you sure?',
                text: "Are you sure the delete this Lead Checklist?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('lead-checklist.destroy', '') }}" + "/" + id,
                        type: 'DELETE',
                        dataType: 'json',
                        data: {
                            _token: "{{ csrf_token() }}",
