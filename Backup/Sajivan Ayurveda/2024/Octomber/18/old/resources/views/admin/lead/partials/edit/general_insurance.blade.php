@if (isset($lead->insuranceLeadData))
    ...
    <div id="general_insurance_div" style="display: @if ($type !== 2) none @endif;">
        ...
        <div id="fire_policy_div" class="{{ $lead->insurance_type != 'fire_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.fire_policy')
        </div>
        <div id="wc_policy_div" class="{{ $lead->insurance_type != 'wc_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.wc_policy')
        </div>
        <div id="health_policy_div" class="{{ $lead->insurance_type != 'health_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.health_policy')
        </div>
        <div id="pa_policy_div" class="{{ $lead->insurance_type != 'pa_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.pa_policy')
        </div>
        <div id="gpa_policy_div" class="{{ $lead->insurance_type != 'gpa_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.gpa_policy')
        </div>
        <div id="gms_policy_div" class="{{ $lead->insurance_type != 'gmc_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.gmc_policy')
        </div>
        <div id="marine_policy_div" class="{{ $lead->insurance_type != 'marine_policy' ? 'd-none' : '' }}">
            @include('admin.lead.partials.edit.insurance.marine_policy')
        </div>
    </div>
@endif
