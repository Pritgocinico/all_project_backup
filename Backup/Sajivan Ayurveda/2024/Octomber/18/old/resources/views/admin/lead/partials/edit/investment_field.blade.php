    ...
    <div class="row align-items-center g-3 mt-6">
        ...
        <div class="col-md-4 col-xl-4" id="generalRadio"
            ...
            <select name="insurance_type" id="insurance_type" class="form-control insurance_type">
                ...
                <option value="health_policy" {{ $lead->insurance_type == 'health_policy' ? 'selected' : '' }}>Health
                    Policy
                </option>
                <option value="pa_policy" {{ $lead->insurance_type == 'pa_policy' ? 'selected' : '' }}>PA POLICY
                </option>
                <option value="gpa_policy" {{ $lead->insurance_type == 'gpa_policy' ? 'selected' : '' }}>GPA POLICY
                </option>
                <option value="gmc_policy" {{ $lead->insurance_type == 'gmc_policy' ? 'selected' : '' }}>GMC POLICY
                </option>
                <option value="marine_policy" {{ $lead->insurance_type == 'marine_policy' ? 'selected' : '' }}>Marine
                    POLICY</option>
            </select>
        </div>
        @if (isset($lead->investmentLeadData))
            ...
            <div class="col-md-4 investement_field_form investement_type_field">
                <select name="investment_field" id="investment_field" class="form-control">
                    <option value="new" {{ $investmentData->investment_field == 'new' ? 'selected' : '' }}>New
                    </option>
        ...
        <div class="row align-items-center g-3 mt-6" id="fd_interest" style="display: none;">
            ...
            <div class="col-md-4 col-xl-4">
                <input type="number" name="interest_rate" id="interest_rate" placeholder="Enter Rate Of Interest"
                    class="form-control" value="{{ $investmentData->interest_rate }}">
                @error('interest_rate')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row align-items-center g-3 mt-6" id="maturity_amount_interest" style="display: none;">
            ...
            <div class="col-md-4">
                <select name="interest_mode" id="interest_mode" class="form-control">
                    ...
                    <option value="Cumulative" {{ $investmentData->interest_mode == 'Cumulative' ? selected : '' }}>
                        Cumulative</option>
        ...
        <div class="row align-items-center g-3 mt-6">
            ...
            <div class="col-md-4 bond_payout_detail">
                <select name="interest_payout" id="interest_payout" class="form-control">
                    ...
                    <option value="Quarterly" {{ $investmentData->interest_payout == 'Quarterly' ? selected : '' }}>
                        Quarterly</option>
                    <option value="half_year" {{ $investmentData->interest_payout == 'half_year' ? selected : '' }}>
                        Half Year</option>
