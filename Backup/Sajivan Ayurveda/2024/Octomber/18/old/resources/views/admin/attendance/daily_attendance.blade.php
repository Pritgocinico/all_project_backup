        ...
        <main class="container-fluid p-0">
            ...
            <div class="px-6 px-lg-7 pt-6">
                <div class="table-responsive custom-scrollbar">
                    <table class="table table-hover table-sm table-nowrap table-responsive mt-6 border" id="daily_attendance_table">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>User</th>
                                ...
                                <th>Logout Time</th>
                                <th>Total Hour</th>
                                <th>Break Hour Time</th>
                                <th>Total Break Time</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($dailyAttendanceListsPaginated as $key => $today)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td><a
                                            href="{{ route('user.show', $today->employee->id) }}">{{ $today->employee->name }}</a>
                                    </td>
                                    <td>
                                        @php
                                            $text = 'Present';
                                            ...
                                            if($today->status == 2){
                                                $color = 'info';
                                                $text = 'Present - Half Day';
                                            }
                                        @endphp
                                        <span class="badge bg-{{ $color }} w-120">{{ $text }}</span>
                                    </td>
                                    <td>{{ $today->login_time ? Utility::convertDmyAMPMFormat($today->login_time) : '-' }}
                                    </td>
                                    <td>{{ $today->logout_time ? Utility::convertDmyAMPMFormat($today->logout_time) : '-' }}
                                    </td>
                                    <td>{{ $today->total_work_hour }}</td>
                                    <td>
                                        Meeting Break {{$today->meeting_break_time }} <br />
                                        Lunch Break {{$today->lunch_break_time }}<br />
                                        Tea Coffee Break {{$today->tea_coffee_break_time }}<br />
                                    </td>
                                    <td>{{ $today->break_time }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
@section('script')
    <script>
        $(document).ready(function() {
            var table = $('#daily_attendance_table').DataTable({
                initComplete: function() {
                    var $searchInput = $('#daily_attendance_table_filter input');
                    $searchInput.attr('id', 'daily_attendance_search');
                    $searchInput.attr('placeholder', 'Search Daily Attendance');
                },
                lengthChange: false,
                "order": [
                    [0, 'asc']
        ...
        function exportCSV() {
            ...
            var search = $('#daily_attendance_search').val();
            $('#format_error').html('');
            if (format.trim() == "") {
