...
<body>
    ...
    <h5 style="text-align: center">Disease Lists</h5>

    <table class="table">
        <thead>
            <th class="table">#</th>
            <th class="table">Disease Name</th>
            <th class="table">Created By</th>
            <th class="table">Created At</th>
        </thead>
        <tbody>
            @forelse ($diseasesList as $key=>$disease)
                <tr>
                    <td class="table">{{ $key +1 }}</td>
                    <td class="table">{{ $disease->name }}</td>
                    <td class="table">{{ isset($disease->userDetail) ? $disease->userDetail->name : '-' }}</td>
                    <td class="table">{{ Utility::convertDmyAMPMFormat($disease->created_at) }}</td>
                </tr>
            @empty
                <tr>
