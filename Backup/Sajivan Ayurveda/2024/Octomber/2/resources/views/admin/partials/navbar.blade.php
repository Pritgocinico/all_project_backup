...
@endif
    <div class="row align-items-center g-3 auth-name-nav">
        <h4>Hello, {{ Auth()->user()->name }} <span class="navbar-span"
    ...
    <div class="hstack flex-fill justify-content-end flex-nowrap gap-3 ms-auto px-6 px-xxl-8 d-nav-time">
        ...
        {{-- @if (Auth()->user() !== null && Auth()->user()->role_id != '1')
            ...
            <div class="app-navbar-item ms-2 ms-lg-6">
                <ul class="navbar-nav justify-content-end ">
                        ...
                        @if ($break->break_over != '')
                            <li class="nav-item">
                                <a href="javascript:void(0);"
                                    class="btn btn-primary mx-sm-3 mx-0 take_break text-nowrap btn-sm">
                    ...
                    @else
                        <li class="nav-item">
                            <a href="javascript:void(0);" class="btn btn-dark take_break mx-xs-3 mx-0 text-nowrap btn-sm">
                                <span>Take a Break</span>
                                <i class="fa-solid fa-clock"></i>
                            </a>
                        </li>
        ...
        <div class="app-navbar-item ms-2 ms-lg-6 d-flex align-items-center">
            <div class="col text-end">
                    <button class="btn btn-warning text-white btn-sm dropdown-toggle header-nav-search-dropdown navbar-btn" type="button"
                        data-bs-toggle="dropdown" aria-expanded="false" id="dropdownMenuButton">
                        Number
                    </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item search_dropdown"
                            href="javascript:void(0)" data-status="name">Name</a></li>
                    <li><a class="dropdown-item search_dropdown"
                            href="javascript:void(0)" data-status="mobile_number">Number</a>
                    </li>
                    <li><a class="dropdown-item search_dropdown"
                            href="javascript:void(0)" data-status="pin_code">Pin Code</a></li>
                    <li><a class="dropdown-item search_dropdown"
                            href="javascript:void(0)" data-status="order_id">Order Id</a></li>
                </ul>
            </div>
            <div class="w-sm-56 input-group input-group-sm input-group-inline header-nav-search-input">
                <input type="search" class="form-control" placeholder="Search" id="customerGlobal_search">
            </div>
        </div>

        <div class="header-nav-search-result-outer" id="search_data">
        ...
        <div class="dropdown"><a href="#" class="nav-link" id="dropdown-notifications " data-bs-toggle="dropdown"
            ...
            <div class="dropdown-menu dropdown-menu-end px-2 top-notification-bar" aria-labelledby="dropdown-notifications">
                <div class="dropdown-item d-flex align-items-center top-notification notification-header">
        ...
        <div class="dropdown"><a class="avatar avatar-sm rounded-circle" href="#" role="button"
                ...
                <img src="{{asset('assets/img/login/favicon-sajivan.png')}}"></a>
            <div class="dropdown-menu dropdown-menu-end">
                ...
                @if (Auth()->user(0)->role_id == 1)
                    {{-- <a class="dropdown-item {{request()->routeIs('business-setting')?'fw-bold':''}}" href="{{ route('business-setting') }}"><i class="fa-solid fa-pen-to-square me-3"></i>Business Setting</a> --}}
                    <a class="dropdown-item {{request()->routeIs('setting')?'fw-bold':''}}" href="{{ route('setting') }}"><i class="fa-solid fa-gear me-3"></i>Dashboard
                        Settings </a>
                @endif
                @if (Auth()->user()->role_id !== 1)
                    <a class="dropdown-item {{request()->routeIs('profile-view')?'fw-bold':''}}" href="{{ route('profile-view') }}"><i class="fa-solid fa-circle-user me-3"></i>User Profile</a>
                @else
                    <a class="dropdown-item {{request()->routeIs('change.password')?'':''}}" href="{{ route('change.password') }}"><i class="fa-solid fa-pen-to-square me-3"></i>Change Password</a>
                @endif
                <div class="dropdown-divider"></div>
                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                    @csrf
                    <button type="submit" class="dropdown-item"><i class="fa-solid fa-arrow-up-from-bracket me-3"></i>Logout</button>
                </form>
            </div>
        </div>
    </div>
