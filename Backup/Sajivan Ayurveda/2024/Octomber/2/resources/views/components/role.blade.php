{{-- @if (collect($accesses)->where('menu_id', '5')->first()->status != 0 ||
    ...
    <li class="nav-item my-1">
        <a class="nav-link d-flex align-items-center {{ request()->routeIs('service-preference.*') || request()->routeIs('user.*') ? 'active collapsed' : '' }}"
            href="#sidebar-user" data-bs-toggle="collapse" role="button"
            aria-expanded="{{ request()->routeIs('service-preference.*') || request()->routeIs('user.*') ? 'true' : 'false' }}"
        ...
        <div class="collapse {{ request()->routeIs('service-preference.*') || request()->routeIs('user.*') ? 'show' : '' }}"
            id="sidebar-user">
