...
@if (collect($accesses)->where('menu_id', '2')->first()->status != 0 ||
        collect($accesses)->where('menu_id', '3')->first()->status != 0 ||
    ...
    <li class="nav-item my-1">
        <a
            ...
            <ul class="nav nav-sm flex-column mt-1 inner-ul {{ request()->routeIs('department.*') || request()->routeIs('designation.*') || request()->routeIs('role.*') ? ' ' : '' }}"
                ...
                @if (collect($accesses)->where('menu_id', '2')->first() &&
                        collect($accesses)->where('menu_id', '2')->first()->status != 0)
                ...
                @endif
                @if (collect($accesses)->where('menu_id', '3')->first() &&
                        collect($accesses)->where('menu_id', '3')->first()->status != 0)
