...
class OrderController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $order_item->product_price = $product->price;
        $order_item->update();
        $name = Auth::guard('admin')->user()->name . " " .Auth::guard('admin')->user()->first_name . " " . Auth::guard('admin')->user()->last_name;
        $log = $order->order_id." Order Updated by ". $name;
    ...
    public function destroy($id)
    {
        ...
        try{        
            ...
            $transaction = Payment::where('order_id', $id)->first();
            if($transaction){
                $transaction->delete();
            }
        }catch(Exception $e){}
        $name = Auth::guard('admin')->user()->name . " " .Auth::guard('admin')->user()->first_name . " " . Auth::guard('admin')->user()->last_name;
        $log = $order->order_id." Order Deleted by ". $name;
        UserLogHelper::storeLog('Order', $log);
    ...
    public function generateInvoice(Request $request, $id)
    {
        ...
        $orders = Order::with('orderItemDetail','orderItemDetail.orderProductItemDetail', 'orderItemDetail.productDetail','orderPdfDetail', 'user')->findOrFail($id);
        $pdf = new Mpdf([
            'mode' => 'utf-8',
            'format' => 'A4',
            // 'orientation' => 'L'
    ...
    {
        $orders = Order::with('orderItemDetail', 'orderItemDetail.productDetail', 'user')->findOrFail($id);
        $pdf = new Mpdf([
            'mode' => 'utf-8',
            'format' => 'A4',
            // 'orientation' => 'L'
        ...
        if (isset($orders)) {
            // return view('admin.pdf.packageslip_pdf', compact('orders'));
