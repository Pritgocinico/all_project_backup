    ...
    public function getRates(Request $request)
    {
        ...
        $payload = [
            ...
            "requestedShipment" => [
                ...
                "serviceType" => $request->ship_method,
        ...
        try {
            ...
            if (isset($data['output']['rateReplyDetails'])) {
                foreach ($data['output']['rateReplyDetails'] as $rateReply) {
                    if (($rateReply['serviceType'] === 'STANDARD_OVERNIGHT' || $rateReply['serviceType'] === 'PRIORITY_OVERNIGHT') && $rateReply['packagingType'] === 'YOUR_PACKAGING') {
                        $shippingRates[] = [
            ...
            $pattern = '/{"transactionId":"[^"]+","errors":\[{"code":"([^"]+)"/';

            if (preg_match($pattern, $errorMessage, $matches)) {
                $errorCode = $matches[1];
                return response()->json(['error-getRates' => $errorCode], 500);
            } else {
                return response()->json(['error-getRates' => $errorMessage], 500);
            }
        }
    }
    public function generateLabel($id)
    {
        ...
        $shipData = [
            ...
            "requestedShipment" => [
                ...
                "shipDatestamp" => $date,
                "serviceType" => $order->shipping_method,
                "packagingType" => $type,
                "pickupType" => "USE_SCHEDULED_PICKUP",
