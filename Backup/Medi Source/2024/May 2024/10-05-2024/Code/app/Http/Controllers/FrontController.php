...
class FrontController extends Controller
{
    ...
    public function reOrder(Request $request, $id = NULL)
    {
        if (!blank($id)) {
            ...
            foreach ($orderItem as $key => $item) {
                ...
                $cartItem = CartItem::where('product_id', $item->product_id)->where('user_id', Auth()->user()->id)->where('package_name', $product_package->varient_name)->first();
                if (blank($cartItem)) {
                    if (isset($product)) {
                    ...
                    $cart_item = CartItem::where('product_id', $item->product_id)->where('user_id', Auth()->user()->id)->where('package_name', $product_package->varient_name)->first();
    ...
    public function defaultCard($id){
        $update = User::where('id',Auth()->user()->id)->update(['card_id'=>$id]);
