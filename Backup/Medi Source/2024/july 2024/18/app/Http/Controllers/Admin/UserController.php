...
class UserController extends Controller
{
    ...
    public function deleteDocument(Request $request, $id)
    {
        if ($id) {
            ...
            if ($document) {
                $destinationPath = storage_path('app/public/' . $document->document);
                if (file_exists($destinationPath)) {
                    unlink($destinationPath);
                }
                $document->delete();
                return redirect()->back()->with('success', 'User document deleted successfully');
            } else {
        ...
        if($request->input('status') == "active"){
            ...
            try {
                if($user->account_manager_name){
                    $manager = User::findOrFail($user->account_manager_name);
                    ...
                    Mail::send('admin.emails.account_manager', $datas, function($messages) use ($manager_email) {
                        $messages->from('info@medisourcerx.com', 'MedisourceRX')
                                ->to($manager_email)
