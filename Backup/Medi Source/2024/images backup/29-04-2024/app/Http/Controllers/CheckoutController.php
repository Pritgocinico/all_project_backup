...
class CheckoutController extends Controller
{
    ...
    public function placeOrder(Request $request){
        ...
        if(!$order){
            ...
            $cart_items = Cartitem::where('user_id',Auth::user()->id)->delete();
                $this->addOrderInQuickBook($order->id);
            try {
            } catch (\Throwable $th) {
            }
            $orders = Order::with('orderItemDetail', 'orderItemDetail.productDetail')->findOrFail($order->id);
            $email = $order->email;
            $mpdf = new Mpdf();
            ...
            $orderId = $order->order_id . " Order Generated";
    ...
    public function addOrderInQuickBook($id){
        ...
        $quickBookCustomerID = Auth()->user()->quick_book_cus_id;
        ...
        foreach ($orders->orderItemDetail as $item) {
                $itemDetail = Item::create([
                    "Name" => $item['product_name'],
                    "Description" => "Sample product description",
                    "Active" => true,
                    "TrackQtyOnHand" => true,
                    "QtyOnHand" => 100, // Initial quantity
                    "InvStartDate" => "2024-04-29" // Inventory start date
                ]);
                $resultingItem = $dataService->Add($itemDetail);
                $line = [
                    "Amount" => $item->total,
            ...
            $salesOrder = Invoice::create([
                ...
                "CustomerRef" => $quickBookCustomerID,
                "TotalAmt" => $totalAmount,
            ]);
            
        $salesOrder = $dataService->Add($salesOrder);
        if(isset($salesOrder)){
            Order::where('id',$orders->id)->update(['quickbooks_invoice_id'=>$salesOrder->Id]);
        }
        return $salesOrder;
    }
