        ...
        <div class="card">
             <div class="card-header card-no-border">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{route('admin.add_subcategory_data')}}" method="POST" class="row g-3" enctype="multipart/form-data">
                        @csrf
                        <div class="col-md-12">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="number" class="form-control" name="gst" id="GST" placeholder="Third Party Gst Rate" value="{{old('gst')}}">
                            @if ($errors->has('gst'))
                        ...
                        <div class="col-md-12 mt-4">
                            <div class="form-check align-items-center">
                                ...
                                <label for="Renewable" class="form-label ms-2 mb-0">Is Renewable ? {{old('description')}}</label>
                            </div>
                        ...
                        @if (count($sub_categories) > 0)
                            @foreach ($sub_categories as $category)
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <ul class="action"> 
                                            <li class="edit"><a href="{{ route('admin.edit_subcategory',$category->id) }}" class=""><i class="icon-pencil-alt"></i></a></li>
                                            <li class="delete"><a href="javascript:void(0);" data-id="{{ $category->id }}" class="ms-2 delete-btn"><i class="icon-trash"></i></i></a> </li>
                                            @if ($category->parent == 1)
                                            <li class="edit"> <a href="{{ route('admin.tp_calculationparameter',$category->id) }}" class="ms-2"><i class="icon-calendar"></i></a></li>
                                            @endif
                                        </ul>
                                    </td>
                                </tr>
    ...
    $(document).ready(function(){
        $(document).on('click','.delete-btn',function(){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url : "{{route('delete.subcategory', '')}}"+"/"+category_id,
                    type : 'GET',
                    dataType:'json',
                    success : function(data) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                top.location.href="{{ route('admin.sub_categories') }}";
                            }
                        });
                    }
                });
