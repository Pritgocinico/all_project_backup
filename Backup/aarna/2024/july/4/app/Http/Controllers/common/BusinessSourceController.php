...
class BusinessSourceController extends Controller
{
    ...
    public function addBusinessSource(Request $request){
        ...
        $plans = Plan::where('status',1)->where('company',$companies[0]['id'])->get();
        if(Auth::user()->role == 1){
            return view('admin.business_source.add_business_source',compact('page','icon','categories','sub_categories','companies','customers','plans'));
        }else{
            return redirect()->route('login');
        }
    }
    ...
    public function editBusinessSource(Request $request, $id = NULL){
        ...
        if(Auth::user()->role == 1){
            return view('admin.business_source.edit_business_source',compact('page','icon','categories','sub_categories','companies','customers','plans','sources'));
        }else{
            return redirect()->route('login');
        }
    }
