...
public function policies(Request $request){
        if($request->has('search')){
            if(Auth::user()->role == 1 || Auth::user()->role == 2){
                $query = Policy::latest()->where('status','!=',2)->orderBy('id','Desc');
            }elseif(Auth::user()->role == 3){
                $query = Policy::latest()->where('status','!=',2)->where('agent',Auth::user()->id)->orderBy('id','Desc');
            }else{
                $query = Policy::latest()->where('status','!=',2)->where('created_by',Auth::user()->id)->orderBy('id','Desc');
            }
            ...
            $policies = $query->orderBy('id','Desc')->paginate(10);
        }else{
            // $policies = Policy::latest()->get()->where('business_type',1)->where('status','!=',2);
            if(Auth::user()->role == 1 || Auth::user()->role == 2){
                $policies = Policy::where('status','!=',2)->orderBy('id','Desc')->latest()->paginate(10);
                // $policies = Policy::latest()->where('status','!=',2)->orderBy('id','Desc')->paginate(10);
            }elseif(Auth::user()->role == 3){
                $policies = Policy::latest()->where('agent',Auth::user()->id)->where('status','!=',2)->orderBy('id','Desc')->paginate(10);
            }else{
                $policies = Policy::latest()->where('created_by',Auth::user()->id)->where('status','!=',2)->orderBy('id','Desc')->paginate(10);
            }
        }
