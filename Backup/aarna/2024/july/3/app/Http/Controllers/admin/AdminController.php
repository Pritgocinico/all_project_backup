    ...
    public function dashboard(Request $req){
        ...
        if(Auth::user()->role != 3){
             if($req->has('from_date') && $req->has('to_date')){
                ...
                $totalPremium = Policy::where('status', 1)->sum('net_premium_amount');
                $sales_overview['npa'] = $totalPremium;
                $sales_overview['nop'] = Policy::where('status',1)->count();
                $sales_overview['motor'] = Policy::where('insurance_type',1)->where('category',1)->where('status',1)->count();
                $sales_overview['nonmotor'] = Policy::where('insurance_type',1)->where('category',2)->where('status',1)->count();
                $sales_overview['helth'] = Policy::where('insurance_type',2)->where('status',1)->count();
                $sales_overview['renewal'] = Policy::where('business_type', '=',2)->where('status',1)->count();
                $sales_overview['new'] = Policy::where('business_type', '!=',2)->where('status',1)->count();
            }
            ...
            return view('admin.dashboard',compact('page','sales_overview','icon','covernotes','claims','renewal','lob','last_month_lob','stacked','chart','agents','health_policy','vehicle_policy','payout_list'));
        }else{
