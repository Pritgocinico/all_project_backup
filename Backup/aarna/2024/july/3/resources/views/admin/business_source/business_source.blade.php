@extends('admin.layouts.app')

@section('content')
<div class="container-fluid">
    <div class="row">
        <div class="col-sm-12">
        <div class="card">
            <div class="card-header justify-content-between d-flex card-no-border">
            <h4>Business Source List</h4>
            @if(Auth::user()->id == 1)
                          <a href="{{route('admin.add.business.source')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New Source</a>
                @else
                    @foreach($permissions as $permission)
                       @if($permission->capability == 'source-create' && $permission->value == 1)
                            <a href="{{route('admin.add.business.source')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New Source</a>
                        @endif
                    @endforeach
                @endif
            </div>
        <div class="card-body">
          <div class="table-responsive custom-scrollbar">
            <table class="display border" id="basic-1">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Insurance Type</th>
                        <th>Customer</th>
                        <th>Business Type</th>
                        <th>Category</th>
                        <th>Risk Start Date</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    @if (!blank($business_source))
                        @foreach ($business_source as $source)
                            <tr>
                                <td>{{$loop->index+1}}</td>
                                <td>
                                    @if ($source->insurance_type == 1)
                                        Motor Insurance
                                    @else
                                        Health Insurance
                                    @endif
                                </td>
                                <td>
                                    @foreach ($customers as $customer)
                                        @if ($customer->id == $source->customer)
                                            {{$customer->name}}
                                        @endif
                                    @endforeach
                                </td>
                                <td>
                                    @if($source->insurance_type == 1)
                                        @if ($source->business_type == 1)
                                            New
                                        @elseif ($source->business_type == 2)
                                            Renewal
                                        @elseif ($source->business_type == 3)
                                            Rollover
                                        @elseif ($source->business_type == 4)
                                            Used
                                        @endif
                                    @else
                                        @if ($source->business_type == 1)
                                            New
                                        @elseif ($source->business_type == 2)
                                            Renewal
                                        @elseif ($source->business_type == 3)
                                            Rollover
                                        @elseif ($source->business_type == 4)
                                            Used
                                        @endif
                                    @endif
                                </td>
                                <td>
                                    @if($source->insurance_type == 1)
                                        @foreach ($categories as $item)
                                            @if ($item->id == $source->category)
                                                {{$item->name}}
                                            @endif
                                        @endforeach
                                    @else
                                        @if ($source->category == 1)
                                            Base
                                        @elseif ($source->category == 2)
                                            Personal Accident
                                        @elseif ($source->category == 3)
                                            Suprer Yopup
                                        @endif
                                    @endif
                                </td>
                                <td>
                                    {{date('d-m-Y',strtotime($source->risk_start_date))}}
                                </td>
                                <td>
                                    <ul class="action">
                                        @if(Auth::user()->id == 1)
                                             <li class="edit"><a href="{{route('admin.edit_business_source',$source->id)}}"><i class="icon-pencil-alt"></i></a></li>
                                             <li class="delete"><a href="javascript:void(0);" data-id="{{ $source->id }}" class="delete-btn"><i class="icon-trash"></i></a></li>
                                        @else
                                            @foreach($permissions as $permission)
                                                @if($permission->capability == 'source-edit' && $permission->value == 1)
                                                     <li class="edit"><a href="{{route('admin.edit_business_source',$source->id)}}"><i class="icon-pencil-alt"></i></a></li>
                                                @endif
                                                @if($permission->capability == 'source-delete' && $permission->value == 1)
                                                    <li class="delete"><a href="javascript:void(0);" data-id="{{ $source->id }}" class="delete-btn"><i class="icon-trash"></i></a></li>
                                                @endif
                                            @endforeach
                                        @endif
                                    </ul>
                                </td>
                            </tr>
                        @endforeach
                    @endif
                </tbody>
            </table>
        </div>
    </div>
</div>
</div>
</div>
</div>
@endsection
@section('script')
<script>
    $(document).ready(function(){
        $(document).on('click','.delete-btn',function(){
            var user_id = $(this).attr('data-id');
            Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert this!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, delete it!'
            }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url : "{{route('delete.source', '')}}"+"/"+user_id,
                    type : 'GET',
                    dataType:'json',
                    success : function(data) {
                        Swal.fire({
                            title: 'Deleted!',
                            text: "Business Source has been deleted.",
                            icon: 'success',
                            showCancelButton: false,
                            confirmButtonColor: '#3085d6',
                            cancelButtonColor: '#d33',
                            confirmButtonText: 'Ok'
                            }).then((result) => {
                            if (result.isConfirmed) {
                                location.reload();
                            }
                        });
                    }
                });
            }
            });
        });
    });
</script>
@endsection
