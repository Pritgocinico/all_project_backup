...
class PolicyController extends Controller
{
    ...
    public function getPolicyData(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Policy::latest()->where('status', '!=', 2)->where('business_type', 1)->where('policy_type', 1);
...
public function policies(Request $request){
        if($request->has('search')){
            if(Auth::user()->role == 1 || Auth::user()->role == 2){
                $query = Policy::latest()->where('status','!=',2)->orderBy('created_at','Desc');
            }elseif(Auth::user()->role == 3){
                $query = Policy::latest()->where('status','!=',2)->where('agent',Auth::user()->id)->orderBy('created_at','Desc');
            }else{
                $query = Policy::latest()->where('status','!=',2)->where('created_by',Auth::user()->id)->orderBy('created_at','Desc');
            }
            ...
            if(Auth::user()->role == 1 || Auth::user()->role == 2){
                $policies = Policy::where('status','!=',2)->where('policy_type',1)->orderBy('created_at','Desc')->latest()->paginate(20);
                // $policies = Policy::latest()->where('status','!=',2)->orderBy('id','Desc')->paginate(10);
            }elseif(Auth::user()->role == 3){
                $policies = Policy::latest()->where('agent',Auth::user()->id)->where('status','!=',2)->where('policy_type',1)->orderBy('created_at','Desc')->paginate(20);
            }else{
                $policies = Policy::latest()->where('created_by',Auth::user()->id)->where('status','!=',2)->where('policy_type',1)->orderBy('created_at','Desc')->paginate(20);
            }
        }   
