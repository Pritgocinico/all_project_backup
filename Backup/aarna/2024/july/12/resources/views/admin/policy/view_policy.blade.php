...
<div class="col-xl-12"> 
                <div class="card"> 
                  <div class="card-header card-no-border pb-0 justify-content-between d-flex">
                    ...
                    <a href="{{route('admin.policy.history',$policy->id)}}" class="btn btn-primary me-2"><i class="icon-time"></i> History</a>
                  </div>
                  <div class="card-body"> 
                    <ul class="nav nav-tabs border-tab border-0 mb-0 nav-danger" id="topline-tab" role="tablist">
                      <li class="nav-item">
                         <a href="#pills-details" class="nav-link me-2 active" id="pills-details-tab" data-bs-toggle="tab"  role="tab" aria-controls="pills-details" aria-selected="true">Policy Details</a>
                     </li>
                    @if($policy->insurance_type == 1)
                    <li class="nav-item">
                        <a href="#pills-vehicleinfo" class="nav-link me-2" id="pills-vehicleinfo-tab" data-bs-toggle="tab" role="tab" aria-controls="pills-vehicleinfo" aria-selected="true">Vehicle Info</a>
                    </li>
                        @endif
                    <li class="nav-item">
                        <a href="#pills-customer" class="nav-link me-2" id="pills-customer-tab" data-bs-toggle="tab" role="tab" aria-controls="pills-customer" aria-selected="false">Customer Info</a>
                    </li>
                    <li class="nav-item">
                        <a href="#pills-attachments" class="nav-link me-2" id="pills-attachments-tab" data-bs-toggle="tab" role="tab" aria-controls="pills-attachments" aria-selected="false">Attachments</a>
                    </li>
                    <li class="nav-item">
                        <a href="#pills-payments" class="nav-link me-2" id="pills-payments-tab" data-bs-toggle="tab" role="tab" aria-controls="pills-payments" aria-selected="false">Payments</a>
                    </li>  
                    </ul>
                    <div class="tab-content" id="topline-tabContent">
                      ...
                      <div class="tab-pane fade" id="pills-vehicleinfo" role="tabpanel" aria-labelledby="pills-vehicleinfo-tab">
                        <div class="card-body px-0 pb-0"> 
                          <div class="user-header pb-2"> 
                          </div>
                          <div class="user-content"> 
                                    ...
                                    @foreach ($parameters as $item)
                                        @if ($item->type == 1)
                                            <tr>
                                                ...
                                                <td>
                                                    <?php $para = DB::table('parameters')->where('id',$item->value)->first(); ?>
                                                    {{$para->carrier}}
                                                </td>
                                            </tr>
                                        @elseif ($item->type == 2)
                                ...
                                @php
                                    ...
                                    if(!blank($tp_param)){
                                        foreach($tp_param as $param){
                                            // echo '<pre>';
                                            // print_r($param->type);
                                            $para = DB::table('parameters')->where('id',$param->parameter_id)->first();
                                            if (!blank($para)) {
                                                if ($para->type != 3) {
                                                    if ($param->type == 7 && ($para->display_type == 'text' || $para->display_type == 'dropdown')) {
                                                        if (property_exists($param, 'value') && property_exists($para, 'carrier_value')) {
                                                            $tp += (float)$param->value * (int)$para->carrier_value;
                                                        }
                                                    } else {
                                    ...
                                    // echo $tp.'<br>';
                                    if($policy->policy_type == 1 && $policy->policy_individual_rate != NULL){
                                    ...
                                    <td>
                                        @if (!blank($attachments))
                                            @foreach ($attachments as $attachment)
                                                <a href="{{url('/')}}/policy_attachment/{{$attachment->file}}" target="_blank"><img src="{{url('/')}}/assets/Images/docs.png" width="40px" alt="">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (!blank($documents))
                                            ...
                                            <a href="{{url('/')}}/policy_document/{{$doc->file}}" target="_new"  title="{{$doc->file_name}}"><img src="{{url('/')}}/assets/Images/docs.png" width="40px" alt=""></a>
                                            @endforeach
                                        @endif
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($payment->payment_type == 1)
                                                        ...
                                                        <p><strong>Payment Made By : </strong> {{$payment->made_by}}</p>
                                                    @elseif($payment->payment_type == 2)
                                                        <p><strong>Payment Date : </strong>{{$payment->payment_date}}</p>
                                                        <p><strong>Cheque No : </strong>{{$payment->cheque_no}}</p>
                                                        <p><strong>Cheque Date : </strong> {{date('d-m-Y',strtotime($payment->cheque_date))}}</p>
