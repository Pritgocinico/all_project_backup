                            ...
                            <table class="table mb-0">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td id="vehicle_chassis_no">{{ $policy->vehicle_chassis_no ?? '' }}</td>
                                        <td>Vehicle Registration No</td>
                                        <td id="vehicle_registration_no">{{ $policy->vehicle_registration_no ?? '' }}</td>
                                    </tr>
                                    <tr>
                            ...
                            <select class="form-control" name="subcategory" id="SubCategory"
                                ...
                                @foreach ($sub_categories as $sub_category)
                                    <option value="{{ $sub_category->id }}"
                                        @if (isset($policy->sub_category) && $sub_category->id == $policy->sub_category) selected @endif>{{ $sub_category->name }}
                                    </option>
                                @endforeach
                            </select>
                                    ...
                                    <div class="custom-date-div-outer justify-content-between d-flex">
                                        <span class="custom-date-span w-100">
                                            <input type="date"
                                                ...
                                                value="{{ isset($policy) ? date('Y-m-d', strtotime($policy->risk_end_date . ' +1 day')) : "" }}"
                                                placeholder="" />
                                        </span>
                                        ...
                                        <span class="custom-date-span w-100">
                                            <input type="date" class="form-control custom-end-date-div custom-date-div"
                                                ...
                                                value="{{ isset($policy) ? date('Y-m-d', strtotime($policy->risk_end_date . '+1 year')) : "" }}"
        ...
        $('#policies_number').on('change', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#InsuranceCompany').val(data.company.id)
                    $('#sub_category_name').html(data.category.name)
                    $('#category_name').html(data.category_detail.name)
                    $('#Category').val(data.category_detail.id)
                    $('#SubCategory').val(data.category.id)
                    if (data.insurance_type == 2) {
                        ...
                        $('#Category').val(data.health_category)
                        $('#SubCategory').val(data.health_sub_category)
                    }
                    var indvidual = "";
                    ...
                    $('#vehicle_chassis_no').html(data.vehicle_chassis_no)
                    $('#vehicle_registration_no').html(data.vehicle_registration_no)
                    ...
                    $('#vehicleChassisNo').val(data.vehicle_chassis_no)
                    $('#vehicleMake').val(data.vehicle_make)
                    $('#vehicleRegistrationNo').val(data.vehicle_registration_no)
                    $('#yearOfManufacture').val(data.year_of_manufacture)
                    $('#PypNo').val(data.policy_no)
                    $('#SourcingAgent').val(data.agent.id)
                    $('#RiskStartDate').val(moment(data.risk_end_date, 'YYYY-MM-DD').add(1, 'days').format('YYYY-MM-DD'))
                    $('#RiskEndDate').val(moment(data.risk_end_date, 'YYYY-MM-DD').add(1, 'years').format('YYYY-MM-DD'))
                },
            });
