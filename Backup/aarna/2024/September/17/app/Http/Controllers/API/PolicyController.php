...
class PolicyController extends Controller
{
    ...
    if (!blank($policies)) {
        foreach ($policies as $policy) {
            ...
            $array['category'] = ($policy->category != NULL) ? (int)$policy->category : "";
            $CategoryBean = array();
            $healthCategory = Category::where('id',$policy->health_category)->first();
            if (!blank($healthCategory)) {
                $CategoryBean['categoryId'] = $healthCategory->id;
                $CategoryBean['categoryName'] = $healthCategory->name;
            }
            $array['healthCategoryBean'] = (object) $CategoryBean;
            $subCategoryBean = array();
            $subHealthCategory = Category::where('id', $policy->health_sub_category)->first();
            if (!blank($subHealthCategory)) {
                $subCategoryBean['subCategoryId']           = $subHealthCategory->id;
                $subCategoryBean['subCategoryName']         = $subHealthCategory->name;
                $subCategoryBean['categoryId']              = $subHealthCategory->parent;
                $category = Category::where('id',$subHealthCategory->parent)->first();
                $subCategoryBean['categoryName']            = $category->name;
                $subCategoryBean['isRenewable']             = $category->renewable;
            }
            $array['subCategoryBean'] = (object) $subCategoryBean;
    ...
    public function addHealthInsurancePolicy(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $policy->health_category                       = $request->categoryId;
