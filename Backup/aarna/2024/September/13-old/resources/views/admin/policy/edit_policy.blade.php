...
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.edit.policy.data') }}" method="post" class="g-3 " id="formDropzone"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-12 form-floating mt-4">
                            <select class="form-control" name="insurance_type" id="InsuranceType" placeholder="">
                                <option value="1" @if ($policy->insurance_type == 1) selected @endif>Motor Insurance
                                    Policy</option>
                                <option value="2" @if ($policy->insurance_type == 2) selected @endif>Health Insurance
                                    Policy</option>
                            </select>
    ...
    @endforeach
                        ...
                        <div class="col-md-6 form-floating mt-4 motor">
                            <select class="form-control" name="category" id="Category" placeholder="">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if ($policy->category == $category->id) selected @endif>
                        ...
                        <div class="col-md-6 mt-4 PolicyType Policy motor" style="display:none;">
                            ...
                            <input type="radio" name="policy_type" value="1" id="individual"
                                class="policy_type policy" @if ($policy->policy_type == 1 || $policy->policy_type == null) checked @endif><label
                                class="mx-2" for="individual"> Individual </label>
                            <input type="radio" name="policy_type" value="2" id="corporate"
                                class="policy_type policy" @if ($policy->policy_type == 2) checked @endif><label
                                class="mx-2" for="corporate"> Corporate </label>
                        ...
                        <div class="col-md-6 form-floating mt-4 PolicyIndividualRate Policy motor" style="display:none;">
                            <input type="text" class="form-control" name="policy_individual_rate"
                                id="PolicyIndividualRate" value="{{ $policy->policy_individual_rate }}" placeholder="">
                        ...
                        <div class="col-md-3 form-floating mt-4 ">
                            <select class="form-control" name="payout_restricted" id="payout_restricted"
                                value="{{ old('payout_restricted') }}">
                                <option value="">Select Payout Restricted</option>
                                <option value="yes" @if ($policy->payout_restricted == 'yes') selected @endif>Yes</option>
                                <option value="no" @if ($policy->payout_restricted == 'no') selected @endif>No</option>
                            </select>
                        ...
                        <div
                            class="col-md-3 form-floating mt-4 payout_restricted_remark_div @if ($policy->payout_restricted == 'yes') d-block @else d-none @endif">
                            <input type="text" class="form-control" name="payout_restricted_remark"
                                id="payout_restricted_remark" value="{{ $policy->payout_restricted_remark }}"
                                placeholder="">
                        ...
                        <div class="col-md-12 form-floating mt-4">
                            <select class="form-control" name="insurance_company" id="InsuranceCompany"
                                value="{{ $policy->company }}" placeholder="">
                                @foreach ($companies as $company)
                                    <option value="{{ $company->id }}"
                                        @if ($company->id == $policy->company) selected @endif>{{ $company->name }}</option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control" name="health_category" id="HealthCategory" placeholder="">
                                ...
                                @foreach ($healthCategory as $health)
                                    <option value="{{ $health->id }}"
                                        @if ($policy->health_category == $health->id) selected @endif>{{ $health->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 form-floating mt-4 health" style="display:none;">
                            <select class="form-control" name="health_sub_category" id="healthSubCategory" placeholder="">
                                <option value="">Select Health Sub Category</option>
                                @foreach ($health_sub_categories as $healthSub)
                                <option value="{{ $healthSub->id }}"
                                    @if ($policy->health_sub_category == $healthSub->id) selected @endif>{{ $healthSub->name }}</option>
                                @endforeach
                            </select>
                            <label for="healthSubCategory" class="form-label">Sub Category *</label>
                            @if ($errors->has('health_sub_category'))
                            <span class="text-danger">{{ $errors->first('health_sub_category') }}</span>
                            @endif
                            <span class="text-danger health_sub_category"></span>
                        </div>
                        <div class="col-md-6 form-floating mt-4 health" style="display:none;">
                                ...
                                @if (!blank($plans))
                                    @foreach ($plans as $plan)
                                        <option value="{{ $plan->id }}"
                                            @if ($policy->health_plan == $plan->id) selected @endif>{{ $plan->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="col-md-6 form-floating mt-4 motor">
                            <input type="text" class="form-control" name="covernote_no" id="covernote_no"
                                value="{{ $policy->covernote_no }}" placeholder="">
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="text" class="form-control" name="policy_no" id="policy_no"
                                value="{{ $policy->policy_no }}" placeholder="">
                                            ...
                                            <select class="form-control select2" name="customer" id="Customer"
                                                ...
                                                @foreach ($customers as $customer)
                                                    <option value="{{ $customer->id }}"
                                                        @if ($customer->id == $policy->customer) selected @endif>
                        ...
                        <div class="col-md-6 form-floating mt-4 motor">
                            <input type="text" class="form-control" name="vehicle_engine" id="vehicleEngine"
                                value="{{ $policy->vehicle_engine }}" placeholder="">
                        ...
                        <div class="col-md-6 form-floating mt-4 motor">
                            <input type="number" class="form-control" min="0" name="idv_amount" id="IdvAmount"
                                value="{{ $policy->idv_amount }}" placeholder="" />
                        ...
                        <div class="col-md-6 form-floating mt-4 health" style="display:none;">
                            <input type="number" class="form-control" min="0" name="sum_insured_amount"
                                id="SumInsuredAmount" value="{{ $policy->sum_insured_amount }}" placeholder="" />
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="number" class="form-control" min="0" step="0.01"
                                name="gross_premium_amount" id="GrossPremiumAmount"
                                value="{{ $policy->gross_premium_amount }}" placeholder="" />
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="number" class="form-control" min="0" step="0.01"
                                name="net_premium_amount" id="NetPremiumAmount"
                                value="{{ $policy->net_premium_amount }}" placeholder="" />
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="number" name="tp" min="0" step="0.01" id="TP"
                                class="form-control" value="{{ $policy->tp }}" placeholder="">
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="number" name="od" min="0" step="0.01" id="OD"
                                class="form-control" value="{{ $policy->od }}" placeholder="">
                        ...
                        <div class="col-md-6 form-floating mt-4 motor">
                            <select class="form-control" name="business_type" id="BusinessType"
                                value="{{ $policy->business_type }}" placeholder="">
                                ...
                                <option value="3" @if ($policy->business_type == 3) selected @endif>Rollover
                                </option>
                        ...
                        <div class="col-md-6 form-floating mt-4 health">
                            <select class="form-control" name="health_business_type" id="HealthBusinessType"
                                placeholder="">
                                <option value="1" @if ($policy->business_type == 1) selected @endif>New</option>
                                <option value="2" @if ($policy->business_type == 2) selected @endif>Renewal</option>
                                <option value="3" @if ($policy->business_type == 3) selected @endif>Portability
                                </option>
                            </select>
                        ...
                        <div
                            class="col-md-6 form-floating mt-4 pyp business_type @if (old('business_type')) @if (old('business_type') == 1 || old('business_type') == 4) d-none @endif
@elseif($policy->business_type == 1 || $policy->business_type == 4)
d-none @endif">
                            <input type="text" name="pyp_no" id="PypNo" value="{{ $policy->pyp_no }}"
                                class="form-control">
...
@elseif($policy->business_type == 1 || $policy->business_type == 4)
d-none @endif">
                            <input type="text" name="pyp_insurance_company" id="PypInsuranceCompany"
                                value="{{ $policy->pyp_insurance_company }}" class="form-control">
...
@elseif($policy->business_type == 1 || $policy->business_type == 4)
d-none @endif">
                            <input type="date" name="pyp_expiry_date" id="PypExpiryDate"
                                value="{{ date('Y-m-d', strtotime($policy->pyp_expiry_date)) }}" class="form-control">
                            ...
                            <select class="form-control" name="sourcing_agent" id="SourcingAgent" placeholder="">
                                ...
                                @foreach ($sourcing_agents as $agent)
                                    <option value="{{ $agent->id }}"
                                        @if ($policy->agent == $agent->id) selected @endif>{{ $agent->name }}</option>
                                @endforeach
                            </select>
                                    ...
                                    <div class="custom-date-div-outer justify-content-between d-flex">
                                        <span class="custom-date-span w-100">
                                            <input type="date"
                                                ...
                                                value="{{ date('Y-m-d', strtotime($policy->risk_start_date)) ? date('Y-m-d', strtotime($policy->risk_start_date)) : date('Y-m-d') }}"
                                                placeholder="" />
                                        </span>
                                        ...
                                        <span class="custom-date-span w-100">
                                            <input type="date" class="form-control custom-end-date-div custom-date-div"
                                                ...
                                                value="{{ date('Y-m-d', strtotime($policy->risk_end_date)) ? date('Y-m-d', strtotime($policy->risk_end_date)) : date('Y-m-d', strtotime('+1 year -1 day')) }}"
                                                placeholder="" />
                                        </span>
                                    </div>
                                </div>
                                ...
                                @if (!blank($team_lead))
                                    @foreach ($team_lead as $lead)
                                        <option value="{{ $lead->id }}"
                                            @if ($policy->team_lead == $lead->id) selected @endif>{{ $lead->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                                ...
                                @if (!blank($managed_by))
                                    @foreach ($managed_by as $manage)
                                        <option value="{{ $manage->id }}"
                                            @if ($policy->managed_by == $manage->id) selected @endif>{{ $manage->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="col-md-6 form-floating mt-4">
                            <input type="text" name="rto_name" id="RtoName" class="form-control"
                                value="{{ $policy->rto_name }}" placeholder="">
                                        ...
                                        <div class="row payment-{{ $payments->id }}">
                                            <div class="col-md-6 form-floating mt-4">
                                                <select class="form-control paymentType"
                                                    name="payment_type[{{ $i }}]"
                                                    id="PaymentType{{ $i }}" data-id="{{ $i }}"
                                                    placeholder="">
                                                    <option value="0">Select Payment Option</option>
                                                    <option value="1"
                                                        @if ($payments->payment_type == 1) selected @endif>Cash</option>
                                                    <option value="2"
                                                        @if ($payments->payment_type == 2) selected @endif>Cheque</option>
                                                    <option value="3"
                                                        @if ($payments->payment_type == 3) selected @endif>Online</option>
                                                </select>
                                                <label for="PaymentType" class="form-label">Payment Type</label>
                                                <input type="hidden" name="hidden[{{ $i }}]"
                                                    value="{{ $payments->id }}">
                                            </div>
                                            <div class="col-md-6 form-floating mt-4">
                                                <input type="date" name="payment_date[{{ $i }}]"
                                                    id="paymentDate-{{ $i }}"
                                                    value="{{ date('Y-m-d', strtotime($payments->payment_date)) }}"
                                                    class="form-control">
                                                <label for="paymentDate-{{ $i }}">Payment Date</label>
                                            </div>
                                            <div class="col-md-6 form-floating mt-4">
                                                <select name="payment_made_by[{{ $i }}]"
                                                    id="paymentMadeBy-{{ $i }}" class="form-control">
                                                    <option value="AGENT"
                                                        @if ($payments->made_by == 'AGENT') selected @endif>AGENT</option>
                                                    <option value="CUSTOMER"
                                                        @if ($payments->made_by == 'CUSTOMER') selected @endif>CUSTOMER
                                                    </option>
                                                </select>
                                                <label for="paymentMadeBy-{{ $i }}">Payment Made By</label>
                                            </div>
                                            <div class="row cheque cheque-{{ $i }}"
                                                @if ($payments->payment_type != 2) style="display:none;" @endif>
                                                <div class="col-md-6 form-floating mt-4">
                                                    <input type="text" class="form-control"
                                                        name="cheque_no[{{ $i }}]"
                                                        value="{{ $payments->cheque_no }}" id="chequeNo"
                                                        placeholder="" />
                                                    <label for="chequeNo" class="form-label">Cheque No *</label>
                                                </div>
                                                <div class="col-md-6 form-floating mt-4">
                                                    <input type="date" class="form-control"
                                                        name="cheque_date[{{ $i }}]" id="cheque_date"
                                                        value="{{ date('Y-m-d', strtotime($payments->cheque_date)) }}"
                                                        placeholder="" />
                                                    <label for="cheque_date" class="form-label">Cheque Date *</label>
                                                </div>
                                                <div class="col-md-6 form-floating mt-4">
                                                    <input type="text" class="form-control"
                                                        name="bank_name[{{ $i }}]" id="BankName"
                                                        value="{{ $payments->bank_name }}" placeholder="" />
                                                    <label for="BankName" class="form-label">Bank Name *</label>
                                                </div>
                                            </div>
                                            <div class="col-md-6 online online-{{ $i }}"
                                                @if ($payments->payment_type != 3) style="display:none;" @endif>
                                                <div class="col-md-12 form-floating mt-4">
                                                    <input type="text" class="form-control"
                                                        name="transaction_no[{{ $i }}]" id="TransactionNo"
                                                        value="{{ $payments->transaction_no }}" placeholder="" />
                                                    <label for="TransactionNo" class="form-label">Transaction No *</label>
                                                </div>
                                            </div>
                                            <div class="col-md-12 mt-3">
                                                <a href="javascript:void(0);" class="btn btn-danger delete-policy-payment"
                                                    data-id="{{ $payments->id }}">Remove</a>
                                            </div>
                                        </div>
                                    @endforeach
                                @endif
                                        ...
                                        <div class="row col-md-12 mt-3 policy-doc-{{ $doc->id }}">
                                            <div class="col-md-1">
                                                <a href="{{ url('/') }}/policy_document/{{ $doc->file }}"
                                                    role="button" class="cursor-pointer" download><img
                                                        src="{{ url('/') }}/assets/Images/docs.png" width="40px"
                                                        role="button" class="img-fluid cursor-pointer"
                                                        alt=""></a>
                                            </div>
                                            ...
                                            <div class="col-md-5 d-flex align-items-center">
                                                <a href="javascript:void(0);" role="button"
                                                    class="btn btn-danger delete-policy-doc"
                                                    data-id="{{ $doc->id }}">Delete</a>
                                            </div>
                                        </div>
                                    @endforeach
                                @endif
                        ...
                        <div class="col-md-12 my-3">
                            <a href="javascript:void(0);" class="btn btn-primary addDocument" role="button">Add
                                Document</a>
                        </div>
                        <hr class="mt-3">
                    </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            var companyID = "{{ $policy->company }}";
            var healthPlan = "{{ $policy->health_plan }}"
            if (companyID !== null) {
                $.ajax({
                    type: 'GET',
        ...
        $(document).on('click', '.addDocument', function() {
            ...
            html += '<a href="javascript:void(0);" role="button" class="text-danger delete-doc delete-doc-' + i +
                '" data-id="' + i + '">Delete</a>';
            html += '</div>';
            html += '</div>';
        ...
        $(document).on('click', '.AddPayment', function() {
            ...
            var html = '';
            var d = new Date();
            var today = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, '0') + "-" + String(d
                .getDate()).padStart(2, '0');
            html += '<div class="row payment-' + i + '">';
            html += '<div class="col-md-6 form-floating mt-4">';
            html += '<select class="form-control paymentType" name="payment_type[' + i + ']" id="PaymentType' + i +
                '" data-id="' + i + '" placeholder="">';
            ...
            html += '</div>';
            html += '<div class="col-md-6 form-floating mt-4">';
            html += '<input type="date" name="payment_date[' + i + ']" value="' + today + '" id="paymentDate-' + i +
                '" class="form-control">';
            ...
            html += '<div class="col-md-6 form-floating mt-4">';
            html += '<input type="text" class="form-control" name="cheque_no[' + i +
                ']" id="chequeNo" placeholder="" />';
            html += '<label for="chequeNo" class="form-label">Cheque No *</label>';
            html += '</div>';
            html += '<div class="col-md-6 form-floating mt-4">';
            html += '<input type="date" class="form-control" name="cheque_date[' + i +
                ']" id="cheque_date" placeholder="" />';
            html += '<label for="cheque_date" class="form-label">Cheque Date *</label>';
            html += '</div>';
            html += '<div class="col-md-6 form-floating mt-4">';
            html += '<input type="text" class="form-control" min="0" name="bank_name[' + i +
                ']" id="BankName" placeholder="" />';
            ...
            html += '<input type="text" class="form-control" name="transaction_no[' + i +
                ']" id="TransactionNo" placeholder="" />';
            html += '<label for="TransactionNo" class="form-label">Transaction No *</label>';
            html += '</div>';
            html += '</div>';
            html += '<div class="col-md-12 mt-3">';
            html += '<a href="javascript:void(0);" class="btn btn-danger delete-payment" data-id="' + i +
                '">Remove</a>';
            html += '</div>';
            html += '</div>';
    ...
    <script>
        ...
        var myDropzone = new Dropzone('#formDropzone', {
            ...
            init: function() {
                ...
                this.on('removedfile', function(data) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "{{ route('admin.remove_policy_attachment', 'empid') }}".replace(
                            'empid', id),
        ...
        $(document).on('change','#HealthCategory',function(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#healthSubCategory').html('');

            $('#healthSubCategory').append(data);
