...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $req)
    {
        ...
        if (Auth::user()->role != 3) {
            $today = Carbon::today();
            $totalPremium = Policy::where('status', 1)->whereDate('risk_start_date', $today)->sum('net_premium_amount');
            $sales_overview['npa'] = $totalPremium;
            $sales_overview['nop'] = Policy::where('status', 1)->whereDate('risk_start_date', $today)->count();
            $sales_overview['motor'] = Policy::where('insurance_type', 1)->where('category', 1)->whereDate('risk_start_date', $today)->where('status', 1)->count();
            $sales_overview['non_motor'] = Policy::where('insurance_type', 1)->where('category', 2)->whereDate('risk_start_date', $today)->where('status', 1)->count();
            $sales_overview['health'] = Policy::where('insurance_type', 2)->where('status', 1)->whereDate('risk_start_date', $today)->count();
            $sales_overview['renewal'] = Policy::where('business_type', '=', 2)->where('status', 1)->whereDate('risk_start_date', $today)->count();
            $sales_overview['new'] = Policy::where('business_type', '!=', 2)->where('status', 1)->whereDate('risk_start_date', $today)->count();
            ...
            $month_overview['non_motor'] = Policy::where('insurance_type', 1)->where('category', 2)->whereBetween('created_at', [$startOfMonth, $today])->where('status', 1)->count();
            $month_overview['health'] = Policy::where('insurance_type', 2)->where('status', 1)->whereBetween('created_at', [$startOfMonth, $today])->count();
            ...
            $year_overview['non_motor'] = Policy::where('insurance_type', 1)->where('category', 2)->where(function ($query) use ($financialYearStart, $financialYearEnd) {
                $query->whereBetween('risk_start_date', [$financialYearStart, $financialYearEnd])
                    ->orWhereBetween('risk_end_date', [$financialYearStart, $financialYearEnd]);
            })->where('status', 1)->count();
            $year_overview['health'] = Policy::where('insurance_type', 2)->where('status', 1)->where(function ($query) use ($financialYearStart, $financialYearEnd) {
                $query->whereBetween('risk_start_date', [$financialYearStart, $financialYearEnd])
                    ->orWhereBetween('risk_end_date', [$financialYearStart, $financialYearEnd]);
            })->count();
            ...
            $finance_overview['non_motor'] = Policy::where('insurance_type', 1)
                ->where('category', 2)
                ->where('status', 1)
                ->where(function ($query) use ($startOfLastFinancialYear, $endOfLastFinancialYear) {
                    $query->whereBetween('risk_start_date', [$startOfLastFinancialYear, $endOfLastFinancialYear])
            ...
            $finance_overview['health'] = Policy::where('insurance_type', 2)
                ->where('status', 1)
                ->where(function ($query) use ($startOfLastFinancialYear, $endOfLastFinancialYear) {
                    $query->whereBetween('risk_start_date', [$startOfLastFinancialYear, $endOfLastFinancialYear])
                        ->orWhereBetween('risk_end_date', [$startOfLastFinancialYear, $endOfLastFinancialYear]);
            ...
            if ($req->has('from_date') && $req->has('to_date')) {
                ...
                $sales_overview['non_motor'] = Policy::where('insurance_type', 1)->where('category', 2)->where('status', 1)->whereBetween('risk_start_date', [$from_date, $to_date])->count();
                $sales_overview['health'] = Policy::where('insurance_type', 2)->where('status', 1)->whereBetween('risk_start_date', [$from_date, $to_date])->count();
            ...
            $claims = Claim::orderBy('id', 'Desc')->with('policy','policy.customers')->get();
            $stacked = array();
            for ($i = 1; $i <= 12; $i++) {
    ...
    {
        $logs = Log::with('userDetail')->latest()->get();
        $page       = 'Logs';
        $icon       = 'logs.png';
        return view('admin.log.log', compact('logs', 'page', 'icon'));
    }
