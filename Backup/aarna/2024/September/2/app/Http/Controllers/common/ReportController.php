...
class ReportController extends Controller
{
    ...
    public function Reports(Request $request)
    {
        ...
        return view('admin.reports.reports', compact('healthCategory','page', 'icon', 'categories', 'sub_categories', 'companies', 'customers', 'plans', 'agents'));
    }
    ...
    public function claimReports(Request $request)
    {
        ...
        if ($request->claim_start_date != '' && $request->claim_end_date != '') {
            ...
            $claim->whereBetween('claim_date', [$request->claim_start_date, $request->claim_end_date]);
        }
    ...
    public function policyReports(Request $request)
    {
        ...
        $query = Policy::with('healthCategoryDetail')->orderBy('id', 'DESC');
        ...
        if ($type == 'csv') {
            if ($request->insurance_type == 1) {
                ...
                return Excel::download(new policyHealthCSVExport($data), 'policy-report.csv');
            }
        } else {
            $records = [];
            if (!blank($data)) {
                ...
                foreach ($data as $source) {
                    ...
                    if ($request->insurance_type == 2) {
                        $category_data = Category::where('id', $source->category)->first();
