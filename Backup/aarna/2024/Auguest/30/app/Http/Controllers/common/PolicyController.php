...
class PolicyController extends Controller
{
    ...
    public function policies(Request $request){
        if($request->has('search')){
            ...
            if($request->has('category') && $request->category != ''){
                ...
                })->orWhereHas('healthCategorDetail',function($query1) use($request){
                    $query1->where('name','LIKE','%'.$request->category.'%');
                });
            }
    ...
    public function editPolicy(Request $request, $id = NULL){
        ...
        // if(Auth::user()->role == 1){
            return view('admin.policy.edit_policy',compact('healthCategory','page','icon','policy','categories','sub_categories','companies','customers','plans','business_source','sourcing_agents','documents','policy_payments','team_lead','managed_by'));
        // }else{
        //     return redirect()->route('login');
        // }
    }
