                ...
                foreach($cate['category'] as $cat_key=>$cat){
                    foreach($cate['payout'] as $payout_key=>$payout){
                        if($cat_key == $payout_key){
                            ...
                            // print_r($hidden_id);
                            // exit;
                            // Added payout_on handling
                            if (isset($cate['payout_on'][$cat_key])) {
                                $agent_payout->payout_on = $cate['payout_on'][$cat_key];
                            } else {
                                $agent_payout->payout_on = null; // Or some default value
                            }
                            ...
                            // Assign the 'payout_on' value
                            // foreach ($cate['payout_on'] as $payout_on_key => $payout_on) {
                            //     if ($payout_on_key == $cat_key) {
                            //         $agent_payout->payout_on = $payout_on;
                            //     }
                            // }
...
public function createPayout(Request $request){
    ...
    public function GeneratePayout(Request $request){
        ...
        $html .= '<th>Vehicle Registration No</th>';
        $html .= '<th>Customer</th>';
        $html .= '<th>Payout Restricted</th>';
        ...
        if(!blank($policies)){
            ...
            foreach($policies as $policy){
                ...
                $html .= '<tr>';
                $html .= '<td>#</td>';
                $html .= '<td><a href="'.route('admin.view_policy',$policy->id).'">'.$policy->policy_no.'</a><input type="hidden" name="payout['.$i.'][policy_id]" value="'.$policy->id.'"><input type="hidden" name="payout['.$i.'][policy_no]" value="'.$policy->policy_no.'"></td>';
                $html .= '<td>'.$subCategory->name.'</td>';
                $html .= '<td>'.$policy->vehicle_registration_no.'</td>';
                $html .= '<td>'.$policy->customers->name.'<input type="hidden" name="payout['.$i.'][customer]" value="'.$policy->customers->id.'"></td>';
                $html .= '<td class="text-capitalize">'.$restrict.'</td>';
                ...
                $html .= '<td>'.$percentage.'% On<br />'.$perString.'<input type="hidden" name="payout['.$i.'][percentage]" value="'.$percentage.'"></td>';
            ...
            $html .= '<tr>';
            $html .= '<td colspan="11" class="text-center">Records Not Found.</td>';
            $html .= '</tr>';
        }
        $html .= '</tbody>';
        $html .= '</table>';
