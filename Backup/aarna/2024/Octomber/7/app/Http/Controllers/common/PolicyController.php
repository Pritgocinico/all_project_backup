...
class PolicyController extends Controller
{
    ...
    public function policies(Request $request){
        if($request->has('search')){
            ...
            if($request->has('category') && $request->category != ''){
                $query = $query->whereHas('categoryDetail', function($query1) use ($request){
                    $query1->where('name','LIKE','%'.$request->category.'%');
                })->orWhereHas('healthCategoryDetail',function($query1) use($request){
                    $query1->where('name','LIKE','%'.$request->category.'%');
                });
            }
            if($request->has('sub_category') && $request->sub_category != ''){
                $query = $query->whereHas('category', function($query1) use ($request){
                    $query1->where('name','LIKE','%'.$request->sub_category.'%');
                })->orWhereHas('healthSubCategoryDetail',function($query1) use($request){
                    $query1->where('name','LIKE','%'.$request->category.'%');
                });
            }
            ...
            if(Auth::user()->role == 1 || Auth::user()->role == 2){
                ...
                // $query = Policy::where('status','!=',2)->where('business_type',1);
    ...
    public function addPolicy(Request $request){
        ...
        $plans = Plan::where('status',1)->where('company',$companies[0]['id'])->get();
        // if(Auth::user()->role == 1){
            return view('admin.policy.add_policy',compact('healthCategory','page','icon','categories','sub_categories','companies','customers','plans','business_source','sourcing_agents','team_lead','managed_by'));
        // }else{
        //     return redirect()->route('login');
        // }
    }
    public function addPolicyData(Request $request){  
       if ($request->has('insurance_type') && $request->insurance_type == 1) {
        ...
        if($request->has('health_sub_category')){
            $policy->health_sub_category        = $request->health_sub_category;
        }
    ...
    public function viewPolicy(Request $request, $id = NULL){
        ...
        $business_source = BusinessSource::orderBy('id','Desc')->get();
        $sourcing_agents = SourcingAgent::all();
        $policy = Policy::with('healthCategoryDetail')->where('id',$id)->first();
        $plans = Plan::where('status',1)->where('company',$policy->company)->where('id',$policy->health_plan)->first();
    ...
    public function editPolicy(Request $request, $id = NULL){
        ...
        $health_sub_categories = Category::where('parent',$policy->health_category)->get();
        if(!blank($policy)){
            $documents = PolicyDocument::where('policy_id',$policy->id)->get();
            $parameters = PolicyParameter::where('policy_id',$policy->id)->get();
        }else{
        ...
        // if(Auth::user()->role == 1){
            return view('admin.policy.edit_policy',compact('health_sub_categories','healthCategory','page','icon','policy','categories','sub_categories','companies','customers','plans','business_source','sourcing_agents','documents','policy_payments','team_lead','managed_by'));
        // }else{
        //     return redirect()->route('login');
        // }
    }
    ...
    public function editPolicyData(Request $request){
        ...
        $policy->risk_start_date         = $request->risk_start_date;
        $policy->risk_end_date           = $request->risk_end_date;
        $policy->last_modified_by           = Auth()->user()->id;
        // $policy->payment_type            = $request->payment_type;
        $policy->business_type           = $business_type;
        // $policy->policy_document         = $img;
        $policy->status                  = 1;
    ...
    public function renewPolicy(Request $request, $id = NULL){
        ...
        // if(Auth::user()->role == 1){
            return view('admin.policy.renew_policy',compact('healthCategory','page','icon','policy','categories','sub_categories','companies','customers','plans','business_source','sourcing_agents','team_lead','managed_by'));
        // }else{
        //     return redirect()->route('login');
        // }
    }
    ...
    public function searchPolicyNumber(Request $request){
        $search  = $request->input('query');
        $results = [];
        if(isset($search)){

            $results = Policy::where('policy_no', 'like', '%' . $search . '%')->where('insurance_type',1)
            ->get(['id', 'policy_no']);
        }
        return response()->json($results);
    }

    public function get_policies_details(Request $request){
        $policyData  = Policy::with('customers','company','category','healthCategoryDetail','healthSubCategoryDetail','categoryDetail','agent')->where('id',$request->policies_number)->first();
