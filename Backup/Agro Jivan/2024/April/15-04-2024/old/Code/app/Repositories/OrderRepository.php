...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function getAllData($status, $search, $type, $driverId = "", $date = "", $order_district = "")
    {
        $query = $this->getQuery($status, $search, $driverId, $date, $order_district)->latest();
        if ($type == 'paginate') {
            $query = $query->paginate(15);
        }
        if ($type == "export") {
    ...
    public function getQuery($status, $search, $driverId, $date = "", $order_district)
    {
        return Order::with('numberOrder', 'districtDetail', 'subDistrictDetail', 'userDetail', 'orderItem', 'orderItem.productDetail', 'orderItem.productDetail.productVariantDetail', 'villageDetail', 'stateDetail')
            ...
            ->when($search, function ($query) use ($search) {
                ...
                $query->where(function ($query) use ($categoryID) {
                    $query->WhereHas('getProductDetail.categoryDetail', function ($query) use ($categoryID) {
                        $query->where('name', 'like', '%' . $categoryID . '%');
                    });
                });
            })->when($date, function ($query) use ($date) {
                $date1 = explode('/', $date);
                $query->whereDate('product.created_at', '>=', $date1[0])
                    ->whereDate('product.created_at', '<=', $date1[1]);
            })->latest();
        if ($field == 'paginate') {
    ...
    public function getStaffOrderReport($search, $field)
    {
        $query = User::with('allOrder')->whereHas('allOrder')->when($search, function ($query) use ($search) {
            $query->where('name', 'like', '%' . $search . '%');
        });
