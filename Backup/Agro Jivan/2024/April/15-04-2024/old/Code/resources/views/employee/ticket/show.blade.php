...
@section('section')
    ...
    <input type="hidden" name="confirm_user_role_id" value="{{Auth()->user() !== null? Auth()->user()->role_id :""}}" id="confirm_user_role_id">
    </body>
@endsection
@section('page')
    <script>
        var role_id =$('#confirm_user_role_id').val();
        var commentRoute = "{{route('employee-ticket-comment')}}";
        if(role_id == 4){
            commentRoute = "{{route('confirm-ticket-comment')}}";
        }
        if(role_id == 5){
            commentRoute = "{{route('driver-ticket-comment')}}";
        }
        if(role_id == 7){
            commentRoute = "{{route('transport-ticket-comment')}}";
        }
        if(role_id == 8){
            commentRoute = "{{route('warehouse-ticket-comment')}}";
        }
        if(role_id == 9){
            commentRoute = "{{route('sales-ticket-comment')}}";
        }
        if(role_id == 10){
            commentRoute = "{{route('sale-service-ticket-comment')}}";
        }
