...
@section('page')
    <script>
        ...
        if(role_id == 5){
            storeURL = "{{ route('driver-leave.store') }}";
            ajax = "{{ route('driver-leave-ajax') }}";
            edit = "{{ route('driver-leave.edit', 'id') }}";
            update = "{{ route('driver-leave.update', 'id') }}";
            deleteURL = "{{ route('driver-leave.destroy', 'id') }}";
            exportFile = "{{route('driver-leave-export')}}" 
        }
        if(role_id == 7){
