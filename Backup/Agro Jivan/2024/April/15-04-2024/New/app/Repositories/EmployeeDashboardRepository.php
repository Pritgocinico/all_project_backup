...
class EmployeeDashboardRepository implements EmployeeDashboardRepositoryInterface
{
    ...
    public function getSalesData($year){
        return Order::select(DB::raw('MONTH(created_at) as month'), DB::raw('YEAR(created_at) as year'))
            ->whereYear('created_at', $year)->groupBy('year', 'month')->where('created_by',Auth()->user()->id)
            ->get();
    }
}
