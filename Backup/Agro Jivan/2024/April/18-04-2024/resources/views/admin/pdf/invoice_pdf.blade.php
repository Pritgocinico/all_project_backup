                ...
                <tr>
                    ...
                    <td style="padding: 5px 24px; width: 50%; text-align: left; margin-bottom: auto;">
                        ...
                        <p style="margin:3px 0px; font-size: 20px; ">Invoice Number: <span
                                style="font-weight: 600px"><b>{{ $orders->order_id }}/{{ Utility::getCurrentFinancialYear() }}</b></span>
                        </p>
            ...
            <tbody>
                <tr>
                    <td style="padding: 5px 24px; width: 50%; text-align: left;    ">
                        ...
                        <p style="margin:3px 0px; font-size: 20px;">{{ $orders->customer_name }}</p>
                        <p style="margin:3px 0px; font-size: 20px;">{{ $orders->address }} ,
                            {{ $orders->villageDetail->village_name }},
                            {{ $orders->subDistrictDetail->sub_district_name }},
                            {{ $orders->districtDetail->district_name }}, {{ $orders->stateDetail->name }} .
                        </p>
                    </td>
                    <td style="padding: 5px 24px; width: 50%; text-align: left; margin-bottom: auto;">
                        ...
                        <p style="margin:3px 0px; font-size: 20px;">{{ $orders->customer_name }}</p>
                        <p style="margin:3px 0px; font-size: 20px;">{{ $orders->address }} ,
                            {{ $orders->villageDetail->village_name }},{{ $orders->subDistrictDetail->sub_district_name }},
                            {{ $orders->districtDetail->district_name }}, {{ $orders->stateDetail->name }} .
                        </p>
                    </td>
                </tr>
        ...
        <div style="margin-bottom: 0px;">
            ...
            <div
                ...
                <p style="padding: 10px 10px ; gap: 20px; margin: 0; font-size: 20px;">Invoice Total: <span
                        ...
                        @php
                            $totalInvoicePrice = $totalTaxPrice + $totalCGPrice + $totalSGPrice;
                        @endphp
                    ...
                    <span style=" font-weight: bold; ">
                        @php
                            $amountInWords = ucwords(
                                (new NumberFormatter('en_IN', NumberFormatter::SPELLOUT))->format($totalInvoicePrice),
                            );
                        @endphp
                        {{ $amountInWords }}
                    </span>
