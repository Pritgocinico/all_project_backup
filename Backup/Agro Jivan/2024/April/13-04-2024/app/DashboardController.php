...
class DashboardController extends Controller
{
    ...
    public function  employeeDashboard()
    {
        ...
        $attendance['order_date'] = Carbon::now()->startOfWeek()->format('Y-m-d') . "/" . Carbon::now()->endOfWeek()->format('Y-m-d');
    ...
    public function leaveAjax(Request $request)
    {
        ...
        if ($totalYearLeave !== $totalPastLeave) {
            $diffBetween = $totalYearLeave - $totalPastLeave;
        }
    ...
    public function winnerAjax(Request $request)
    {
        ...
        foreach ($users as $user) {
            $data = [];
            $order = DB::table('orders')->where('order_status', '6')
    ...
    {
        $year = $request->year;
        $salesData = $this->employeeDashboardRepository->getSalesData($year);
        foreach ($salesData as $sale) {
            $sale['pending'] = $this->employeeDashboardRepository->getPendingCount($sale->month, $sale->year, '1');
            $sale['confirm'] = $this->employeeDashboardRepository->getPendingCount($sale->month, $sale->year, '2');
            $sale['return'] = $this->employeeDashboardRepository->getPendingCount($sale->month, $sale->year, '5');
            $sale['month'] = UtilityHelper::monthName($sale->month);
        }
