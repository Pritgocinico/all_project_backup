        ...
        <div class="d-flex flex-column flex-column-fluid">
            ...
            <div id="kt_app_content" class="app-content  flex-column-fluid">
                <div id="kt_app_content_container" class="app-container  container-fluid">
                    <form class="form" action="{{ route('employee-orders.update', $orderData->id) }}" method="post" 
                                    ...
                                    <option value="">Select Village</option>
                                        @foreach($villages as $village)
                                            <option value="{{$village->village_code}}"
                                                @if ($village->village_code == $orderData->village) {{ 'selected' }} @endif>
                                                        ...
                                                        @if (count($category->childCategoryDetails) > 0)
                                                            <optgroup label="{{ $category->name }}">
                                                                @foreach ($category->childCategoryDetails as $child)
                                                                    <option
                                                                        value="{{ $child->id }}"@if ($child->id == $item->category_id) {{ 'selected' }} @endif>
                                                                        {{ $child->name }}</option>
                                                                @endforeach
                                                            </optgroup>
                                                        @else
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-4">
                                                ...
                                                <select name="scheme_code[]"
                                                    class="form-control scheme_code scheme_code-{{ $i }}"
                                                    data-id ="{{ $i }}" id="">
                                                    <option value="">Select Scheme Code...</option>
                                                    @foreach ($allScheme as $scheme)
                                                        <option value="{{ $scheme->discount_code }}"
                                                            @if ($scheme->discount_code == $item->discount_code) selected @endif>
