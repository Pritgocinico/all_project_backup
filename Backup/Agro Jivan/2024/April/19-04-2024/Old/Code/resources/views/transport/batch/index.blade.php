    ...
    function viewBacthDetail(id) {
        $.ajax({
            ...
            success: function (res) {
                ...
                const ordersArray = Object.values(res).filter(obj => typeof obj === 'object');
                ordersArray.forEach(order => {
                    const { product_id, product_name, quantity, total_order } = order;

                    if (!ordersByProductId[product_id]) {
                        ordersByProductId[product_id] = [];
                    }

                    ordersByProductId[product_id].push({ name: product_name, quantity, order: total_order });
                });
                const structuredData = Object.values(ordersByProductId).map(orders => {
                    const totalQuantity = orders.reduce((acc, curr) => acc + curr.quantity, 0);
                    const totalOrder = orders.reduce((acc, curr) => acc + curr.order, 0);
                ...
                $.each(structuredData, function (i, v) {
                    html += `<tr >
                                <td><b>` + v.name + `:</b></td>
                                <td>` + v.order + `</td>
                                <td>` + v.quantity + `</td>
