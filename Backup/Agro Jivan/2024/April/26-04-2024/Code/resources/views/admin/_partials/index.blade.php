<div class="row">
    <div class="col-md-6">
        <ul class="nav nav-pills mb-3 mx-auto" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-daily-tab" data-bs-toggle="pill" data-bs-target="#pills-daily"
                    type="button" role="tab" aria-controls="pills-daily" aria-selected="true">Daily</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home"
                    type="button" role="tab" aria-controls="pills-home" aria-selected="true">Weekly</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-profile-tab" data-bs-toggle="pill" data-bs-target="#pills-profile"
                    type="button" role="tab" aria-controls="pills-profile" aria-selected="false">Monthly</button>
            </li>
        </ul>
        ...
        <div class="tab-content custom-main-div-dynamic" id="pills-tabContent">
                ...
                <div class="row custom-dynamic-row justify-content-center mt-4">
                    ...
                    <div class="col-xxl-11 col-xl-6 col-lg-8 col-md-6">
                        ...
                        @foreach ($coms as $items)
                            <?php $i++; ?>
                            @if ($i <= 3)
                                <div class="card card-bg1 d-flex justify-content-between winner-box mt-2">
                                    <div class="parent-winner-box">
                                        <div>
                                            <img src="{{ url('/') }}/public/assets/images/certificate/{{ $i }}-m.png"
                                                class="winner-img" alt="">
                                        </div>
                                        <div>
                                            <h5 class="houmanity-color">Winner {{ $i }}</h5>
                                            <h6>Name : {{ $items['winner'] }}</h6>
                                            <h6>Orders : {{ $items['order'] }}</h6>
                                            <a href="{{ route('admin-generate-certificate', ['winnerNumber' => $i, 'empName' => $items['winner']]) }}"
                                                class="btn btn-sm btn-success" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Generate Certificate">Generate
                                                Certificate</a>
                                        </div>
                                    </div>
                                </div>
            ...
            @if(Auth()->user() !== null && Auth()->user()->role_id == 1 || Auth()->user()->role_id == 4)
            <div class="text-center">
                <a class="btn btn-primary" onclick="viewAll('weekly')" href="#">See All</a>
            </div>
            @endif
            </div>
        ...
        <div class="mt-3">
            @if(Auth()->user() !== null && Auth()->user()->role_id == 1 || Auth()->user()->role_id == 4)
            <div class="text-center">
                <a class="btn btn-primary" onclick="viewAll('monthly')" href="#">See All</a>
            </div>
            @endif
    </div>
    <div class="col-md-6">
                ...
                <div id="kt_app_content_container" class="app-container  container-fluid">
                    <form class="form" action="{{ route('generate-certificate') }}">
                        <div class="row mt-2">
                            ...
                            <input type="text" name="certificate_title" id="certificate_title" value="{{old('certificate_title')}}"
                                placeholder="Certificate Title" maxlength="25" class="form-control">
                        ...
                        <div class="row mt-3">
                            ...
                            <textarea id="certificate_description" name="certificate_description" class="form-control" cols="10"
                                rows="10" placeholder="Certificate Description" maxlength="150">{{old('certificate_description')}}</textarea>
