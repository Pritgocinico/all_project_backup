...
class OrderController extends Controller
{
    ...
    public function ajaxList(Request $request)
    {
        ...
        $orderList = $this->orderRepository->getAllData($status, $search, 'paginate',"",$date,$order_district);
    ...
    {
        $search = $request->search;
        $orderList = $this->orderRepository->getAllData('', $search, 'export');
    ...
    public function confirmOrderQueryAjax(Request $request)
    {
        ...
        $query = Order::when($status == "1",function($query){
    ...
    public function generateOrderInvoice(Request $request)
    {
        ...
        foreach ($orderList as $key => $orders) {
             $footer = '<p style="margin-top: 50px; margin-bottom: 0;"><hr></p><p><strong>Sign</strong></p>';
