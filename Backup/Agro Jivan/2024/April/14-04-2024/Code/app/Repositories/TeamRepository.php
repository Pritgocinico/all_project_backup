...
class TeamRepository implements TeamRepositoryInterface
{
    ...
    {
        return Team::with('managerDetail','deliveredOrder')->withCount('pendingOrder', 'confirmOrder', 'deliveredOrder', 'cancelOrder', 'returnOrder', 'allOrder')->where('id', $id)->first();
    }
    ...
    public function getTopFiveUserTeam($date)
    {
        ...
        $teams = $teams->sortByDesc(function ($team) {
            return $team->teamDetails->sum('total_pending_orders_count');
        });
        return $teams;
    }
}
