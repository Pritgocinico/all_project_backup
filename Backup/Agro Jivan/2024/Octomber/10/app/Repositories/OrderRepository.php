...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function getVillages()
    {
        ...
        return SubLocation::select('village_code', 'village_name', 'sub_district','sub_district_name')
    ->get()
    ->keyBy(function ($item) {
        return $item->village_name . '|' . $item->sub_district;
    });
    }
    ...
    public function getFeedbackOrders($search,$order_district,$order_sub_district)
    {
        return Order::with('districtDetail','subDistrictDetail' ,'userDetail', 'feedbackDetail')->where('order_status', '6')->whereDoesntHave('feedbackDetail')
            ->when($search, function ($query) use ($search) {
                $query->where('order_id', 'like', '%' . $search . '%')
                    ->orWhereHas('districtDetail', function ($query) use ($search) {
                        $query->where('district_name', 'like', '%' . $search . '%');
