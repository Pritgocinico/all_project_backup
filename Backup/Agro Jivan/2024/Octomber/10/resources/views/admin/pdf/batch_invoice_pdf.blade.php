            ...
            <tbody>
                <tr>
                    ...
                    <th style="padding: 5px 5px; width: 70%; border: 0;">
                        <h2 style="width: 100%; text-align: center; font-size: 30px; margin:0;">
                            <img alt="Logo" src="{{ asset('public/assets/media/svg/AgroJivanLogoDash.png') }}"
                    class="theme-light-show" style="width: 300px;" />
                </div>
                        </h2>
                        <h4 style="width: 100%; text-align: center; font-size: 25px; margin:0;">Tax Invoice</h4>
                    </th>
                    ...
                    <th style="padding: 5px 5px; width: 30%; border: 0; ">
                        @if (isset($orders->orderDetail))
                            <img src="data:image/png;base64,{{ DNS1D::getBarcodePNG($orders->orderDetail->order_id, 'C39') }}"
                                height="30" width="180" />
                            <p style="font-size: 20px; margin:3px 0px;">{{ $orders->orderDetail->order_id }}</p>
                        @endif
                        <p style="margin:3px 0px; font-size: 18px; "><span style="">{{ date('d-m-Y') }}</span></p>
                    </th>
                </tr>
            </tbody>
        </table>
            ...
            <tbody>
                <tr>
                    <td style="padding: 5px 24px; width: 50%; text-align: left;    ">
                        <p style="margin:3px 0px;  font-weight: bold; font-size: 20px; ">Details of consignor:</p>
                        @php $status = 0 @endphp
                        @if (isset($orders->orderDetail))
                        ...
                        @if ($status == 1)
                            ...
                            <p style="margin:3px 0px; font-size: 20px;"> Address: 616- Shivalik Satyamev, Near Vakil
                                Saheb
                                Brigde, S. P. Ring Road, Bopal, Ahmedabad- 380 058 (GUJ.)</p>
                            <p style="margin:3px 0px; font-size: 20px; ">GSTIN: <span
                                    style="">24AUPPD7929L1Z3</span>
                            @else
                            <p style="margin:3px 0px; font-size: 20px;">AUNITY INDIA PRIVATE LIMITED</p>
                            <p style="margin:3px 0px; font-size: 20px;"> Address: 402, City Gold Center, Near Ramol Toll
                                Plaza, Vastral Ahmedabad- 380058, Gujarat.</p>
                            <p style="margin:3px 0px; font-size: 20px; ">GSTIN: <span
                                    style="">24AAZCA6344F1ZE</span>
                        @endif
                        <p style="margin:3px 0px; font-size: 20px; ">Agent Name: <span style="">
                                {{ isset($orders->orderDetail) ? (isset($orders->orderDetail->userDetail) ? $orders->orderDetail->userDetail->name : '-') : '-' }}
                            </span>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px; ">Mobile Number: <span style="">
                                7096400011 / 7096700011
                            </span>
                        </p>
                    </td>
                    <td style="padding: 5px 24px; width: 50%; text-align: left; margin-bottom: auto;">
                        <p style="margin:3px 0px;  font-weight: bold; font-size: 20px; ">Details of consignee:</p>
                        <p style="margin:3px 0px; font-size: 20px;">Name:-
                            <b>{{ isset($orders->orderDetail) ? $orders->orderDetail->customer_name : '' }}</b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">Address:-
                            <b>{{ isset($orders->orderDetail) ? $orders->orderDetail->address : '' }} </b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">Village:-
                            <b>{{ isset($orders->orderDetail->villageDetail) ? $orders->orderDetail->villageDetail->village_name : '' }}
                            </b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">Sub District:-
                            <b>{{ isset($orders->orderDetail->subDistrictDetail) ? $orders->orderDetail->subDistrictDetail->sub_district_name : '' }}
                            </b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">District:-
                            <b>{{ isset($orders->orderDetail->districtDetail) ? $orders->orderDetail->districtDetail->district_name : '' }}
                            </b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">State:-
                            <b>{{ isset($orders->orderDetail->stateDetail) ? $orders->orderDetail->stateDetail->name : '' }},
                                {{ isset($orders->orderDetail) ? $orders->orderDetail->pincode : '' }}
                            </b>
                        </p>
                        <p style="margin:3px 0px; font-size: 20px;">Mobile Number:-
                            <b>{{ isset($orders->orderDetail) ? $orders->orderDetail->phoneno : '' }}</b>
                        </p>
                    </td>
                </tr>
            </tbody>
        ...
        <div style="margin-bottom: 0px;">
                        ...
                        @foreach ($orders->orderDetail->orderItem as $key => $item)
                            <tr>
                                ...
                                <td style="width: 50px; padding:5px 5px;text-align: center">
                                    {{ $item->productDetail->product_name }}</td>
                                <td style="width: 50px; padding:5px 5px;text-align: center">
                                    {{ $item->varientDetail->sku_name }}</td>
                                ...
                                <td style="width: 50px; padding:5px 5px;text-align: center">
                                    @php
                                        ...
                                        $CgstPer = 0;
                                        $SgstPer = 0;
                                        if ($item->varientDetail->sku_name !== 'AJ- 711 RESEARCH CASTOR') {
                                            if ($item->productDetail->product_type == 'tax_product') {
                                                $CgstPer = 9;
                                                if ($item->productDetail->c_gst !== null) {
                                                    $CgstPer = $item->productDetail->c_gst;
                                                }
                                                $SgstPer = 9;
                                                if ($item->productDetail->s_gst !== null) {
                                                    $SgstPer = $item->productDetail->s_gst;
                                                }
                                            } else {
                                                $CgstPer = 9;
                                                if ($item->productDetail->s_gst !== null) {
                                ...
                                <td style="width: 50px; padding:5px 5px;text-align: center">
                                    @php
                                        if ($item->productDetail->product_type == 'tax_product') {
                                            $toatalPrice = $item->varientDetail->price_without_tax * $item->quantity;
                                        } else {
                                            $toatalPrice = $item->varientDetail->price_without_tax * $item->quantity;
                                        }
                                    @endphp
                                    {{ number_format($toatalPrice, 2) }}
                                </td>
                                <td style="width: 50px; padding:5px 5px;text-align: center">
                                    @php
                                        $totalTaxPrice += $toatalPrice;

                                        $CGPrice = ($toatalPrice * $CgstPer) / 100;
                                        $totalCGPrice += $CGPrice;
                                    @endphp
                                    {{ number_format($CGPrice, 2) }} /
                                    @php
                                        $SGPrice = ($toatalPrice * $SgstPer) / 100;
            ...
            <div style=" text-align: right; border-bottom: 1px solid; border-top: 0; margin: 0; gap: 15px;">
                <p style="padding: 10px 10px ; gap: 20px; margin: 0; font-size: 20px;">Invoice Total: <span
                        ...
                        @php $totalBillAmount = $totalTaxPrice + ($totalSGPrice + $totalCGPrice); @endphp
                    ...
                    <span style=" font-weight: bold; ">
                        @php
                            $amountInWords = ucwords(
                                (new NumberFormatter('en_IN', NumberFormatter::SPELLOUT))->format($totalBillAmount),
                            );
                        @endphp
                        {{ $amountInWords }}
                    </span>
