...
class SalarySlipController extends Controller
{
    ...
    public function store(CreateSalarySlip $request)
    {
        ...
        foreach ($employee as $key => $emp) {
            ...
            $data['totalPresentDay'] = $totalPresentDay = $this->attendanceRepository->getPresentCount($monthNumber, $emp->id, '1');
            $data['totalAbsentDay'] = $this->attendanceRepository->getPresentCount($monthNumber, $emp->id, '0');
            $data['totalHalfDay'] = $this->attendanceRepository->getPresentCount($monthNumber, $emp->id, '2');
            $totalUseLeaveYear = $this->leaveRepository->getTotalYearLeave($emp->id);
            $leaveMonth = $this->leaveRepository->getTotalMonthLeave($monthNumber, $emp->id);
            $data['totalLeave'] = 0;
            ...
            $thisMonthLeave = $this->leaveRepository->getMonthLeave($monthNumber,$emp->id);
            $totalMonthLeave = 0;
            foreach ($thisMonthLeave as $leave) {
                $data['totalMonthLeave'] = $totalMonthLeave += UtilityHelper::getDiffBetweenDateLeave($leave->leave_from, $leave->leave_to);
            }
            $data = [
                'emp_id' => $emp->id,
    ...
    public function calculateSalarySlip(Request $request)
    {
        ...
        $leaveMonth = $this->leaveRepository->getTotalMonthLeave($number, $id);
        $data['totalLeave'] = 0;
        $data['totalDay'] = $totalDay = 22;
        foreach ($leaveMonth as $leave) {
            $data['totalLeave'] += UtilityHelper::getDiffBetweenDates($leave->leave_from, $leave->leave_to);
        }
        $data['totalPresentDay'] = $totalPresentDay = $this->attendanceRepository->getPresentCount($number, $id, '1');
        $data['totalAbsentDay'] = $this->attendanceRepository->getPresentCount($number, $id, '0');
        $data['totalHalfDay'] = $this->attendanceRepository->getPresentCount($number, $id, '2');
        $perDaySalary = $salary / $totalDay;
