        ...
        success: function (res) {
            ...
            $.each(res.data, function (i, value) {
                ...
                if (value.order_status == 6) {
                    statusClass = 'success';
                    text = 'Delivered';
                    iconShow = "d-inline-block delivered_order_detail_card";
                }
                ...
                html += '<span class=""><strong>Status: </strong><span class="badge badge-' + statusClass + ' fw-bold">' + text + '</span> &nbsp; <i class="fas fa-check-double text-success '+iconShow+'"></i><br>';
...
$(document).on('change', '.cat', function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var productAmount = $('.pr-total-' + id).val();
            if (data.price !== undefined) {
                productAmount = data.price;
            }
            var gTotal = $('.grand_total').val();
            var ttl = parseInt(gTotal) - productAmount;
            $('.g-total').html(ttl);
            $('.grand_total').val(ttl);
            $('.pr-total-' + id).val('0')
            $('#item_amt-' + id).val(0);
...
$(document).on('change', '.variant', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var ttl = parseInt(total) + productAmount - amt;
            $('.g-total').html(ttl);
            $('.grand_total').val(ttl);
...
$(document).on('change', '.qty', function () {
    ...
    $('.product_total-' + id).val(amount);
    $('.pr-total-' + id).val(amount);
    $('.g-total').html(ttl);
    $('.grand_total').val(ttl);
});
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (data) {
                    var price = $('.pr-total-' + itemId).val();
                    var gtl = $('.grand_total').val();
                    $('.g-total').html(parseInt(gtl) - price);
                    $('.grand_total').val(parseInt(gtl) - parseInt(price));
                    $('.product-' + itemId).remove();
                    $('.var-' + itemId).remove();
                }
            });
...
function exportCSV() {
    ...
    var order_district = $('#order_district').val();
    var date = $('#search_date').val();
    window.open(exportUrl + '?format=' + format + '&userId=' + user_id + '&search=' + search + "&status=" + status + "&order_sub_district=" + order_sub_district + '&order_district=' + order_district + "&date=" + date, '_blank');
}
