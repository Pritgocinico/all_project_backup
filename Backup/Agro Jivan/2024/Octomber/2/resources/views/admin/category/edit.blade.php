        ...
        <div class="d-flex flex-column flex-column-fluid">
            ...
            <div id="kt_app_content" class="app-content  flex-column-fluid">
                <div id="kt_app_content_container" class="app-container  container-fluid">
                    <form class="form" action="{{ route('category.update',$category->id) }}" method="post" enctype="multipart/form-data" onsubmit="return categoryValidate()">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control mb-3 mb-lg-0"
                                    placeholder="Enter Name" name="name" value="{{ $category->name }}" id="name">
                                ...
                                <select name="parent_category_id" id="parent_category_id" class="form-select">
                                    ...
                                    @foreach ($parentCategory as $parent)
                                        <option value="{{ $parent->id }}"
                                            @if ($category->parent_category_id == $parent->id) selected @endif>{{ $parent->name }}</option>
                                    @endforeach
                                </select>
                                <span class="text-danger"
