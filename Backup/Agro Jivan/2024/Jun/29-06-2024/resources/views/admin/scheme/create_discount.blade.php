        ...
        <div class="d-flex flex-column flex-column-fluid">
            <div id="kt_app_content" class="app-content  flex-column-fluid">
                <div id="kt_app_content_container" class="app-container  container-fluid">
                    <form class="form" action="{{ route('scheme.store') }}" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <label class="required fs-6 fw-semibold mb-2">Discount Code</label>
                                <input type="text" name="scheme_code" id="scheme_code" class="form-control" value="{{Str::random(5)}}"
                            </div>
                        </div>
                        <div class="row">
                                        ...
                                        @if (count($product->productVariantDetail) > 0)
                                            <optgroup label="{{ $product->product_name }}">
                                                @foreach ($product->productVariantDetail as $child)
                                                    <option
                                                        value="{{ $child->id }}"@if ($child->id == old('product0')) {{ 'selected' }} @endif>
                                                        {{ $child->sku_name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @else
                                            <option value="{{ $product->id }}"
                                                @if ($product->id == old('product0')) {{ 'selected' }} @endif>
