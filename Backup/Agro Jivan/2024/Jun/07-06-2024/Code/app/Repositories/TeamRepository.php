...
class TeamRepository implements TeamRepositoryInterface
{
    public function getAllData($search, $type)
    {
        $query =  Team::with('managerDetail', 'teamMember', 'teamMember.userDetail', 'teamMember.userDetail.confirmOrder')->when($search, function ($query) use ($search) {
            $query->where('team_id', 'like', '%' . $search . '%')
                ...
                })->orWhereHas('teamMember.userDetail',function($query)use($search){
                    $query->where('name','like','%'.$search.'%');
                });
        })->latest();
        if ($type == "paginate") {
    ...
    {
        return TeamDetail::with('userDetail')->with('deliveredOrder',function($query)use($date){
    ...
    public function getTopFiveUserTeam($date)
    {
        ...
        foreach ($teams as $team) {
            $teamTotalOrdersCount = 0;
            $managerCount = Order::when($date,function($query)use($date){
                $date1 = explode('/', $date);
                $query->whereDate('created_at', [$date1[0], $date1[1]]);
            })->where('created_by',$team->manager_id)->count();
            if($managerCount !== 0){
                $teamTotalOrdersCount = $managerCount;
            }
            foreach ($team->teamDetails as $teamDetail) {
                ...
                $teamDetail['total_orders_count'] = 0;
                if (isset($user)) {
                    ...
                    $teamDetail['total_confirm_orders_count'] = $totalOrdersCount = $ordersWithinRange->count();
                    $teamDetail['total_orders_count'] = $totalOrdersCount;
                    $teamTotalOrdersCount = $teamTotalOrdersCount+$totalOrdersCount;
                }
            }
            $team->total_orders_count = $teamTotalOrdersCount;
    ...
    public function getManagerOrderCount($id,$date)
    {
        return Team::with('managerDetail')->with('deliveredOrder',function($query)use($date){
            $query->when($date,function($query)use($date){
                $date1 = explode('/',$date);
                    $query->whereDate('delivery_date',">=",$date1[0])->whereDate('delivery_date',"<=",$date1[1]);
            });
