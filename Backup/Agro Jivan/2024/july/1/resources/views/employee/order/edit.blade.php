    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_content" class="app-content  flex-column-fluid">
            <div id="kt_app_content_container" class="app-container  container-fluid">
                <form class="form" action="{{ route('employee-orders.update', $orderData->id) }}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <label class="required fs-6 fw-semibold mb-2">Phone Number</label>
                            <input type="number" class="form-control mb-3 mb-lg-0" placeholder="Enter Phone Number" name="phoneno" value="{{ $orderData->phoneno }}" id="phoneno">
                            <span class="text-danger" id="name_error">{{ $errors->getBag('default')->first('phoneno') }}</span>
                        </div>
                        <div class="col-md-6">
                            <label class="fs-6 fw-semibold mb-2">Customer Name</label>
                            <input type="text" class="form-control" placeholder="Enter Customer Name" name="customer_name" value="{{ $orderData->customer_name }}" id="customer_name">
                            <span class="text-danger" id="email_error">{{ $errors->getBag('default')->first('customer_name') }}</span>
                        </div>
                    </div>
                    <div class="row mt-2">
                        <div class="col-md-12">
                            <label class="required fs-6 fw-semibold mb-2">Address</label>
                            <input type="text" class="form-control" placeholder="Enter Address" name="address" value="{{ $orderData->address }}" id="address">
                            <span class="text-danger" id="phone_number_error">{{ $errors->getBag('default')->first('address') }}</span>
                        </div>
                    </div>
                    <div class="row mt-2">
                        <div class="col-md-6 employeePassword">
                            ...
                            <select name="state_id" id="state_id" class="form-select">
                                ...
                                <option value="{{ $states->id }}" @if ($states->id == 12) {{ 'selected' }} @endif disabled>
                                    {{ $states->name }}
                                </option>
                                @endforeach
                            </select>
                            <span class="text-danger" id="password_error">{{ $errors->getBag('default')->first('state') }}</span>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="district" id="district" class="form-select" onchange="getSubDistrict()">
                                ...
                                <option value="{{ $districts->district }}" @if ($districts->district == $orderData->district) {{ 'selected' }} @endif>
                                    {{ $districts->district_name }}
                                </option>
                                @endforeach
                            </select>
                            <span class="text-danger" id="aadhar_card_error">{{ $errors->getBag('default')->first('district') }}</span>
                        </div>
                    ...
                    <div class="row mt-2">
                        <div class="col-md-6">
                            ...
                            <select name="sub_district" id="sub_district" class="form-select" onchange="getVillage()">
                                ...
                                <option value="{{ $sub->sub_district }}" @if ($sub->sub_district == $orderData->sub_district) {{ 'selected' }} @endif>
                                    {{ $sub->sub_district_name }}
                                </option>
                                @endforeach
                            </select>
                            <span class="text-danger" id="pan_card_error">{{ $errors->getBag('default')->first('sub_district') }}</span>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="village" id="village" class="form-select">
                                ...
                                <option value="{{$village->village_code}}" @if ($village->village_code == $orderData->village) {{ 'selected' }} @endif>
                                    {{$village->village_name}}
                                </option>
                                @endforeach
                            </select>
                            <span class="text-danger" id="qualification_error">{{ $errors->getBag('default')->first('village') }}</span>
                        </div>
                    </div>
                    <div class="row mt-2">
                        <div class="col-md-6">
                            <label class="required fs-6 fw-semibold mb-2">Pincode</label>
                            <input type="number" class="form-control" name="pincode" placeholder="Enter Pincode" value="{{ $orderData->pincode }}" id="pincode">
                            <span class="text-danger" id="system_code_error">{{ $errors->getBag('default')->first('pincode') }}</span>
                        </div>
                        <div class="col-md-6">
                            <label class="required fs-6 fw-semibold mb-2">Expected Delivery Date</label>
                            <input type="date" class="form-control" name="excepted_delievery_date" placeholder="Enter Pincode" value="{{ $orderData->excepted_delievery_date }}" id="excepted_delievery_date">
                            <span class="text-danger" id="system_code_error">{{ $errors->getBag('default')->first('excepted_delievery_date') }}</span>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-4">
                            <label>Scheme Code</label>
                            <select name="scheme_code[]" class="form-control scheme_code scheme_code" id="scheme_code">
                        ...
                        <div class="col-md-6 d-none" id="scheme_product_div">
                            <label>Product</label>
                            <select name="scheme_product_id[]" class="form-control scheme_product_id " id="scheme_product_id">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="category[]" id="" class="form-control cat cat-{{ $i }}" data-id="{{ $i }}" required>
                                        ...
                                        <optgroup label="{{ $category->name }}">
                                            ...
                                            <option value="{{ $child->id }}" @if ($child->id == $item->category_id) {{ 'selected' }} @endif>
                                                {{ $child->name }}
                                            </option>
                                            @endforeach
                                        </optgroup>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-4">
                                    ...
                                    <select name="variant[]" class="form-control variants-{{ $i }} variant" data-id="{{ $i }}" id="" required>
                                        ...
                                        <option value="{{ $variant->id }}" @if ($variant->id == $item->variant_id) selected @endif>
                                    ...
                                    <p class="text-danger">
                                        <?php $pr = DB::table('product_variant')
                                            ...
                                            ->first(); ?>
                                    <p class="text-danger fs-12">( Stock : <span class="stock-{{ $i }} fs-12">
                                            @if (!blank($pr))
                                            {{ $pr->stock }}
                                            @else
                                            0
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="hidden" name="pr_price[]" class="form-control pr-price-{{ $i }}" value="{{ $item->price }}">
                                    <input type="hidden" id="item_amt-{{ $i }}" value="{{ $item->price }}">
                                    <input type="text" name="price[]" class="form-control pe-none price-{{ $i }}" value="{{ $item->price }}">
                                </div>
                                <div class="col-md-2">
                                    <label for="">Quantity</label>
                                    <input type="number" name="quantity[]" min="0" value="{{ $item->quantity }}" class="form-control qty qty-{{ $i }}" data-id="{{ $i }}">
                                </div>
                                <div class="col-md-2">
                                ...
                                <div class="col-md-2 m-auto text-end">
                                    <input type="hidden" name="product_total[]" class="pr-total-{{ $i }}" value="{{ $item->quantity * $item->price }}">
                                    <h6 class="mb-0">Total : <span class="text-danger"> &#x20B9; <span class="total total-{{ $i }}">{{ $item->quantity * $item->price }}</span></span>
                                    </h6>
                                </div>
                            </div>
                    ...
                    <div class="row justify-content-end mt-3">
                        <div class="col-md-4">
                            <h5 class="text-end">Total : <span class="text-danger fs-24"> &#x20B9; <span class="g-total fs-24">
                                        {{ $orderData->amount }}
                                    </span></span></h5>
                        </div>
                    </div>
                    ...
                    <div class="row mt-2">
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="orderlead" id="flexSwitchCheckChecked" @if ($orderData->order_lead_status == 1) {{ 'checked' }} @endif>
                        ...
                        <div class="col-md-6 lead_datetime @if ($orderData->order_lead_status != 1) d-none @endif">
                            ...
                            <input type="datetime-local" name="lead_datetime" id="lead_datetime" class="form-control" value="">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <select name="order_status" class="form-control" id="order_status">
                                <option value="1" @if ($orderData->order_status == 1) selected @endif>Pending
                                    Order</option>
                                <option value="4" @if ($orderData->order_status == 4) selected @endif>Canceled
                                </option>
                            </select>
...
@section('page')
<script>
    var ordersList = "{{ route('employee-orders-list') }}";
    ...
    var schemeCodeDetail = "{{route('employee-scheme-detail-code')}}"
    var schemeCodeProduct = "{{route('employee-scheme-detail-product')}}"
</script>
<script>
