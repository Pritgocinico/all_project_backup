...
@section('section')
    ...
    <input type="hidden" id="bathc_role_id" value="{{Auth()->user() !== null? Auth()->user()->role_id:''}}">
    </body>
@endsection
@section('page')
    <script>
        var role = $('#bathc_role_id').val();
        var updateStatus = "{{route('admin-order-status-update')}}"
        if(role == 7){
            updateStatus = "{{route('transport-order-status-update')}}"
        }
        if(role == 8){
            updateStatus = "{{route('warehouse-order-status-update')}}"
        }
        $(function() {
            $('.search_batch_date').daterangepicker({
                ...
                $('.search_batch_date').val(start.format('Y-MM-DD') + '/' + end.format('Y-MM-DD'));
            });
        });
        ...
        function exportInvoice() {
            ...
            window.open("{{ route('batch-order-invoice-pdf') }}" + '?search=' + search + '&date=' + batch_date + '&id=' +
                "{{ $id }}" + "&order_status=" + order_status, '_blank');
        }
        ...
        function exportBatchOrder() {
            ...
            window.open("{{ route('export-batch-order') }}" + '?format=' + format + '&search=' + search + '&date=' +
                batch_date + '&id=' + "{{ $id }}" + "&order_status=" + order_status, '_blank');
        }
        ...
        function cancelOrderStatus(id) {
            new swal({
                title: 'Are you sure change status Return this Order?',
