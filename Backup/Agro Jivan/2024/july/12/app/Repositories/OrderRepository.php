...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function getSalesOrderReport($search, $date, $field, $order_status, $user_id, $order_district, $order_sub_district)
    {
        ...
        $query = ProductVariant::with([
            ...
            ->withSum(['orderItems as total_quantity' => function ($query) use ($dateRange, $order_status, $order_district, $order_sub_district) {
                $query->whereHas('orderDetail', function ($query) use ($order_status, $dateRange, $order_district, $order_sub_district) {
                    $query->where('order_status', $order_status);
                    if ($dateRange) {
                        $this->filterOrders($query, $order_status, $dateRange, $order_district, $order_sub_district);
                    }
            ...
            ->withSum(['orderItems as total_amount' => function ($query) use ($dateRange, $order_status, $order_district, $order_sub_district) {
                $query->whereHas('orderDetail', function ($query) use ($order_status, $dateRange, $order_district, $order_sub_district) {
                    $query->where('order_status', $order_status);
                    if ($dateRange) {
                        $this->filterOrders($query, $order_status, $dateRange, $order_district, $order_sub_district);
            ...
            ->addSelect(['total_order_count' => function ($query) use ($order_district, $order_sub_district, $order_status) {
                $query->select(DB::raw('count(distinct order_id)'))
                    ->from('order_items')
                    ->whereColumn('variant_id', 'product_variant.id')
                    ->whereExists(function ($subQuery) use ($order_district, $order_sub_district, $order_status) {
