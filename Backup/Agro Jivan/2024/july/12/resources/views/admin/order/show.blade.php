            ...
            <div id="kt_app_toolbar" class="app-toolbar  pt-6 pb-2 ">
                <div id="kt_app_toolbar_container" class="container-fluid d-flex align-items-stretch ">
                    <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                        ...
                        <div class="page-title d-flex justify-content-center gap-1 me-3">
                            <div
                                class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bold fs-3 m-0 ms-3">
                                @if ((Auth()->user() !== null && Auth()->user()->role_id == '1') || Auth()->user()->role_id == '4')
                                    @if ($order->order_status == 1)
                                        <button class="btn btn-warning text-dark btn-sm dropdown-toggle" type="button"
                                    ...
                                    <ul class="dropdown-menu task-status">
                                        <li><a class="dropdown-item status @if ($order->order_status == 1) hide_detail @endif"
                                                href="#" data-status="1" data-task="{{ $order->id }}">Pending
                                                Order</a></li>
                                        <li><a class="dropdown-item status @if ($order->order_status == 2) hide_detail @endif"
                                                href="#" data-status="2" data-task="{{ $order->id }}">Confirmed</a>
                                        </li>
                                        <li><a class="dropdown-item status @if ($order->order_status == 3) hide_detail @endif"
                                                href="#" data-status="3" data-task="{{ $order->id }}">On
                                                Delivery</a></li>
                                        <li><a class="dropdown-item status @if ($order->order_status == 4) hide_detail @endif"
                                                href="#" data-status="4" data-task="{{ $order->id }}">Cancel
                                                Order</a></li>
                                        <li><a class="dropdown-item status @if ($order->order_status == 5) hide_detail @endif"
                                                href="#" data-status="5" data-task="{{ $order->id }}">Returned</a>
                                        </li>
                                        <li><a class="dropdown-item status @if ($order->order_status == 6) hide_detail @endif"
                                                href="#" data-status="6" data-task="{{ $order->id }}">Delivered</a>
                                        </li>
                                    </ul>
                                @else
                                    @if ($order->order_status == 1)
                                        <button class="btn btn-warning text-dark btn-sm " type="button"
                                            aria-expanded="false">
                                            Pending Order
                                        </button>
                                    @elseif($order->order_status == 2)
                                ...
                                <div class="card-body pt-15">
                                    ...
                                    <div id="kt_customer_view_details" class="collapse show">
                                        <div class="py-5 fs-6">
                                            ...
                                            @else
                                            <div class="fw-bold mt-5">Delivered Date</div>
                                            <div class="text-gray-600">
                                                {{ Utility::convertMDY($order->delivery_date) }}
                                            </div>
                                            @endif
                                            ...
                                            <div class="text-gray-600">
                                                {{ isset($order->userDetail) ? $order->userDetail->name : '-' }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <table class="table align-middle table-row-dashed gy-5"
                                                ...
                                                <tbody class="fs-6 fw-semibold text-gray-600">
                                                    @forelse ($order->orderItem as $item)
                                                        <tr>
                                                            <td>{{ isset($item->productDetail) ? $item->productDetail->product_name : '' }}
                                                            </td>
                                                            <td>{{ isset($item->categoryDetail) ? $item->categoryDetail->name : '' }}
                                                            </td>
                                                            <td>{{ isset($item->varientDetail) ? $item->varientDetail->sku_name : '' }}
                                                            </td>
