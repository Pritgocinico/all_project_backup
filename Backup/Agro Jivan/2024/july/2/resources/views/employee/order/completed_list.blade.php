<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
    ...
    <tbody class="text-gray-600 fw-semibold">
        @forelse ($completed_order as $key=>$order)
            <tr>
                <td>{{ $completed_order->firstItem() + $key }}</td>
                ...
                <td>
                    ...
                    @if($order->order_status == 3)
                        @php $status = 'info';$text = "On Deliverrf" @endphp
                    @endif
                    @if($order->order_status == 4)
                        @php $status = 'danger';$text = "Cancelrf" @endphp
                    @endif
...
<div class="d-flex justify-content-end">
    {{ $completed_order->links('pagination::bootstrap-5') }}
</div>
