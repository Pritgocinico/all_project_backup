<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
    <thead>
        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
            ...
            <th class="min-w-100px">Confirm Date</th>
        </tr>
    </thead>
    <tbody class="text-gray-600 fw-semibold">
        @forelse ($orderList as $key=>$order)
            <tr>
                <td>
                    @if(Auth()->user() !== null && Auth()->user()->role_id == 7)
                    <a href="{{route('transport-confirm-view',$order->id)}}" class="pre-agro-emp">
                        {{ $order->order_id }}</a>
                    @else
                    {{ $order->order_id }}
                    @endif
                ...
                <td>{{ Utility::convertDmyWith12HourFormat($order->confirm_date) }}</td>
            </tr>
        @empty
            <tr>
