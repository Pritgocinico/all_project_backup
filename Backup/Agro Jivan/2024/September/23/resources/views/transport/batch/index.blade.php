                                            ...
                                            <div class="px-7 py-5" data-kt-user-table-filter="form">
                                                ...
                                                <div class="mb-10">
                                                    ...
                                                    <select class="form-control" name="batch_status" id="batch_status">
                                                        ...
                                                        <option value="1" @if($status==1) {{'selected'}} @endif>On Delivery</option>
                                                        <option value="2" @if($status==2) {{'selected'}} @endif>Delivered</option>
                                                    </select>
                                                </div>
...
<div class="modal fade" id="edit_batch_model" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <label>Driver</label>
                    <select id="edit_driver_id" name="edit_driver_id" class="form-select js-example-basic-multiple">
                        <option value="">Select Driver</option>
                        @foreach ($driverList as $driver)
                        <option value="{{ $driver->id }}">{{ $driver->name }}
                        </option>
                    ...
                    <span id="driver_id_error" class="text-danger"></span>
                </div>
                <div class="row">
                    <label>Select Order Id</label>
                    <select name="order_id[]" id="order_id" class="form-select js-example-basic-multiple" multiple>
...
<script>
    ...
    function exportCSV() {
        ...
        window.open(exportFile + '?format=' + format + '&diver_id=' + diver_id + '&search=' + search + '&batch_date=' +
            batch_date + '&status=' + status, '_blank');
    }
    ...
    function updateBatchStatus(id,status) {
        var text = "Are you sure change status Delivered this Batch?"
        var buttonText = "Yes Delivered it!"
        if(status == 1){
            text = "Are you sure change status On Delivery this Batch?"
            buttonText = "Yes On Delivery it!"
        }
        new swal({
            title: 'Are you sure change status Delivered this Batch?',
