    ...
    <tbody class="text-gray-600 fw-semibold">
        @forelse ($batchList as $key=>$batch)
            <tr>
                ...
                <td>
                    <a class="btn btn-icon btn-primary w-30px h-30px me-3" href="#"
                        onclick="viewBacthDetail({{ $batch->id }})" data-bs-toggle="tooltip" data-bs-placement="top"
                        aria-label="Edit Employee" data-bs-original-title="View Batch Item"><i
                            class="fas fa-eye"></i></a>
                    @if ($batch->status == 1)
                        <a class="btn btn-icon btn-info w-30px h-30px me-3" href="#"
                            onclick="updateBatchStatus({{ $batch->id }},'2')" data-bs-toggle="tooltip"
                    ...
                    @else
                        <a class="btn btn-icon w-30px h-30px me-3 on_delivery_batch_icon" href="#"
                            onclick="updateBatchStatus({{ $batch->id }},'1')" data-bs-toggle="tooltip"
                            data-bs-placement="top" aria-label="Edit Employee"
                            data-bs-original-title="Mark As On Delivery" aria-describedby="tooltip553274">
                            <i class="fas fa-shipping-fast"></i>
                        </a>
                    @endif
                    <a class="btn btn-icon btn-success w-30px h-30px me-3" href="#"
                        onclick="editBatch({{ $batch->id }},{{ $batch->driver_id }})" data-bs-toggle="tooltip"
                        data-bs-placement="top" aria-label="Edit Batch" data-bs-original-title="Edit Batch"
                        aria-describedby="tooltip553274">
                        <i class="fas fa-edit"></i>
                    </a>
                    @if (Auth()->user() !== null && Auth()->user()->role_id !== 8)
                        <a class="btn btn-icon btn-info w-30px h-30px me-3" href="#"
                            onclick="deleteBatch({{ $batch->id }})" data-bs-toggle="tooltip" data-bs-placement="top"
                            aria-label="Delete Batch" data-bs-original-title="Delete Batch"
                            aria-describedby="tooltip553274">
