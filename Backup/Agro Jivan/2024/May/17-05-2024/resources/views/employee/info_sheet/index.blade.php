...
@section('section')
    ...
    <input type="hidden" id="role_id" value="{{Auth()->user() !== null?Auth()->user()->role_id:''}}">
    </body>
@endsection
@section('page')
    <script>
        ...
        if(role == 10){
            ajaxList = "{{ route('service-info-ajax') }}";
            exportFile = "{{route('service-info-export')}}";
            deleteUrl = "{{route('service-info-sheet.destroy','id')}}";
        }
