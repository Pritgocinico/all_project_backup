    ...
    <table class="table align-middle table-row-dashed fs-6 gy-5">
        <thead>
            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0 table">
                ...
                <th class="min-w-100px table">Created AT</th>
            </tr>
        </thead>
        <tbody class="text-gray-600 ">
            @forelse ($orderList as $key=>$order)
                <tr>
                    ...
                    <td class="table">
                        @php $variant = "";$product = ""; @endphp
                        @foreach ($order->orderItem as $item)
                            @if($item->productDetail)
                                @php $product = $item->productDetail->sku_name @endphp
                            @endif
                            @if($item->varientDetail)
                                @php $variant = $item->varientDetail->sku_name @endphp
                            @endif

                            {{$product ."[".$variant ."-". $item->quantity."]"}}<br />
                        @endforeach
                    </td>
