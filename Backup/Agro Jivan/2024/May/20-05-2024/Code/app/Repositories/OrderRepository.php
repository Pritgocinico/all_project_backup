...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    {
        $query = ProductVariant::with(['productDetail', 'orderItems.orderDetail'])
            ->whereHas('orderItems', function ($query) use ($order_district, $order_sub_district, $date, $order_status, $user_id) {
                $query->whereHas('orderDetail', function ($query) use ($order_district, $order_sub_district, $date, $order_status, $user_id) {
                    $query->where('order_status', $order_status)
                        ...
                        ->when($date, function ($query) use ($date, $order_status) {
                            $dateRange = explode('/', $date);
                            $query->when(in_array($order_status, ['1', '3']), function ($query) use ($dateRange) {
                                $query->whereDate('created_at', '>=', $dateRange[0])
                                    ->whereDate('created_at', '<=', $dateRange[1]);
                            })->when($order_status == '2', function ($query) use ($dateRange) {
                                $query->whereDate('confirm_date', '>=', $dateRange[0])
                                    ->whereDate('confirm_date', '<=', $dateRange[1]);
                            })->when($order_status == '4', function ($query) use ($dateRange) {
                                $query->whereDate('cancel_date', '>=', $dateRange[0])
                                    ->whereDate('cancel_date', '<=', $dateRange[1]);
                            })->when($order_status == '5', function ($query) use ($dateRange) {
                                $query->whereDate('return_date', '>=', $dateRange[0])
                                    ->whereDate('return_date', '<=', $dateRange[1]);
                            })->when($order_status == '6', function ($query) use ($dateRange) {
                                $query->whereDate('delivery_date', '>=', $dateRange[0])
                                    ->whereDate('delivery_date', '<=', $dateRange[1]);
                            });
                        })
                        // Filtering by district
                        ->when($order_district, function ($query) use ($order_district) {
                            $query->whereHas('districtDetail', function ($query) use ($order_district) {
                                $query->where('district', $order_district);
                            });
                        })
                        // Filtering by sub-district
                        ->when($order_sub_district, function ($query) use ($order_sub_district) {
                            $query->whereHas('subDistrictDetail', function ($query) use ($order_sub_district) {
                                $query->where('sub_district', $order_sub_district);
                            });
                        });
                });
            })
            ->withSum(['orderItems as total_quantity' => function ($query) use ($date, $order_status) {
                $dateRange = explode('/', $date);
            ...
            ->withSum(['orderItems as total_amount' => function ($query) use ($date, $order_status) {
                $dateRange = explode('/', $date);
