...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    {
        $query = ProductVariant::with('productDetail', 'orderItems', 'orderItems.orderDetail')
            ->has('productDetail')->has('orderItems.orderDetail')
                    ...
                    $query->where('order_status', $order_status)
                        ...
                        ->when($date, function ($query) use ($date, $order_status) {
                            ...
                            $query->when(in_array($order_status, ['1', '3']), function ($query) use ($dateRange) {
                                ...
                                $query->whereDate('confirm_date', '>=', $dateRange[0])->whereDate('confirm_date', '<=', $dateRange[1]);
                            })->when($order_status == '4', function ($query) use ($dateRange) {
                                $query->whereDate('cancel_date', '>=', $dateRange[0])->whereDate('cancel_date', '<=', $dateRange[1]);
                            })->when($order_status == '5', function ($query) use ($dateRange) {
                                $query->whereDate('return_date', '>=', $dateRange[0])->whereDate('return_date', '<=', $dateRange[1]);
                            })->when($order_status == '6', function ($query) use ($dateRange) {
                                $query->whereDate('delivery_date', '>=', $dateRange[0])->whereDate('delivery_date', '<=', $dateRange[1]);
            ...
            })->withSum(['orderItems' => function ($query) use ($date) {
                $dateRange = explode('/', $date);
