...
class LeaveRepository implements LeaveRepositoryInterface
{
    ...
    {
        return Leave::where('created_by', $id)
        ->where(function ($query) {
            $currentYear = Carbon::now()->year;
            $query->whereYear('leave_from', $currentYear)
                    ->orWhereYear('leave_to', $currentYear);
            })
            ->get();
    }

    public function getMonthLeave($month,$id)
    {
        $year = date('Y');
        return Leave::whereYear('leave_from', $year)->where('created_by',$id)
