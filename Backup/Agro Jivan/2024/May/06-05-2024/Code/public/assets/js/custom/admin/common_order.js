            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var textNumber = document.querySelector('.pagination .page-item.active .page-link');
                            if (textNumber !== null) {
                                orderAjaxList(parseInt(textNumber.innerText));
                            } else {
                                orderAjaxList(1);
                            }
                },
...
function exportCSV() {
    ...
    var order_sub_district = $('#order_sub_district').val();
    var userId = $('#user_id').val();
    window.open(exportUrl + '?format=' + format + '&search=' + search+'&type=' + type+"&status="+status+"&driverId="+driverId+"&date="+date+"&order_district="+order_district+"&order_district="+order_sub_district+"&userId="+userId, '_blank');
}
