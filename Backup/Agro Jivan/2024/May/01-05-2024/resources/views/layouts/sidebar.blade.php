                ...
                <div id="kt_app_sidebar" class="app-sidebar  flex-column " data-kt-drawer="true"
                    ...
                    <div class="app-sidebar-wrapper">
                        <div id="kt_app_sidebar_wrapper" class="hover-scroll-y my-5 my-lg-2 mx-4"
                                        ...
                                        <div class="menu-sub menu-sub-accordion"><!--begin:Menu item-->
                                            <div class="menu-item"><!--begin:Menu link--><a
                                                    ...
                                                    href="{{ route('admin-top-five-confirm-order') }}"><span
                                                        class="menu-bullet"><span
                                                            class="bullet bullet-dot"></span></span><span
                                                        class="menu-title">Top Confirmed Orders</span></a><!--end:Menu link-->
                                            </div>
                                        </div>
                                    </div>
                                    @php $product = "" @endphp
                                ...
                                <div id="#kt_app_sidebar_menu" data-kt-menu="true" data-kt-menu-expand="false"
                                    ...
                                    <div class="menu-item">
                                        <a class="menu-link @if (\Route::currentRouteName() == 'confirm-product') active @endif"
                                            ...
                                            <span class="menu-icon"><i class="fa-brands fa-product-hunt"></i></span><span
                                                class="menu-title">
                                                Products</span>
                                        </a>
                                    </div>
                            ...
                            @endif
                            @if (Auth()->user() !== null && Auth()->user()->role_id !== 5)
                                @php $log = route('admin-all-logs'); $text = 'admin-all-logs'  @endphp
                                @if (Auth()->user() == null)
                                    @php $log = route('login') @endphp
                                @elseif (Auth()->user()->role_id == 2)
                                    @php $log = route('employee-all-logs'); $text = 'employee-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 3)
                                    @php $log = route('hr-all-logs'); $text = 'hr-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 4)
                                    @php $log = route('confirm-all-logs'); $text = 'confirm-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 6)
                                    @php $log = route('system-all-logs'); $text = 'system-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 7)
                                    @php $log = route('transport-all-logs'); $text = 'transport-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 8)
                                    @php $log = route('warehouse-all-logs'); $text = 'warehouse-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 9)
                                    @php $log = route('sales-all-logs'); $text = 'sales-all-logs' @endphp
                                @elseif (Auth()->user()->role_id == 10)
                                    @php $log = route('sale-service-all-logs'); $text = 'sale-service-all-logs' @endphp
                                @endif
                                <div id="#kt_app_sidebar_menu" data-kt-menu="true" data-kt-menu-expand="false"
                                    class="app-sidebar-menu-primary menu menu-column menu-rounded menu-sub-indention menu-state-bullet-primary px-3 mb-5 log_menu">
                                    <div class="menu-item">
