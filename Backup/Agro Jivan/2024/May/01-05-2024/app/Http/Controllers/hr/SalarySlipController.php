...
class SalarySlipController extends Controller
{
    ...
    public function store(CreateSalarySlip $request)
    {
        ...
        foreach ($employee as $key => $emp) {
            ...
            $data['totalPresentDay'] = $totalPresentDay = $this->attendanceRepository->getPresentCount($month, $emp->id, '1');
            $data['totalAbsentDay'] = $this->attendanceRepository->getPresentCount($month, $emp->id, '0');
            $data['totalHalfDay'] = $this->attendanceRepository->getPresentCount($month, $emp->id, '2');
