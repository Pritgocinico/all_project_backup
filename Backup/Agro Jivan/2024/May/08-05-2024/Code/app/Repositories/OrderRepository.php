...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function getQuery($status, $search, $driverId, $date = "", $order_district, $userId, $order_sub_district = "")
    {
        return Order::with('numberOrder', 'districtDetail', 'subDistrictDetail', 'userDetail', 'orderItem', 'orderItem.productDetail', 'orderItem.varientDetail', 'orderItem.productDetail.productVariantDetail', 'villageDetail', 'stateDetail')
            ->when($status, function ($query) use ($status) {
                ...
                })->when($status == 1 || $status == 3, function ($query) use ($date1) {
    ...
    {
        $productVariants = ProductVariant::with('getProductDetail', 'getProductDetail.categoryDetail')->with(['orderItems' => function ($query) use ($date) {
    ...
    public function getSalesOrderReport($search, $date, $field, $order_status, $user_id, $order_district, $order_sub_district)
    {
        ...
        $query = ProductVariant::with('getProductDetail', 'orderItems', 'orderItems.orderDetail')
            ->whereHas('orderItems', function ($query) use ($order_district, $order_sub_district, $date, $order_status, $user_id) {
                $query->whereHas('orderDetail', function ($query) use ($order_district, $order_sub_district, $date, $order_status, $user_id) {
                    $query->where('order_status', $order_status)
                        ->when($user_id, function ($query) use ($user_id) {
                            $query->where('created_by', $user_id);
                        })->when($order_status, function ($query) use ($order_status, $date) {
                            $query->when($date, function ($query) use ($date, $order_status) {
                                $date1 = explode('/', $date);
                                $start = Carbon::parse($date1[0]);
                                $end = Carbon::parse($date1[1]);
                                $query->when($order_status == '1' || $order_status == '3', function ($query) use ($start, $end) {
                                    $query->whereDate('created_at', '>=', $start->format('Y/d/m'))->whereDate('created_at', '<=', $end->format('Y/d/m'));
                                });
                                $query->when($order_status == '2', function ($query) use ($start, $end) {
                                    $query->whereDate('confirm_date', '>=', $start->format('Y/d/m'))->whereDate('confirm_date', '<=', $end->format('Y/d/m'));
                                });
                                $query->when($order_status == '4', function ($query) use ($start, $end) {
                                    $query->whereDate('cancel_date', '>=', $start->format('Y/d/m'))->whereDate('cancel_date', '<=', $end->format('Y/d/m'));
                                });
                                $query->when($order_status == '5', function ($query) use ($start, $end) {
                                    $query->whereDate('return_date', '>=', $start->format('Y/d/m'))->whereDate('return_date', '<=', $end->format('Y/d/m'));
                                });
                                $query->when($order_status == '6', function ($query) use ($start, $end) {
                                    $query->whereDate('delivery_date', '>=', $start->format('Y/d/m'))->whereDate('delivery_date', '<=', $end->format('Y/d/m'));
                                });
                            });
                        })
                        ->whereHas('districtDetail', function ($query) use ($order_district) {
                            $query->when($order_district, function ($query) use ($order_district) {
                                $query->where('district', $order_district);
                            });
                        })->whereHas('subDistrictDetail', function ($query) use ($order_sub_district) {
                            $query->when($order_sub_district, function ($query) use ($order_sub_district) {
                                $query->where('sub_district', $order_sub_district);
                            });
                        });
                });
