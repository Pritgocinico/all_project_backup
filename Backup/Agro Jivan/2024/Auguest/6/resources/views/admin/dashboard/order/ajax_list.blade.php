<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
    <thead>
        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
            ...
            @php $buttonText = 'Created At';@endphp
            @if($status == 2)
                @php $buttonText = 'Confirm At';@endphp
            @endif
            @if($status == 4)
                @php $buttonText = 'Cancel At';@endphp
            @endif
            @if($status == 5)
                @php $buttonText = 'Delivered At';@endphp
            @endif
            @if($status == 6)
                @php $buttonText = 'Return At';@endphp
            @endif
            <th class="min-w-100px">{{$buttonText}}</th>
        </tr>
    </thead>
    <tbody class="text-gray-600 fw-semibold">
        @forelse ($query as $key=>$order)
            <tr>
            <td>{{ $query->firstItem() + $key }}</td>
                ...
                <td>
                    @php $status = 'warning';$text = "Pending Order"; $date = $order->created_at @endphp
                    @if($order->order_status == 2)
                        @php $status = 'success';$text = "Confirmed"; $date = $order->confirm_date @endphp
                    @endif
                    @if($order->order_status == 3)
                        @php $status = 'warning';$text = "On Delivery"; $date = $order->created_at; @endphp
                    @endif
                    @if($order->order_status == 4)
                        @php $status = 'danger';$text = "Cancelled";$date = $order->cancel_date; @endphp
                    @endif
                    @if($order->order_status == 5)
                        @php $status = 'danger';$text = "Returned";$date = $order->return_date; @endphp
                    @endif
                    @if($order->order_status == 6)
                        @php $status = 'primary';$text = "Delivered";$date = $order->delivery_date; @endphp
                    @endif
                
                    <span class="badge bg-{{$status}}">{{$text}}</span>
                </td>

                <td>{{ Utility::convertDmyWith12HourFormat($date) }}</td>
            </tr>
        @empty
            <tr>
