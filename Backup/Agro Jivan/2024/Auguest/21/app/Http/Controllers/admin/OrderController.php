...
class OrderController extends Controller
{
    ...
    public function edit(string $id)
    {
        ...
        return redirect()->route('orders.index')->with('error', 'Something went to wrong.');
    }
    ...
    public function update(CreateOrderRequest $request, string $id)
    {
        ...
        if ($request->order_status == 6) {
            $data['delivery_date'] = Carbon::now()->format('Y-m-d h:i:s');
        }
        $where['id'] = $id;
        ...
        if ($update) {
            ...
            foreach ($request->category as $key => $category) {
                $where = ['order_id' => $id, 'id' => $request->ids[$key]];
                $code = "";
                ...
                $totalAmount +=  $request->price[$key] * $request->quantity[$key];
            ...
            $dataUpdate['amount'] = $totalAmount;
            $whereUpdate['id'] = $id;
            $this->orderRepository->update($dataUpdate, $whereUpdate);
            if (Auth()->user()->role_id == "4") {
                return redirect('confirm/confirm-orders')->with('success', 'Orders Updated Successfully.');
            }
            if (Auth()->user()->role_id == "9") {
                return redirect('sales/sale-orders')->with('success', 'Orders Updated Successfully.');
            }
            if (Auth()->user()->role_id == "7") {
                return redirect()->route('batch-list')->with('success', 'Orders Updated Successfully.');
            }
    ...
    public function deliveredAjaxList(Request $request)
    {
        ...
        if (Auth()->user() !== null && Auth()->user()->role_id == 7) {
            return view('transport.order.delivered_order_ajax', compact('orderList'));
        }
    ...
    public function deliverOrder()
    {
        ...
        if (Auth()->user() !== null && Auth()->user()->role_id == 7) {
            return view('transport.order.deliver_order', compact('page'));
        }
    ...
    public function generateOrderInvoice(Request $request)
    {
        ...
        foreach ($orderList as $key => $orders) {
            ...
            $viewFile = view('admin.pdf.invoice_pdf', compact('orders', 'setting'))->render();
    ...
    {
        $where['id'] = $request->id;
        $orderItemDetail = OrderItem::find($request->id);
        $delete = $this->orderRepository->deleteOrderItem($where);
        if ($delete) {
            $orderDetail = Order::find($orderItemDetail->order_id);
            $orderDetail->amount = $orderDetail->amount - $orderItemDetail->amount;
            $orderDetail->save();
    ...
    public function getSubDistrictInOrder(Request $request)
    {
        $district = $request->district;
        $orderSubDistricts = $this->employeeOrderRepository->getUseSubDistrictOrder($district);
        return $orderSubDistricts;
    }
}
