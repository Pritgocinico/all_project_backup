    ...
    public function webhook(Request $request)
    {
        ...
        if ($dataSet['event'] == "subscription-created") {
            ...
            if ($planDetailData  == null) {
                ...
                $storePlan = Plan::create($createPlan);
            } else {
            ...
            if ($user) {
                $businessDetail = Business::where('client_id', $user->id)->where('name', $businessName)->first();
                if (isset($businessDetail)) {
                    $businessDetail->sub_end_date = $date;
                    $businessDetail->status = 1;
                    $businessDetail->plan_id = $storePlan->id;
                ...
                if ($userDetail) {
                    $businessDetail = Business::where('client_id', $userDetail->id)->where('name', $businessName)->first();
                    if (isset($businessDetail)) {
                        $businessDetail->sub_end_date = $date;
                        $businessDetail->plan_id = $storePlan->id;
                        $businessDetail->payment_option = "online";
