...
<body>
        ...
        <div class="container-fluid gc_log">
            <div class="form">
                <form action="{{ route('admin_login') }}" method="post" class="mt-4">
                    @csrf
                    <img src="{{ url('/') }}\assets\Images\fwdreviewswhite.png" alt=""
                        class="img-fluid mb-md-4 md-2" width="150px">
                    ...
                    <div class="input">
                        ...
                        <div class="inputBox my-4">
                            <div class="input-group mb-3">
                                ...
                                <input type="password" name="password" value="{{ old('password') }}"
                                    class="form-control" id="password" placeholder="Password" aria-label="Password"
                                    aria-describedby="basic-addon2">
                                <span class="input-group-text px-2" id="basic-addon2eye" onclick="showpw()"><i class="fa fa-eye text-white"></i></span>
                            </div>
                            @if ($errors->has('password'))
                                <p class="text-danger">{{ $errors->first('password') }}</p>
                            @endif
                            <div>
                                <input type="checkbox" id="showPassword" class="me-2"><label
                                    class="d-inline-block" for="showPassword"><small>Remember Me</small></label>
                        ...
                        <div class="text-center mt-2"> <a href="{{ route('forget.password') }}"
                                class="text-white">Forget Password?</a></div>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function showpw() {
            ...
            if (x.type === "password") {
                x.type = "text";
                $('#basic-addon2eye').html('<i class="fa fa-eye-slash text-white"></i>')
            } else {
                x.type = "password";
                $('#basic-addon2eye').html('<i class="fa fa-eye text-white"></i>')
            }
        }
    </script>
    <script>
