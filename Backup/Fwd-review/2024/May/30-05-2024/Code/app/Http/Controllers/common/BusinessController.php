...
class BusinessController extends Controller
{
    ...
    public function addBusinessData(Request $request){
        ...
        $business->sub_end_date      = $request->sub_end_date;
        $business->status       = 1;
        $business->active       = 1;
    ...
    public function businessDetail($id = NULL){
        $page  = "Business Detail Page";
        $icon = 'add-user.png';
        $business = Business::with('client','paymentDetail')->where('id',$id)->first();
        if(!blank($business)){
            $paymentDetail = Payment::where('business_id',$business->id)->first();
            $clientDetail = User::where('id',$business->client_id)->first();
            return view('admin.business.view_business',compact('page','icon','business','clientDetail','paymentDetail'));
        }
        abort(404);
    }
