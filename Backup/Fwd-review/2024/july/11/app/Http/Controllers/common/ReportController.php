...
class ReportController extends Controller
{
    ...
    public function analyticReport(Request $request)
    {
        ...
        while ($startDate->lessThanOrEqualTo($endDate)) {
            ...
            $customerFeedback[] = CustomerFeedback::whereMonth('created_at', $startDate->format('m'))->whereYear('created_at', $startDate->format('Y'))->where('user_id', $business->id)->count();
            $campaign[] = SentEmail::whereMonth('created_at', $startDate->format('m'))->whereYear('created_at', $startDate->format('Y'))->count();
            $clickThrough[] = UserEmailSetting::whereMonth('created_at', $startDate->format('m'))->whereYear('created_at', $startDate->format('Y'))->where('user_id', $business->id)->count();
            $startDate->addMonth();
            $reviewCounts[] = count($reviewsInCurrentMonth);
        }
        return view('client.analytic_report.analytic', compact('tabType', 'customerFeedback', 'clickThrough', 'campaign', 'labels', 'reviewCounts', 'startingDate', 'startDate', 'endDate', 'page', 'icon', 'business', 'feedbacks', 'data', 'filteredReviews', 'recentReviewCount'));
    }
    ...
    public function exportReport(Request $request, $id)
    {
        ...
        $jsonData1 = json_decode($response1->getBody(), true);
        $message = '';
        $filteredReviewData = json_decode($mapReport->details);
        $filteredReviews = count((array) $filteredReviewData);
        $feedbacks = CustomerFeedback::where('user_id', Auth::user()->business)->where('status', 1)->get();
        $recentTimeFrame = strtotime('-4 months');
        $recentReviewCount = 0;
        ...
        while ($startDate->lessThanOrEqualTo($endDate)) {
            $labels[] = $startDate->format('M Y');
            $reviewsInCurrentMonth = 0;
            $reviewsInCurrentMonth = array_filter((array) $filteredReviewData, function ($review) use ($startDate, $endDate) {
                $reviewDate = Carbon::createFromTimestamp($review->time);
                return ($reviewDate >= $startDate && $reviewDate <= $endDate);
            });
            $campaign[] = SentEmail::whereDate('created_at', '>=',$startDate)->whereDate('created_at','<=' ,$endDate)->count();
            $startDate->addMonth();
            $reviewCounts[] = count($reviewsInCurrentMonth);
        }
        if ($type == "pdf") {
            return view('admin.pdf.generated_report', compact('campaign','page', 'icon', 'labels', 'reviewCounts', 'business', 'feedbacks', 'recentReviewCount', 'filteredReviews', 'mapReport', 'filteredReviewData'));
        }
        if ($type == "csv") {
            $headers = array(
            ...
            $callback = function () use ($filteredReviewData, $columns) {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
                foreach ($filteredReviewData as $order) {
    ...
    public function sentTestMail(Request $request)
    {
        ...
        $pdfUrl = asset('uploads/pdf/' . $pdfFileName);
        ...
        $subject = str_replace("[[business_name]]", $business->name, $subject);
        $body = str_replace("[[report_title]]", $reportTitle, $body);
        $body = str_replace("[[report_date]]", $reportDate, $body);
        $body = str_replace("[[report_url]]", $pdfUrl, $body);
        $body = str_replace("[[brand_name]]", $brandName, $body);
        $name = $request->first_name . " " . $request->last_name;
        $body = str_replace("[[profile_name]]", $name, $body);
        $email = $request->email_address;
        Mail::send([], [], function ($message) use ($email, $subject, $pdfUrl, $body, $template, $business) {
            $message->from($template->from_name_mail, $business->name)
                ->to($email)
                ->replyTo($template->reply_to, $business->name)
    ...
    function scheduleReportStop(Request $request)
    {
        ...
        if ($schedule) {
            $schedule->status = '0';
            $schedule->save();
            return response()->json(['data' => '', 'message' => "Schedule stopped successfully.", 'status' => 1], 200);
        }
        return response()->json(['data' => '', 'message' => "Schedule not found.", 'status' => 1], 500);
    }
}
