<table class="table table-hover table-sm table-nowrap table-responsive mt-6 border" id="ticket_table">
    <thead>
        <tr>
            <th>#</th>
            <th>Ticket Id</th>
                ...
                <td class="text-end">
                    @if (Auth()->user()->role_id == 1)
                        @if ($ticket->status == 0)
                            <a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="top"
                                title="Reopen Ticket" onclick="changeTicketStatus({{ $ticket->id }},1)"><i
                        ...
                        <a href="{{ route('ticket.show', $ticket->id) }}" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="View Ticket"><i class="bi bi-eye"></i></a>
                    @elseif (collect($accesses)->where('menu_id', 11)->first()->status == 2)
                        <a href="javascript:void(0)" onclick="editTicket({{ $ticket->id }})" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="Edit Ticket"><i class="bi bi-pencil-square"></i></a>
