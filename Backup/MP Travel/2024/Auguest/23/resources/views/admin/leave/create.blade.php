        ...
        <main class="container-fluid px-3 py-5 p-lg-6 p-xxl-8">
            ...
            <div
                ...
                <main class="container-fluid px-6 pb-10">
                    @php
                        $route = route('leave.store');
                        $method = 'POST';
                        if (isset($leave)) {
                            $route = route('leave.update', $leave->id);
                            $method = 'PUT';
                        }
                    @endphp
                    <form action="{{ $route }}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-4 col-xl-4">
                                <select name="leave_type" class="form-control">
                                    ...
                                    <option value="sick_leave" @if (old('leave_type', $leave->leave_type ?? '') == 'sick_leave') selected @endif>Sick Leave
                                    </option>
                                    <option value="casual_leave" @if (old('leave_type', $leave->leave_type ?? '') == 'casual_leave') selected @endif>Casual
                                        Leave</option>
                                </select>
                                @error('leave_type')
                            ...
                            <div class="col-md-4 col-xl-4">
                                <select name="leave_feature" class="form-control">
                                    ...
                                    <option value="0" @if (old('leave_feature', $leave->leave_feature ?? '') == '0') selected @endif>Half Day
                                    </option>
                                    <option value="1" @if (old('leave_type', $leave->leave_feature ?? '') == '1') selected @endif>Full Day
                                    </option>
                                </select>
                                @error('leave_feature')
                                    <div class="text-danger">{{ $message }}</div>
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-4 col-xl-4">
                                <input type="date" name="leave_from" class="form-control" placeholder="Enter Leave From"
                                    ...
                                    value="{{ old('leave_from' ?? date('Y-m-d'), $leave->leave_from ?? date('Y-m-d'))  }}">
                                @error('leave_from')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 col-xl-4">
                                <input type="date" name="leave_to" class="form-control" placeholder="Enter Leave From"
                                    ...
                                    value="{{ old('leave_to' ?? date('Y-m-d'), $leave->leave_to ?? date('Y-m-d')) }}">
                                @error('leave_to')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-4 col-xl-4">
                                <textarea name="leave_reason" class="form-control" placeholder="Enter Leave Reason">{{ old('leave_reason', $leave->reason ?? '') }}</textarea>
                                @error('leave_reason')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
