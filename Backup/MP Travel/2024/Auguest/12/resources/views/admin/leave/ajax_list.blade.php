    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @php $subject = $leave->reason @endphp
                @if (strlen($leave->reason) > 30)
                @php $subject = substr($leave->reason, 0, 30) . '...'; @endphp
                @endif
            <td data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $leave->reason }}">{{ $subject }}</td>
            </td>
            <td>
                @php
                $status = 'pending';
                ...
                @endphp
                <span class="badge bg-{{ $class }}">{{ $status }}</span><br />
                {{$reason}}
