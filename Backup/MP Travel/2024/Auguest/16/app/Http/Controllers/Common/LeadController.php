...
class LeadController extends Controller
{
    ...
    {
        $leads = $this->lead->latest()->get();
        $type = request('type');
        return view('admin.lead.index', compact('leads', 'type'));
    }
    ...
    public function store(CreateLeadRequest $request)
    {
        ...
        if ($request->invest_type == "investments") {
            $lead->investment_field = $request->investment_field;
            ...
            if ($request->insurance_type == "pms") {
                ...
                $lead->managed_by = $request->managed_by;
                $lead->maturity_amount = $request->maturity_amount;
                $lead->lead_date = $request->lead_date;
            ...
            $lead->policy_no = $request->policy_no;
            ...
            if ($request->insurance_type == "health") {
                ...
                $lead->sme_insurance = $request->sme_insurance;
                if($request->sme_insurance == "fire&burglary"){
                    $lead->insurance_cover = $request->insurance_cover;
                    $lead->insurance_value = $request->insurance_value;
                    $lead->hypothication = $request->hypothication;
                    $lead->nature_of_business = $request->nature_of_business;
                    $lead->fire_claim_history = $request->fire_claim_history;
                }elseif ($request->sme_insurance == "marine") {
                    $lead->nature_of_business = $request->nature_of_business;
                    $lead->good_description = $request->good_description;
                    if ($request->hasFile('invoice_copy')) {
                        $file = $request->file('invoice_copy');
                        $newFilename = Str::random(40) . '.' . $file->getClientOriginalExtension();
                        $invoiceCopyPath = $file->storeAs('lead/marine_invoice', $newFilename, 'public');
                        $lead->invoice_copy = $invoiceCopyPath;
                    }
                }elseif ($request->sme_insurance == "wc") {
                    $lead->workers_number = $request->workers_number;
                    $lead->salary_range = $request->salary_range;
                    $lead->designation = $request->designation;
                }
                $lead->gmc = $request->gmc;
                $lead->gpa = $request->gpa;
                $lead->professional_indemnity = $request->professional_indemnity;
    ...
    public function addCustomerForm(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'aadhaar_number' => 'required_if:customer_department,individual',
            'gst_certificate' => 'required_if:customer_department,corporate',
            'address' => 'required',
            'country' => 'required',
            'state' => 'required',
            'city' => 'required',
