...
class CustomerController extends Controller
{
    ...
    {
        $page = "Customer";
        $this->customer = resolve(Customer::class)->with('moduleModifiedLog','moduleModifiedLog.userDetail','servicePreferenceTagDetail', 'servicePreferenceTagDetail.userDetail', 'servicePreferenceTagDetail.servicePreferenceDetail');
    ...
    public function store(CreateCustomerRequest $request)
    {
        ...
        if ($insert) {
            ...
            return redirect()->route('customer.change-profile', $customer->id)->with('success', 'Customer has been created successfully and update customer Profile.');
        }
    ...
    public function customerProfileUpdate(Request $request)
    {
        ...
        if ($customer) {
            if (!empty($request->preference_id)) {
                foreach ($request->preference_id as $key => $preference) {
                    $servicePreferenceTag =  new ServicePreferenceTag();
                    $servicePreferenceTag->customer_id = $request->customer_id;
                    $servicePreferenceTag->service_preference_id = $preference;
                    $servicePreferenceTag->created_by = Auth()->user()->id;
                    $servicePreferenceTag->save();
                    $changedColumns = $servicePreferenceTag->getDirty();
                    if (!empty($changedColumns)) {
                        foreach ($changedColumns as $column => $newValue) {
                            $originalValue = $servicePreferenceTag->getOriginal($column);
                            if ($originalValue != $newValue) {
                                $modifiedLog = new ModuleModifiedLogs();
                                $modifiedLog->module_name = "Service Preference";
                                $modifiedLog->module_task_id = $preference;
                                $modifiedLog->column_name = $column;
                                $modifiedLog->new_value = $newValue;
                                $modifiedLog->original_value = $originalValue;
                                $modifiedLog->created_by = auth()->user()->id;
