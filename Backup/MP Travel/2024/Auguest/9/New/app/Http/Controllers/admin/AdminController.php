...
class AdminController extends Controller
{
    ...
    public function addClientData(Request $request)
    {
        $request->validate([
            ...
            'phone' => 'required|unique:users,phone,deleted_at,NUL',
            'password' => 'required',
        ]);
    ...
    public function updateClient(Request $req)
    {
        $req->validate([
            ...
            'email' => [
                'required',
                Rule::unique('users')->ignore($req->user_id)->whereNull('deleted_at'),
            ],
            'phone' => [
                'required',
                Rule::unique('users')->ignore($req->user_id)->whereNull('deleted_at'),
        ...
        if (isset($req->password)) {
            $data1['password'] = $req->password;
            MemberHelper::updateMemberPassword($user->member_id,$data1);
    ...
    public function deleteClient($id)
    {
        ...
        MemberHelper::deleteMember($user->member_id);
        $user->delete();
