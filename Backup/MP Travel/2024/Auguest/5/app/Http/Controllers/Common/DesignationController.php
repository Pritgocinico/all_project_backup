...
class DesignationController extends Controller
{
    ...
    public function designationAjaxList(Request $request){
        ...
        ->paginate(10);
        $departmentList = Department::latest()->get();
        return view('admin.designation.ajax_list',compact('designationList','departmentList'));
    }
    ...
    public function exportFile(Request $request)
    {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            $headers = array(
                ...
                "Content-Disposition" => "attachment; filename=department Export.csv",
                "Pragma" => "no-cache",
                "Cache-Control" => "must-revalidate, post-check=0, pre-check=0",
                "Expires" => "0"
            );

            $columns = array('Designation Name', 'Department Name', 'Status', 'Created At');
            $callback = function () use ($designationList, $columns) {
                ...
                foreach ($designationList as $designation) {
                    ...
                    fputcsv($file, array($designation->name, $department, $status, $date));
                }
                fclose($file);
            };
        ...
        if ($request->format == "pdf") {
            ...
            $html = view('admin.pdf.designation', ['designationList' => $designationList, 'mainLogoUrl' => $mainLogoUrl])->render();
