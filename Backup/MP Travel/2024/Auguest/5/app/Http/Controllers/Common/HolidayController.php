...
class HolidayController extends Controller
{
    ...
    public function exportFile(Request $request)
    {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $columns = array('Holiday Name', 'Holiday Date', 'Description', 'Status', 'Created At');
            $callback = function () use ($holidayList, $columns) {
                ...
                foreach ($holidayList as $holiday) {
                    ...
                    $holidaydate = "";
                    if (isset($holiday->created_at)) {
                        $holidaydate = (new \DateTime($holiday->holiday_date))->format('d-m-Y');
                    }
                    $status = "";
                    if ($holiday->status == 0) {
                        $status = 'Inactive';
                    ...
                    fputcsv($file, array($holiday->holiday_name, $holidaydate, $holiday->description, $status, $date));
                }
                fclose($file);
            };
