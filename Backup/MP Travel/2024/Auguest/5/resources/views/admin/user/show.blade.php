    ...
    <div
        ...
        <main class="container-fluid px-3 py-5 p-lg-6 p-xxl-8">
            ...
            <ul class="nav nav-tabs nav-tabs-flush gap-8 overflow-x border-0 mt-4">
                <li class="nav-item">
                    <a href="#" class="nav-link active" id="generate-certificate-tab">User Detail</a>
                </li>
                <li class="nav-item">
                    <a href="#" class="nav-link" id="certificate-list-tab">Attendance Detail</a>
                </li>
            </ul>
            <div id="break_detail" class="d-none">
            ...
            <div id="user_detail" class="d-block">
                ...
                <table class="table table-hover table-striped table-sm table-nowrap table-responsive">
                    ...
                    <tbody>
                        @forelse ($user->deductionDetail as $key=>$deduct)
                            <tr>
                                <td>{{ $key+1; }}</td>
                                <td>{{ $deduct->deduction_type }}</td>
                                <td>&#x20B9; {{ number_format($deduct->amount??0,2) }}</td>
                                <td>
                                    @php
                                        $text = 'Active';
                                        $color = 'success';
...
@section('script')
    <script>
        ...
        $('#generate-certificate-tab').on('click', function(event) {
            event.preventDefault();
            $('#user_detail').addClass('d-block');
            $('#user_detail').removeClass('d-none');
            $('#break_detail').removeClass('d-block');
            $('#break_detail').addClass('d-none');
            $('#generate-certificate-tab').addClass('active');
            $('#certificate-list-tab').removeClass('active');
        });

        $('#certificate-list-tab').on('click', function(event) {
            event.preventDefault();
            $('#user_detail').addClass('d-none');
            $('#user_detail').removeClass('d-block');
            $('#break_detail').removeClass('d-none');
            $('#break_detail').addClass('d-block');
            $('#generate-certificate-tab').removeClass('active');
            $('#certificate-list-tab').addClass('active');
        });
        ...
        var calendar = $('#attendance_calendar').fullCalendar({
            ...
            eventClick: function(event, element, view) {
                if (event.title == "Casual Leave" || event.title == "Sick Leave") {
                    $('#leave_reason').html(event.description)
