@if (collect($accesses)->where('menu_id', '4')->first()->status != 0 ||
        ...
        collect($accesses)->where('menu_id', '21')->first()->status != 0 ||
        collect($accesses)->where('menu_id', '6')->first()->status != 0)
    <li class="nav-item my-1">
        <a class="nav-link d-flex align-items-center {{ request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*')|| request()->routeIs('lead-checklist.*') || request()->routeIs('role.*') || request()->routeIs('user.*') ? 'active collapsed' : '' }}"
            href="#sidebar-user" data-bs-toggle="collapse" role="button"
            aria-expanded="{{ request()->routeIs('role.*') || request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*') || request()->routeIs('lead-checklist.*') || request()->routeIs('user.*') ? 'true' : 'false' }}"
        ...
        <div class="collapse {{ request()->routeIs('role.*') || request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*') || request()->routeIs('lead-checklist.*') || request()->routeIs('user.*') ? 'show' : '' }}"
            id="sidebar-user">
            <ul class="nav nav-sm flex-column mt-1  inner-ul">
                ...
                @if (collect($accesses)->where('menu_id', '20')->first() &&
                        collect($accesses)->where('menu_id', '20')->first()->status != 0 && (Auth()->user()->role_id == 1 ||Auth()->user()->department_id == 1))
                    <li class="nav-item"><a href="{{ route('document-checklist.index') }}"
                            class="nav-link {{ request()->routeIs('document-checklist.*') ? 'fw-bold' : '' }}"><i class="fa-solid fa-file-export"></i>
                            Document CheckList</a>
                    </li>
                @endif
                @if (collect($accesses)->where('menu_id', '21')->first() &&
                        collect($accesses)->where('menu_id', '21')->first()->status != 0 )
                    <li class="nav-item"><a href="{{ route('lead-checklist.index') }}"
                            class="nav-link {{ request()->routeIs('lead-checklist.*') ? 'fw-bold' : '' }}"><i class="fa-solid fa-envelope-circle-check"></i>
                            Lead CheckList</a>
                    </li>
                @endif
            </ul>
