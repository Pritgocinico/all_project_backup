...
class EmployeeDashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $leadStatusData = Lead::where('created_by', Auth()->user()->id)->orWhereHas('leadMemberDetail', function ($query) {
            ...
            ->toArray();
        $data['labels'] = ['Pending', 'Assigned', 'Hold', 'Completed', 'Extends', 'Cancel'];
        $statusArray = [];
        ...
        if(Auth()->user()->is_manager == "yes"){
            $managerMember = ManagerMemberDetail::where('manager_id',Auth()->user()->id)->pluck('user_id')->toArray();
            array_push($managerMember,Auth()->user()->id);
            $data['followUpList'] = FollowUpEvent::whereIn('created_by', $managerMember)
            ->orWhereHas('followUpMemberDetail', function ($query) use($managerMember) {
                $query->whereIn('user_id', $managerMember);
            })->latest()->get();
        } else {
            $data['followUpList'] = FollowUpEvent::where('created_by', Auth()->user()->id)
            ->orWhereHas('followUpMemberDetail', function ($query) {
                $query->where('user_id', Auth()->user()->id);
            })->latest()->get();
        }
