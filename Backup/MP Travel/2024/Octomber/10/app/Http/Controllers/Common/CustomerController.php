    ...
    public function store(CreateCustomerRequest $request)
    {
        ...
        if ($request->hasFile('gst_certificate')) {
            ...
            foreach ($request->file('gst_certificate') as $key => $gst) {
                ...
                array_push($gstCertificateArray, $gstCertificatePath);
            }
            $customer->gst_certificate = json_encode($gstCertificateArray);
        }
        if ($request->hasFile('aadhar_card_file')) {
            $aadharCardArray = array();
            foreach ($request->file('aadhar_card_file') as $key => $aadhar) {
                $newFilename = Str::random(40) . '.' . $aadhar->getClientOriginalExtension();
                $aadharPath = $aadhar->storeAs('customer/aadhar_card', $newFilename, 'public');
                array_push($aadharCardArray, $aadharPath);
            }
            $customer->aadhar_card_file = json_encode($aadharCardArray);
        }
        if ($request->hasFile('pan_card_file')) {
            ...
            foreach ($request->file('pan_card_file') as $key => $pan) {
                ...
                $panPath = $pan->storeAs('customer/pan_card', $newFilename, 'public');
                array_push($panCardArray, $panPath);
            }
            $customer->pan_card_file = json_encode($panCardArray);
        }
        if ($request->hasFile('passport_file')) {
            $passportArray = array();
            foreach ($request->file('passport_file') as $key => $passport) {
                $newFilename = Str::random(40) . '.' . $passport->getClientOriginalExtension();
                $passportPath = $passport->storeAs('customer/passport', $newFilename, 'public');
                array_push($passportArray, $passportPath);
            }
            $customer->pan_card_file = json_encode($passportArray);
        }
