    ...
    public function certificateAjaxList(Request $request)
    {
        ...
        if (Auth()->user()->role_id == 2) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $certificateList = $certificateList->whereIn('emp_id', $managerMember);
            } else {
                $certificateList = $certificateList->where('emp_id', Auth()->user()->id);
            }
        }
        $certificateList = $certificateList->when($search, function ($query, $search) {
