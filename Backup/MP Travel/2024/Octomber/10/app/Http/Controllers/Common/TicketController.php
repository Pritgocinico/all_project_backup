...

{
...

    {
        ...
        return view('admin.ticket.index', compact('departmentList','userList'));
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $callback = function () use ($ticketList, $columns) {
                ...
                foreach ($ticketList as $ticket) {
                    ...
                    $employee = isset($ticket->userDetail) ?ucfirst($ticket->userDetail->name) : "-";
                    $department = isset($ticket->departmentDetail) ?ucfirst($ticket->departmentDetail->name) : "-";
                    fputcsv($file, array($ticket->ticket_id, $employee, $department, $ticket->ticket_type, $ticket->subject, $ticket->description, $status, $date));
        ...
        if ($update) {
            ...
            foreach ($userList as $key => $user) {
                ...
                $offerData = [
                    ...
                    'title' => "Ticket Status changes is " . $statusName,
            ...
            Log::create([
                ...
                'description' => auth()->user()->name . " changed a ticket status with ".$ticket->ticket_id,

            ]);

            return response()->json(['data' => $ticket, 'message' => "Ticket Status Changed Successfully.", 'status' => 1], 200);
