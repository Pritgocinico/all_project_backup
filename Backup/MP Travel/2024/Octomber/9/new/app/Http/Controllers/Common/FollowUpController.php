...
class FollowUpController extends Controller
{
    ...
    public function followupAjaxList(Request $request)
    {
        ...
        if(Auth()->user()->role_id !== "1"){
            if(Auth()->user()->is_manager == "yes"){
                ...
                ->orWhereHas('followUpDetail.followUpMemberDetail', function ($query) use($managerMember) {
                    $query->whereIn('user_id', $managerMember);
                });
            } else {
                ...
                ->orWhereHas('followUpDetail.followUpMemberDetail', function ($query) {
                    $query->where('user_id', Auth()->user()->id);
                });
            }
        }
        $leads = $leads->with('customerDetail', 'leadMemberDetail', 'leadMemberDetail.userDetail', 'followUpDetail')->latest()->get();
        $employees      = User::where('role_id', 2)->get();
    ...
    public function create()
    {
        ...
        $leads          = Lead::with('leadMemberDetail', 'leadMemberDetail.userDetail')->latest()->get();
    ...
    public function update_due_date(Request $req)
    {
        ...
        $data = [
            ...
            'comment' => 'Change Due Date OF ' . $item->event_name . ' To ' . $req->due_date,
            'message_type' => 'text',
            'user_id' => Auth()->user()->id,
        ];
        FollowUpComment::create($data);
        Log::create([
            'user_id' => auth()->user()->id,
            'module' => 'Follow Up',
    ...
    public function update_checklist_note(Request $request)
    {
        ...
        $item->save();
        $followUpData = FollowUpEvent::where('id', $item->follow_up_id)->first();
        $data = [
            'type' => 'lead',
            'follow_id' => $item->follow_up_id,
            'lead_id' => $followUpData->lead_id,
            'comment' => 'Update Sub Task Note OF ' . $item->event_name . ' To ' . $request->val,
            'message_type' => 'text',
            'user_id' => Auth()->user()->id,
        ];
        FollowUpComment::create($data);
        if($request->type == 'ajax'){
            return response()->json(['message' => "Update Note Successfully."], 200);
        }
        return redirect()->back();
    }
    function deleteChecklistItem($id)
    {
        $item = FollowUpChecklistItem::where('id', $id)->first();
        $followUpData = FollowUpEvent::where('id', $item->follow_up_id)->first();
        $item->delete();
        $data = [
            'type' => 'lead',
            'follow_id' => $followUpData->id,
            'lead_id' => $followUpData->lead_id,
            'comment' => 'Delete Sub Task Note OF ' . $followUpData->event_name,
            'message_type' => 'text',
            'user_id' => Auth()->user()->id,
        ];
        FollowUpComment::create($data);
    ...
    public function change_status(Request $request)
    {
        ...
        $data = [
            ...
            'lead_id' => $item->lead_id,
            'comment' => 'Change Status OF ' . $item->event_name . ' To ' . $statusName,
            'message_type' => 'text',
            'user_id' => Auth()->user()->id,
        ];
        FollowUpComment::create($data);
    ...
    public function change_priority(Request $request)
    {
        ...
        $data = [
            ...
            'follow_id' => $request->id,
            'lead_id' => $item->lead_id,
            'comment' => 'Change Priority OF ' . $item->event_name . ' To ' . $priorityName,
            'message_type' => 'text',
            'user_id' => Auth()->user()->id,
        ];
        FollowUpComment::create($data);
    ...
    {
        $lead = Lead::with('leadMemberDetail')->where('id', $request->lead_id)->first();
        if (isset($lead)) {
            $followUp = new FollowUpEvent();
            $followUp->lead_id = $lead->id;
            $followUp->event_name = $request->follow_up_remarks;
            $followUp->event_start = Carbon::now()->format('Y-m-d');
            $followUp->event_end = Carbon::tomorrow()->format('Y-m-d');
            $followUp->created_by = Auth()->user()->id;
            $followUpLastId =  FollowUpEvent::all()->last() ? Lead::all()->last()->id + 1 : 1;
            $followUpCode = 'MP-FLD-' . $lead->id . "-" . substr("00000{$followUpLastId}", -6);
            $followUp->follow_up_code = $followUpCode;
            $followUp->event_status = 1;
            $followUp->save();
            if ($followUp) {
                foreach ($lead->leadMemberDetail as $key => $member) {
                    $followMember = new FollowUpMember();
                    $followMember->user_id = $member->user_id;
                ...
                Log::create([
                    'user_id' => auth()->user()->id,
                    'module' => 'Follow Up',
                    'description' => auth()->user()->name . " Created a Follow Up named '" . $followUp->event_name . "'"
                ]);
                return redirect()->route('leads.index')->with('success', 'Follow up Created successfully');
            }
            return redirect()->route('leads.index')->with('error', 'Something went to wrong.');
        }
        return redirect()->route('leads.index')->with('error', 'Something went to wrong.');
    }
}
