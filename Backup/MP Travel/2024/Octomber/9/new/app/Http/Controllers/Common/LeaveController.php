...

{
...

    {
        ...
        $this->leave = resolve(Leave::class)->with('leaveComment', 'userDetail');
    ...
    public function leaveAjaxList(Request $request)
    {
        ...
        $leaveList = $leaveList->when($search, function ($query, $search) {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $callback = function () use ($leaveList, $columns) {
                ...
                foreach ($leaveList as $leave) {
                    ...
                    $employee = isset($leave->userDetail) ? ucfirst($leave->userDetail->name) : "-";
    ...
    public function create()
    {
        ...
        return view('admin.leave.create', compact('userList'));
    }
    ...
    {
        $userList = User::where('role_id', '!=', '1')->get();
        return view('admin.leave.edit', compact('leave', 'userList'));
    }
    ...
    public function removeCardLeave(Request $request)
    {
        $column = $request->type;
        $leave = Leave::find($request->id);
    ...
    public function replyLeave($id)
    {
        $leave = $this->leave->find($id);
        return view('admin.leave.reply', compact('leave'));
    }
    public function addCommentLeave(Request $request)
    {
        $leaveComment = new LeaveComments();
        $leaveComment->leave_id = $request->leave_id;
        $leaveComment->comment = $request->comment_message;
        $leaveComment->message_type = 'text';
        $leaveComment->user_id = Auth()->user()->id;
        if ($request->hasFile('message_file')) {
            ...
            $leaveComment->message_file = $messageFilePath;
            $leaveComment->message_type = "file";
            $leaveComment->file_ext = $file->getClientOriginalExtension();
        }
        
        if ($request->comment_message !== null && $request->message_file !== null) {
            
            $leaveComment->message_type = "text_file";
        }

        $insert = $leaveComment->save();
        ...
        if ($insert) {
            ...
            foreach ($userList as $key => $user) {
                ...
                $offerData = [
                    ...
                    'url' => route('reply-leave', $request->ticket_id),
            ...
            $LeaveDetail = Leave::find($request->leave_id);
            ...
            $offerData = [
                ...
                'url' => route('reply-leave', $request->leave_id),
            ...
            Log::create([
                ...
                'description' => auth()->user()->name . " added a comment in Leave"

            ]);

            return response()->json(['data' => $leaveComment, 'message' => '', 'status' => 1], 200);
        }
