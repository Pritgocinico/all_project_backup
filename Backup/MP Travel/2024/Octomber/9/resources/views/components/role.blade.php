@if (collect($accesses)->where('menu_id', '4')->first()->status != 0 ||
    ...
    <li class="nav-item my-1">
        <a class="nav-link d-flex align-items-center {{ request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*') || request()->routeIs('role.*') || request()->routeIs('user.*') ? 'active collapsed' : '' }}"
            href="#sidebar-user" data-bs-toggle="collapse" role="button"
            aria-expanded="{{ request()->routeIs('role.*') || request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*') || request()->routeIs('user.*') ? 'true' : 'false' }}"
        ...
        <div class="collapse {{ request()->routeIs('role.*') || request()->routeIs('service-preference.*') || request()->routeIs('document-checklist.*') || request()->routeIs('user.*') ? 'show' : '' }}"
            id="sidebar-user">
