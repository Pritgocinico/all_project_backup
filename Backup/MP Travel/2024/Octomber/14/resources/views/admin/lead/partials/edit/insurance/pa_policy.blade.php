...
<div class="row align-items-center g-3 mt-6">
    <div class="col-md-2"><label class="form-label mb-0">Permanent Partial Disability+Permanent Total
...
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-4">
        <select name="hospital_cash_benefit" id="hospital_cash_benefit" class="form-control">
            ...
            <option value="yes"
                {{ $insuranceData->hospital_road_ambulance_covercash_benefit == 'yes' ? 'selected' : '' }}>Yes
            </option>
...
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-4">
        <select name="travel_expenses_benefit" id="travel_expenses_benefit" class="form-control">
            ...
            <option value="yes" {{ $insuranceData->travel_expense_benefit == 'yes' ? 'selected' : '' }}>Yes
            </option>
...
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-4">
        <select name="accidental_hospitalization_expenses" id="accidental_hospitalization_expenses"
            ...
            <option value="yes"
                {{ $insuranceData->accidental_hospitalization_expenses == 'yes' ? 'selected' : '' }}>Yes</option>
            <option value="no"
                {{ $insuranceData->accidental_hospitalization_expenses == 'no' ? 'selected' : '' }}>No</option>
        </select>
    </div>
    ...
    <div class="col-md-4">
        <select name="adventure_sports_benefit" id="adventure_sports_benefit" class="form-control">
            ...
            <option value="yes" {{ $insuranceData->adventure_sports_benefit == 'yes' ? 'selected' : '' }}>Yes
            </option>
            <option value="no" {{ $insuranceData->adventure_sports_benefit == 'no' ? 'selected' : '' }}>No
            </option>
        </select>
    </div>
</div>
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-4">
        <select name="childrens_education_benefit" id="childrens_education_benefit" class="form-control">
            ...
            <option value="yes" {{ $insuranceData->child_education_benefit == 'yes' ? 'selected' : '' }}>Yes
            </option>
            <option value="no" {{ $insuranceData->child_education_benefit == 'no' ? 'selected' : '' }}>No
            </option>
        </select>
    </div>
</div>
...
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-4">
        <input type="file" name="pa_policy_attachement[]" id="pa_policy_attachement" class="form-control"
            multiple>
    </div>
    <div class="col-md-6">
        <div class="row existing_lead_div_outer">
            @foreach ($paPolicyAttachment as $paPolicyAttac)
                <div class="col-md-2 existing_lead_div position-relative"
                    id="remove_pa_policy_div{{ $paPolicyAttac->id }}">
                    <a href="{{ asset('storage/' . $paPolicyAttac->file_path) }}" target="_blank"
                        class="btn btn-dark btn-sm w-100" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="{{ $paPolicyAttac->file_name }}">View</a>
                    <span>{{ $paPolicyAttac->file_name }}</span>
                    <a class="position-absolute lead_file_remove_btn" data-bs-toggle="tooltip"
                        onclick="removePaPolicy({{ $paPolicyAttac->id }})" data-bs-placement="top"
...
<div class="row align-items-center g-3 mt-6">
    ...
    <div class="col-md-6">
        <div class="row existing_lead_div_outer">
            @foreach ($paOtherAttachment as $paOtherAttac)
                <div class="col-md-2 existing_lead_div position-relative"
                    id="remove_fire_other_div{{ $paOtherAttac->id }}">
                    <a href="{{ asset('storage/' . $paOtherAttac->file_path) }}" target="_blank"
                        class="btn btn-dark btn-sm w-100" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="{{ $paOtherAttac->file_name }}">View</a>
                    <span>{{ $paOtherAttac->file_name }}</span>
                    <a class="position-absolute lead_file_remove_btn" data-bs-toggle="tooltip"
                        onclick="removeFireOther({{ $paOtherAttac->id }})" data-bs-placement="top"
                        title="Remove File"><i class="fa-solid fa-circle-xmark"></i></a>
                </div>
            @endforeach
        </div>
