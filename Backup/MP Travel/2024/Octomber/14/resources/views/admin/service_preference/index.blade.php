...
@section('content')
    <div
        ...
        <div class="modal fade" id="addServicePreferenceDetail" tabindex="-1" aria-labelledby="depositLiquidityModalLabel"
            aria-hidden="true">
                        ...
                        <div class="modal-body undefined">
                            <div class="vstack gap-1">
                                <div class="row align-items-center g-3 mt-6">
                                    <div class="col-md-5"><label class="form-label mb-0">Service Preference Name<span
                                                class="error_span">*</span></label></div>
                                    <div class="col-md-7 col-xl-7">
                                        <input type="text" name="service_preference_name" class="form-control"
                                            id="service_preference_name" placeholder="   Service Preference Name">
                                        <div id="service_preference_name_error" class="error_span"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="editServicePreferenceDetail" tabindex="-1" aria-labelledby="depositLiquidityModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content overflow-hidden">
                    <div class="modal-header pb-0 border-0">
                        ...
                        <div class="modal-body undefined">
                            <div class="vstack gap-1">
                                <div class="row align-items-center g-3 mt-6">
                                    ...
                                    <div class="col-md-7 col-xl-7">
                                        <input type="text" name="service_preference_name" class="form-control"
                                            id="edit_sevice_preference_name" placeholder="Service Preference Name">
                                            <div id="edit_sevice_preference_name_error" class="error_span"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script>
        ...
        function submitForm() {
            var service = $('#service_preference_name').val();
            if (service.trim() == "") {
                $('#service_preference_name_error').html('Please Enter Service Preference.');
                return false;
            }
        ...
        function editServicePreference(id) {
            $.ajax({
                url: "{{ route('service-preference.edit', ['service_preference' => 'empid']) }}".replace('empid',
                    id),
                type: 'GET',
                dataType: 'json',
                success: function(data) {
        ...
        function deleteServicePreference(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('service-preference.destroy', ['service_preference' => 'empid']) }}"
                            .replace('empid', id),
                        type: 'DELETE',
                        dataType: 'json',
                        data: {
        ...
        function updateServicePreference() {
            ...
            var name = $('#edit_sevice_preference_name').val();
            if (name.trim() == "") {
                $('#edit_sevice_preference_name_error').html('Please Enter Service Preference.');
                editServicePreference(id)
                return false;
            }
            $('#submitBtnUpdate').html('<i class="fa fa-spinner fa-spin"></i>');
            $.ajax({
                url: "{{ route('service-preference.update', ['service_preference' => 'empid']) }}".replace('empid',
                    id),
                type: 'PUT',
        ...
        document.querySelector("#editForm").addEventListener("keydown", function(event) {
            if (event.key === "Enter") {
                event.preventDefault();
            }
        });
