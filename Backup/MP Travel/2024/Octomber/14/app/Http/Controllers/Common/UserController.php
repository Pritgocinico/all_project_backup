...

{
...

    {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $columns = array('Name','Department Name','Designation Name', 'Email', 'Shift Detail', 'Phone Number','Salary', 'Created BY' ,'Created At');
            ...
            $callback = function () use ($userList, $columns) {
                ...
                foreach ($userList as $user) {
                    ...
                    fputcsv($file, array($user->name,$department ,$designation, $user->email, $shift, $user->phone_number, $user->employee_salary,$userName ,$date));
...

    {
        ...
        return view('admin.user.create', compact('userList','countryList', 'roleList', 'departmentList', 'shiftList'));
...

    {
        ...
        $user->basic_amount = $request->basic_amount;

        $user->hra_amount = $request->hra_amount;

        $user->allowance_amount = $request->allowance_amount;
        $user->is_manager = $request->is_manager;

        $user->petrol_amount = $request->petrol_amount;
        ...
        if ($insert) {
            if(isset($request->user_id)){
                foreach ($request->user_id as $key => $userId) {
                    $managerDetail = new ManagerMemberDetail();
                    $managerDetail->manager_id = $user->id;
                    $managerDetail->user_id = $userId;
                    $managerDetail->save();
                }
            }
...

    {
        ...
        $userManagerData = ManagerMemberDetail::where('manager_id',$id)->pluck('user_id')->toArray();
        // $userArray = ManagerMemberDetail::pluck('user_id')->toArray();
        $userList = User::where('role_id','!=',"1")->get();

        return view('admin.user.edit', compact('userList','userManagerData','shiftList', 'user', 'countryList', 'roleList', 'departmentList', 'deductionDetail'));
...

    {
        ...
        if ($update) {
            ManagerMemberDetail::where('manager_id',$id)->delete();
    ...
    public function leadAjax(Request $request){
        $date = $request->date;
        ...
        $id = $request->id;
        $leadData = Lead::with('leadMemberDetail')->orWhere('created_by', $id)
            ->orWhereHas('leadMemberDetail', function ($query) use($id) {
                $query->orWhere('user_id', $id);
            })->whereDate('created_at', '>=', $startDate)
                ->whereDate('created_at', '<=', $endDate)->latest()->get();
        return view('admin.user.lead_ajax',compact('leadData'));
    }
}
