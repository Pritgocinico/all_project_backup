...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['leads'] = Lead::latest()->get();
        $data['followUpList'] = FollowUpEvent::latest()->get();
        ...
        ->toArray();
        $data['lead_labels'] = ['Pending','Assigned','Hold','Completed','Extends','Cancel'];
        $statusArray = [];
        ...
        ->toArray();
        $data['follow_labels'] = ['Pending','Assigned','Hold','Completed','Extends','Cancel'];
        $followStatusArray = [];
        foreach ($statuses as $status) {
            $followStatusArray[] = isset($followStatusData[$status]) ? $followStatusData[$status] : 0;
        }
        $data['userList'] = User::where('role_id', "!=", 1)->get();
        ...
        $data['salarySlipMonth'] = UtilityHelper::getPreviousMonthName();
        $data['totalEmployee'] = $totalEmployee = User::where('role_id', "!=", 1)->count();
        $data['presentCount'] = $presentCount = Attendance::selectRaw('status, COUNT(*) as count')->where('attendance_date', Carbon::today())
            ->where('status', "1")
            ->count();
        $data['halfDayCount'] = $halfDayCount = Attendance::selectRaw('status, COUNT(*) as count')->whereDate('attendance_date', Carbon::today())
            ->where('status', "2")
            ->count();
    ...
    public function followUpData(Request $request){
        $id = $request->user_id;
        $type = $request->type;
        $followUpData = FollowUpEvent::where('created_by', $id)->where('status',"!=",2)
        ->orWhereHas('followUpMemberDetail',function($query)use($id){
            $query->orWhere('user_id', $id);
        })->when($type=="today",function($query){
            $query->whereDate('event_end', Carbon::today());
        })->when($type=="upcoming",function($query){
            $query->whereDate('event_end','>', Carbon::today());
        })->latest()->get();
        return view('admin.dashboard.followup_ajax',compact('followUpData'));
    }
}
