...

    {
        ...
        if (Auth()->user()->role_id == 2) {
            if (Auth()->user()->is_manager == "yes") {
                ...
                $salarySlipList = $salarySlipList->whereIn('emp_id', $managerMember);
            } else {
                $salarySlipList = $salarySlipList->where('emp_id', Auth()->user()->id);
            }
        }
        $salarySlipList = $salarySlipList->when($search, function ($query, $search) {
        ...
        if ($request->format == "csv" || $request->format == "excel") {
            ...
            $callback = function () use ($salarySlipList, $columns) {
                ...
                foreach ($salarySlipList as $salary) {
                    ...
                    $name = isset($salary->employeeDetail) ? $salary->employeeDetail->name : "-";

                    fputcsv($file, array($name, $salary->month, $salary->year, $salary->total_working_days, $salary->present_days, $salary->payable_salary, $salary->leave));
                }
