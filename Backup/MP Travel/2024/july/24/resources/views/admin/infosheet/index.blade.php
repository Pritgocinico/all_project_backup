                ...
                <div class="d-flex align-items-center mb-5">
                    ...
                    <div class="hstack gap-2 ms-auto">
                        @if (collect($accesses)->where('menu_id', '8')->first()->status == 2)
                            <a href="{{ route('infosheet.create') }}" class="btn btn-sm btn-primary"><i
                    ...
                    <tbody>
                        @forelse ($infosheetList as $key=>$infosheet)
                            <tr>
                                ...
                                <td> <a href="{{ Storage::url($infosheet->info_sheet) }}" download><img src="{{ url('assets/img/user/file.png') }}" width="60px"></a> </td>
                                    ...
                                    @if (collect($accesses)->where('menu_id', '8')->first()->status == 2)
                                        <div class="dropdown"><a class="text-bg-dark rounded-circle" href="#"
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a class="dropdown-item" href="{{ route('infosheet.edit', $infosheet->id) }}"><i
...
@section('script')
    <script>
        function deleteInfoSheet(id) {
            Swal.fire({
                title: 'Are you sure?',
                text: "Are you sure you want to delete this InfoSheet?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('infosheet.destroy', '') }}" + "/" + id,
                        type: 'DELETE',
                        dataType: 'json',
                        data: {
                            _token: "{{ csrf_token() }}",
