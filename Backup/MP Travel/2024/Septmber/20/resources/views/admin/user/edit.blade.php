                ...
                <main class="container-fluid px-6 pb-10">
                    ...
                    <form action="{{ route('user.update', $user->id) }}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="row align-items-center g-3 mt-6">
                            ...
                            <div class="col-md-2 manger_list_div {{$user->is_manager !== "yes" ? "d-none" : ""}}"><label class="form-label mb-0">User List</label></div>

                            <div class="col-md-4 col-xl-4 manger_list_div {{$user->is_manager !== "yes" ? "d-none" : ""}}">

                                <select name="user_id[]" class="form-control js-example-basic-single" id="user_id" multiple>
                                    <option value="">Select User</option>
                                    @foreach ($userList as $user)
                                        <option value="{{$user->id}}" {{ in_array($user->id, $userManagerData?? []) ? 'selected' : '' }}>{{$user->name}}</option>
                                    @endforeach
                                </select>
