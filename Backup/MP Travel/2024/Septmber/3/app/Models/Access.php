    ...
    public function sortMenus($accesses, $only_active_menu = false)
    {
        ...
        foreach ($accesses as $access) {
            if ($only_active_menu == true) {
                ...
                if (isset($access->menu) && $access->menu->is_active == false) {
                    continue;
                }
                if (isset($access->menu) && $access->menu->parent_id == 0){
                    array_push($menus, $access);
                }else {
                    $key = array_search(isset($access->menu) && $access->menu->parent_id, array_column($menus, 'menu_id'));
                    $menus[$key] = $access;
                }
            }
        }
