    ...
    <div class="row">
        <div class="col-md-4">
            <div class="page-header">
                <div>
                    <h3>Add New Qualtiy Analytic</h3>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <form action="{{ route('admin.add.quality.analytic') }}" method="POST">
                                        @csrf
        ...
        <div class="col-md-8">
            <div class="page-header d-md-flex justify-content-between">
                <div>
                    <h3 class="mb-0">Qualtiy Analytic</h3>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($qualityUser as $quality)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>{{ $quality->name }}</td>
                                    <td>{{ $quality->email }}</td>
                                    <td>{{ $quality->phone }}</td>
                                    <td>{{ $quality->created_at->format('d/m/Y h:i:s') }}</td>
                                    <td>
                                        @if ($quality->app_user_active == '0')
                                            <a href="javascript:void(0);" data-id="{{ $quality->id }}"
                                                class="ms-2 verify-btn"><i class="fa fa-check" aria-hidden="true"></i>
                                            </a>
                                        @endif
                                        <a href="#" class="editFitting" data-id="{{ $quality->id }}"
        ...
        <div class="modal-dialog modal-dialog-centered  modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
                            <form action="" method="POST" id="fittingForm" name="fittingForm">
                                @csrf
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label for="adminPassword">Password <span class="text-danger">(*Leave blank if don't
                                            want to change)</span></label>
                                ...
                                <input type="hidden" name="customer_id" value="" id="customerid_hidden">
                            </form>
                        </div>
                    </div>
                </div>
...
@section('script')
    <script>
        $(document).on('click', '.editFitting', function() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('admin.edit.quality.analytic') }}",
                data: {
                    'id': id
                },
                success: function(data) {
                    var action = "{{ route('admin.update.quality.analytic') }}";
        ...
        $(document).on('click', '#updateCustomerBtn', function() {
            ...
            if (i == 0) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.update.quality.analytic') }}",
        ...
        $(document).ready(function() {
            $(document).on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.delete.quality.analytic', '') }}" + "/" +
                                employee_id,
                            type: 'GET',
                            dataType: 'json',
                            success: function(data) {
                                Swal.fire({
                                    title: 'Deleted!',
                                    text: "Quality Analytic User has been deleted.",
                                    icon: 'success',
                                    showCancelButton: false,
                                    confirmButtonColor: '#3085d6',
                                    cancelButtonColor: '#d33',
