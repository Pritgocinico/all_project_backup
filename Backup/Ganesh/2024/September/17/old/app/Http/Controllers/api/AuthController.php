...
class AuthController extends Controller
{
    ...
    public function loginUser(Request $req)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($fieldType === 'email') {
                ...
                $fieldTypeData = 'customer_id';
            }
            // Check Admin
            $type = 'user';
            $loginColumnIp = 'last_login_ip';
            if ($fieldTypeData == "customer_id") {
                ...
                $role = isset($user) ? in_array($user->role, [1, 3, 5, 6, 8,10]) : true;
            }
    ...
    public function signUp(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($insert) {
                return response()->json(['status' => 1, 'data' => $array, 'message' => 'Your account should be processed and active within 24 hours. Once you receive your "Account Approval Notification" SMS, you can start use app.'], 200);
            } else {
                return response()->json(['status' => 0, 'error' => 'Something went wrong.'], 404);
            }
        }
        ...
        if ($validator->fails()) {
            ...
            if (isset($user)) {
                if (Hash::check($request->old_password, $user->password)) {
                    ...
                    $log->save();
                    return response()->json(['result' => 1, 'status' => 1, 'description' => 'Password change successfully.'], 200);
                } else {
                    return response()->json(['result' => 0, 'status' => 0, 'description' => 'Old password is Incorrect!!!'], 404);
                }
            } else {
                return response()->json(['result' => 0, 'status' => 0, 'description' => 'Something went wrong.'], 404);
            }
        }
    }
