...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        if ($role == 1) {
            ...
            $completedTasks = TaskManagement::where('deleted_at', null)->whereHas('project')->get();
            return view('purchase.dashboard', compact('totalCustomer','projects','page', 'icon', 'completedTasks'));
        } else if($role == 10){
            $page = 'Quality Analytic Dashboard';
            $icon = 'dashboard.png';
            $projects = Project::whereIn('status', [1, 2])->get();
            $totalCustomer = Customer::get();
            $completedTasks = TaskManagement::where('deleted_at', null)->whereHas('project')->get();
            return view('quality_analytic.dashboard', compact('totalCustomer','projects','page', 'icon', 'completedTasks'));
        }
    }
    public function edit_profile()
    {
        ...
        if (Auth::user()->role == 1) {
            ...
            return view('purchase/profile/edit_profile', compact('user', 'page', 'icon'));
        } else if(Auth::user()->role == 10){
            return view('quality_analytic/profile/edit_profile', compact('user', 'page', 'icon'));
        }
    ...
    public function userVerify($id = null){
        $userData = Customer::where('id', $id)->first();
        ...
        if($update){
            ...
            try {
                ...
                $message = "Your user id of Customer has been created in the CRM of Shree Ganesh Aluminum. Id: " . $userData->customer_id . " , Password: " . $password;
                $setting = Setting::first();
                $number = [];
