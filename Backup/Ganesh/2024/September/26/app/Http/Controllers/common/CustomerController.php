...
class CustomerController extends Controller
{
    ...
    public function customers(WorldHelper $world, Request $req)
    {
        ...
        $projectCount++;
        $projectIdPadding = str_pad($projectCount, 6, '0', STR_PAD_LEFT);
        $customerId = 'SGACUS' . $projectIdPadding;
        if(Auth::user()->role == 1){
            return view('admin.customers.customers', compact('page', 'icon', 'customer', 'states', 'cities', 'customerId'));
        }
        if(Auth::user()->role == 8){
            return view('purchase.customers.customers', compact('page', 'icon', 'customer', 'states', 'cities', 'customerId'));
        }
        return view('quotation.customers.customers', compact('page', 'icon', 'customer', 'states', 'cities', 'customerId'));
    }
    public function addCustomer(Request $request)
    {
        ...
        if(Auth::user()->role == 8){
            return view('purchase.customers.add_customer', compact('page', 'icon', 'customer'));
        }
        return view('quotation.customers.add_customer', compact('page', 'icon', 'customer'));
    }
    public function addCustomerData(WorldHelper $world, Request $request, $id = NULL)
    {
        ...
        if ($insert) {
            ...
            if (!blank($id) && $id != '') {
                ...
                if(Auth::user()->role == 8){
                    return redirect()->route('purchase.customers');
                }
                return redirect()->route('quotation.customers');
            }
        } else {
            ...
            if(Auth::user()->role == 8){
                return redirect()->route('purchase.add_customer');
            }
            return redirect()->route('quotation.add_customer');
        }
    }
    public function editCustomer(WorldHelper $world, Request $request, $id = NULL)
    {
        ...
        if(Auth::user()->role == 8){
            return view('purchase.customers.edit_customer', compact('page', 'icon', 'customer','states'));
        }
        return view('quotation.customers.edit_customer', compact('page', 'icon', 'customer','states'));
    }
    public function updateCustomer(WorldHelper $world, Request $request, $id)
    {
        ...
        try {
            ...
            $password = $customer->original_password;
