...
<div class="project">
    <div class="page-header d-md-flex justify-content-between  align-items-center">
        ...
        <div class="">
            <a href="{{ route('addleads') }}" class="btn btn-primary ms-auto">
        ...
        <div class="card-body">
            <table id="leads_table" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td> {{ $i++ }} </td>
                        <td><a href="{{route('view.lead.project',$lead->id)}}" class="text-dark">{{ $lead->lead_no }}</a>
                        </td>
                        <td>
                            @if ($lead->customer)
                            <a href="{{route('view.lead.project',$lead->id)}}" class="text-dark">{{ $lead->customer->name }}</a>
                            @else
                            No Customer
                            @endif
                        </td>
                        ...
                        <td>
                            @if($lead->lead_cancel_status == 1)
                                <span class="badge bg-secondary">Lead Cancel</span> - {{date('d/m/Y h:i:s A',strtotime($lead->lead_cancel_date_time))}}
                            @else
                        ...
                        <td>
                            <div class="d-flex">
                                <a href="{{ route('view.lead', $lead->id) }}" class="editUser"><i
                                ...
                                <span class="badge bg-success ms-2">
                                    <a href="javascript:void(0);" class="text-light convert_project"
                                        data-lead-id="{{ $lead->id }}" id="convertBtn{{ $lead->id }}">Convert To
                                        Project</a>
                                </span>
                                @else
    ...
    $(document).ready(function () {
        $(document).on('click', '.project_delete_btn', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('delete.project', '') }}" + "/" + leads_id,
                        type: 'GET',
                        dataType: 'json',
        ...
        $(document).on('click', '.convert_project', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('convert.to.project', '') }}" + "/" + leadId,
                        type: 'GET',
                        dataType: 'json',
                        success: function (data) {
