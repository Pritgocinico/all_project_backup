                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($project->add_work == 2)
                                            @php
                                                $lastAdditionalProjectDetail = \App\Models\AdditionalProjectDetail::where(
                                                    'project_id',
                                                    $project->id,
                                                )
                                                    ->latest()
                                                    ->first();
                                            @endphp
                                            Issue Work - {{ $project->total_issue_work }} -
                                            {{ $lastAdditionalProjectDetail->sub_step }}
                                        @endif
                                        @if ($project->status == 0)
                                    ...
                                    <td>
                                        @if ($project->step == 2 || $project->step == 1 || $project->step == 0)
                                            <a href="#" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="Convert Lead" onclick="conertToLead('{{ $project->id }}')"
                                                class="generateReport"><i class="fa fa-rotate-right"></i></i>
                                                </i>
                                            </a>
                                        @endif
                                        @if ($project->status == 2)
...
@section('script')
    <script>
        ...
        function conertToLead(id) {
            Swal.fire({
                title: 'Are you sure?',
                text: "Are you sure Convert To Lead?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('convert.lead.project', '') }}" + "/" + id,
                        type: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            if (data.status == '1') {
                                Swal.fire({
                                    title: 'create!',
                                    text: "Convert To Lead Successfully.",
                                    icon: 'success',
                                    showCancelButton: false,
                                    confirmButtonColor: '#3085d6',
                                    cancelButtonColor: '#d33',
