...
class ProjectController extends Controller
{
    ...
    public function projects()
    {
        $page = 'Projects';
        $users = Customer::all();
        $projects = Project::with('user')->orderBy('id', 'DESC')->whereIn('status',[1,2])->get();
        $type = 'Project';
        if(Auth::user()->role == 1){
    ...
    public function viewproject(WorldHelper $world, Request $request, $id)
    {
        ...
        if(Auth::user()->role == 1){
            if($projects->step == 0){
                ...
                return redirect()->route('view.complete', $projects->id);
            }
        }else{
            if($projects->step == 0){
                ...
                return redirect()->route('quotation_view.complete', $projects->id);
            }
        }
    }
    ...
    public function storeProjectCost(Request $request){
        ...
        $project->margin_cost = $request->project_cost - $request->quotation_cost;
        $project->save();
        return redirect()->back();
    }
    ...
    public function viewcomplete(Request $request, $id){
        ...
        if(Auth::user()->role == 1){
            return view('admin.projects.view_complete', compact('type','page', 'projects', 'fittings','purchases', 'workshops', 'quotations', 'quotationfiles', 'measurements', 'measurementfiles', 'measurementphotos', 'sitephotos', 'workshop_questions', 'workshop_doneTasks', 'invoiceFiles', 'fitting_questions', 'fitting_doneTasks', 'partialDeliverDatas'));
        }else{
            return view('quotation.projects.view_complete', compact('type','page', 'projects', 'fittings','purchases', 'workshops', 'quotations', 'quotationfiles', 'measurements', 'measurementfiles', 'measurementphotos', 'sitephotos', 'workshop_questions', 'workshop_doneTasks', 'invoiceFiles', 'fitting_questions', 'fitting_doneTasks', 'partialDeliverDatas'));
        }
    }
    ...
    public function updateCutting(Request $request){
        ...
        if($request->cutting_option == 0){
            $message = "Workshop cutting for ".$project->project_generated_id ." is not done, kindly complete it withing next 24 hours. - Shri Ganesh Aluminum.";
    ...
    public function updateShutter(Request $request){
        ...
        if($request->shutter_joinery == 0){
            $message = "Workshop shutter joinery for ".$project->project_generated_id." is not done. Kindly complete it within next 24 hours. Shree Ganesh Aluminum";
    ...
    public function updateGlassmeasure(Request $request){
        ...
        if($request->glass_measurement == 0){
            $message = "Workshop glass measurement for ".$project->project_generated_id." is not done. Kindly complete it within next 24 hours. Shree Ganesh Aluminum";
    ...
    public function updateGlassReceive(Request $request){
        ...
        if($request->glass_receive == 0){
            $message = "Workshop glass is not received for ".$project->project_generated_id.". Shree Ganesh Aluminum";
    ...
    public function updateShutterReady(Request $request){
        ...
        if($request->shutter_ready == 0){
            $message = "Workshop glass fitting is not done for ".$project->project_generated_id.". Kindly complete it within next 24 hours. Shree Ganesh Aluminum";
