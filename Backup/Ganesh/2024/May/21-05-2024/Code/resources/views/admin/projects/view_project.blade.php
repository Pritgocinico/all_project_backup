...
<div class="fieldset">
    <div class="project">
        ...
        <hr>
        <form class="alert-repeater" action="{{ route('update.project', $projects->id) }}" enctype="multipart/form-data"
            method="POST">
            @csrf
            <div class="form-row row">
                ...
                {{-- <div class="form-group col-md-4">
                    ...
                    <select class="form-control select2-example" id="cityname" name="cityname">
                        ...
                        <option <?php if($projects->cityname == $city['name']){ echo "selected"; } ?>
                ...
                <div class="form-group col-md-8">
                    ...
                    <select class="form-control select2-example" id="measurement_user" name="measurement_user[]" multiple>
                        <option value="">Select User</option>
                        @foreach($measurement_users as $measurement)
                            <option value="{{ $measurement->id }}" 
                                @if(in_array($measurement->id, $measurementtaskUsers->pluck('user_id')->toArray())) 
                                    selected 
                                @endif>
                                {{ $measurement->name }}
                            </option>
        ...
        <div class="d-flex align-items-center justify-content-end mt-3">
            <a href="{{ route('view.measurement', $projects->id) }}" class="btn btn-primary">Next <i
        ...
        $(document).on('click', '#Customername', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // console.log(data.city);
                    $("#cityname").val(data.city).trigger('change');
                    // $('#cityname').val('Ahmedabad').trigger('change');
                },
                error: function(data) {
                    // Handle error
                }
