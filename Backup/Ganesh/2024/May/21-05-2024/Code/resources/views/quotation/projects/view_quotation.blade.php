...
<div class="fieldset">
    <div class="d-flex align-items-center">
        ...
        <div class="add_quotation_btn btn btn-primary mt-1 ms-auto">Add Quotation</div>
    </div>
    <hr>
    <form class="" action="{{ route('quotation_store.quotations') }}" enctype="multipart/form-data" method="POST">
        @csrf
        <input type="hidden" name="project_id" value="{{ $projects->id }}">
        @if ($quotations)
        <input type="hidden" name="quotation_id" value="{{ $quotations->id }}">
        @else
        <!-- Handle the case when $quotations is null -->
        <input type="hidden" name="quotation_id" value="">
        @endif
        <?php $i=0; ?>
        {{-- <div class="d-none">
            <div class="quotationCard row quotation_add_div ">
                <div class="form-group">
                    <label for="formFile_1" class="form-label">Upload Quotation File <span
                            class="text-danger">*</span></label>
                    <input class="form-control" type="file" id="formFile_1" name="quotation[0][quotations_file][]" multiple>
                </div>
                <div class="form-group">
                    <label for="quotationCost1" class="form-label">Quotation Cost<span
                            class="text-danger">*</span></label>
                    <input class="form-control" type="number" id="quotationCost1" name="quotation[0][quotation_cost]">
                </div>
                <div class="form-group">
                    <label for="quotationCost1" class="form-label">Project Cost<span
                            class="text-danger">*</span></label>
                    <input class="form-control" type="number" id="projectCost1" name="quotation[0][project_cost]">
                </div>
                <div class="form-group">
                    <label for="measurementdesc">Quotation Description <span class="text-danger">*</span></label>
                    <textarea class="form-control" id="measurementdesc" rows="3"
                        name="quotation[0][description]"></textarea>
                    @if ($errors->has('description'))
                    <span class="text-danger">{{ $errors->first('description') }}</span>
                    @endif
                </div>
                <input type="hidden" name="quotation[0][hidden_div]" value="1">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="quotationfinal1" name="quotation[0][final]">
                    <label for="quotationfinal1" class="form-check-label">Final Quotation?</label>
                </div>
            </div>
        </div> --}}
            ...
            @if(!blank($quotationfiles))
                ...
                <?php $i++; ?>
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-auto">
                            @if($quo->status == 0 || $quo->status == NULL)
                                {{-- <a href="javascript:void(0);" class="btn btn-warning finalizeQuotation" data-id="{{$quo->id}}">
                                    Finalize
                                </a> --}}
                                {{-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
                                    Launch static backdrop modal
                                  </button> --}}
                                {{-- <a href="javascript:void(0);" class="btn btn-danger rejectQuotation" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-id="{{$quo->id}}">
                                    Reject
                                </a> --}}
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu">
                                      <li><a class="dropdown-item rejectQuotation" href="#" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-reason="1" data-id="{{$quo->id}}">Delayed/Cool of</a></li>
                                      <li><a class="dropdown-item rejectQuotation" href="#" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-reason="2" data-id="{{$quo->id}}">Cancel</a></li>
                                      <li><a class="dropdown-item rejectQuotation" href="#" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-reason="3" data-id="{{$quo->id}}">Addon</a></li>
                                    </ul>
                                  </div>
                            @else
                                {{-- @if ($quo->status == 1)
                                ...
                                @endif --}}
                            @endif
                            <a class="btn btn-primary" data-bs-toggle="collapse" href="#collapseExample{{$quo->id}}" role="button" aria-expanded="false" aria-controls="collapseExample">
                                Edit Quotation
                            </a>
                        </div>
                    </div>
                    <div class="collapse" id="collapseExample{{$quo->id}}">
                        <div class="quotationCard row">
                            <div class="form-group col-md-8">
                                <input type="hidden" name="quotation[{{$i}}][id]" value="{{$quo->id}}">
                                <label for="formFile" class="form-label">Upload Quotation File</label>
                                <input class="form-control" type="file" id="formFile" name="quotation[{{$i}}][quotations_file][]" multiple>
                            </div>
                            {{-- <div class="form-group col-md-6">
                                <label for="quotationCost{{$i}}" class="form-label">Quotation Cost<span
                                        class="text-danger">*</span></label>
                                <input class="form-control" type="number" id="quotationCost{{$i}}" name="quotation[{{$i}}][quotation_cost]" value="{{$quo->cost}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="quotationCost{{$i}}" class="form-label">Project Cost<span
                                        class="text-danger">*</span></label>
                                <input class="form-control" type="number" id="projectCost{{$i}}" name="quotation[{{$i}}][project_cost]" value="{{$quo->project_cost}}">
                            </div> --}}
                            <div class="form-group col-md-12">
                                <label for="measurementdesc">Quotation Description <span class="text-danger">*</span></label>
                                <textarea class="form-control" id="measurementdesc" rows="3"
                                    name="quotation[{{$i}}][description]">@if(!empty($quo->description)){{$quo->description}} @else {{old('description')}} @endif</textarea>
                                @if ($errors->has('description'))
                                <span class="text-danger">{{ $errors->first('description') }}</span>
                                @endif
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="quotationfinal{{$i}}" name="quotation[{{$i}}][final]" @if($quo->final == 1) checked @endif>
                                <label for="quotationfinal{{$i}}" class="form-check-label">Final Quotation?</label>
                            </div>
                    ...
                    <div class="card bg-light card-body mt-3 rounded">
                        {{-- <p><strong>Quotation Cost: </strong>{{$quo->cost}}</p>
                        <p><strong>Project Cost: </strong>{{$quo->project_cost}}</p>
        ...
        {{-- <div class="quotation_new"></div> --}}
        <hr>
    </form>

    <div class="d-flex align-items-center justify-content-end mt-3">
        <a href="{{ route('quotation_view.measurement', $projects->id)  }}" class="btn btn-primary me-3"><i
                data-feather="arrow-left" class="me-2 fw-bold"></i>Previous</a>
        @if($projects->status == 1)
            <a href="{{ !empty($quotations->project_id) ? route('quotation_view.material', $projects->id) : '#' }}"
        ...
        @else
            <a href="{{ !empty($quotations->project_id) ? route('quotation_view.selection', $projects->id) : '#' }}"
            class="{{ !empty($quotations->project_id) ? 'active_btn' : 'disabled' }} btn btn-primary">Next<i
                data-feather="arrow-right" class="ms-2 fw-bold"></i></a>
        @endif
    </div>
...
<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{route('quotation.reject.quotation')}}" method="post">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label for="addressone">Reject Quotation Note *</label>
...
<script>
    $(document).on('click','.finalizeQuotation',function(){
        ...
        $.ajax({
            url: "{{ route('quotation_finalize.quotation', '') }}" + "/" + quotation_id,
            type: 'GET',
            dataType: 'json',
            ...
            var quo_id = $(this).attr('data-id');
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('quotation_delete.quotation', '') }}" + "/" + quo_id,
                            type: 'GET',
                            dataType: 'json',
                            success: function(data) {
                                Swal.fire({
...
<script>
    ...
    $(".add_quotation_btn").click(function() {
        ...
        html += '<input class="form-control" type="file" id="formFile" name="quotations_file[]" multiple>';
        html += '</div>';
...
$(document).ready(function() {
    $(document).on('click', '.delect_btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('quotation_delete.quotation', '') }}" + "/" + employee_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        Swal.fire({
...
$(document).ready(function() {
    $(document).on('click', '.delect_btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('quotation_delete.quotation', '') }}" + "/" +
                        employee_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
