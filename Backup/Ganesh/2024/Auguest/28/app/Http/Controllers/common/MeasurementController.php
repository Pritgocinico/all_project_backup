  ...
  public function view_store_measurement(Request $request)
{
    ...
    // Check if measurements exist for the project
    ...
    // Handle file uploads for measurement files
    ...
    // Handle file uploads for site photos
    ...
    // Handle material option selections
    if ($measurements->material_option == 1 && !blank($request->input('measurement'))) {
    ...
    // Fetch the customer and check if it exists
    ...
    $notificationData = [
        ...
        'url' => Auth::user()->role == 1 ? route('view.measurement', $project->id) : route('quotation_view.measurement', $project->id),
    ];
    Notification::send($user, new OffersNotification($notificationData));

    // Log the action
    $log = new Log();
    $log->user_id = $user->id; // Changed to use user's ID
    ...
    try {
        ...
        $customerDetail = "Name: " . $customer->name . " and Phone Number: " . $customer->phone . " and Address: " . $customer->address . " " . ($customer->cityDetail->name ?? 'N/A') . " " . ($customer->stateDetail->name ?? 'N/A') . " " . $customer->zipcode;
        ...
        SmsHelper::sendSms($mobileNumber, $message, $templateid, true);
        ...
        foreach ($measurementList as $measurement) {
            ...
            if ($userDetail) {
                SmsHelper::sendSms($userDetail->phone, $message, $templateid, false);
            }
        }
    } catch (Exception $e) {
