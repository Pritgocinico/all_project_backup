    ...
    public function listProject(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if($accessToken->type =="customer"){
                ...
                $projects = Project::where('type', 1)->where('customer_id',$accessToken->user_id)->when($status, function ($query) use ($status) {
                ...
                $role = $user->role;
                $status = $request->status;
                $projects = Project::where('type', 1)->when($status, function ($query) use ($status) {
                    $query->where('status', $status);
                })->orderBy('id', 'desc')->get();
            }
            if (!blank($projects)) {
        ...
        if ($validator->fails()) {
            ...
            if (!blank($project)) {
                if ($project->add_work == 1) {
                    ...
                    $addWork->laber_cost = $request->labor_cost;
                    $addWork->save();
                } else {
                    ...
                    $project->laber_cost = $request->labor_cost;
                    $project->save();
                    ...
                    if (isset($addProject) && $project->add_type == 2) {
                        ...
                        $addProject->laber_cost = $request->labor_cost;
                        $addProject->save();
                    }
                }
