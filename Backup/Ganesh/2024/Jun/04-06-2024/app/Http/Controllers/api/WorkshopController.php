...
class WorkshopController extends Controller
{
    ...
    public function listworkshopQuestion(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $role = $user->role;
            $workshopQuestions = WorkshopQuestion::whereIn('project_id', [$request->project_id, 0])->get();
            ...
            if (!blank($workshopTasks)) {
                ...
                foreach ($workshopTasks as $tasks) {
                    ...
                    $array['workshop_question'] = $tasks->wquestion->workshop_question;
                    $array['chk'] = $tasks->chk;
                ...
                if (!blank($workshopQuestions)) {
                    ...
                    foreach ($workshopQuestions as $workshop) {
                        ...
                        $array['project_detail'] = $project_data;
                        $array['workshop_question'] = $workshop->workshop_question;
    ...
    public function updateworkshopQuestion(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $project = Project::where('id', $request->project_id)->first();
            if (!blank($project)) {
                $workshopQuestion = WorkshopQuestion::where('id', $request->question_id)->first();
                // $workshopDoneQuestion = WorkshopDoneTask::where('question_id', $request->question_id)->first();
                if (!blank($workshopQuestion)) {
                    $workshopQuestion->workshop_question = $request->workshop_question;
    ...
    public function deleteworkshopQuestion(Request $request, $id)
    {
        ...
        $workshopQuestion = WorkshopQuestion::find($id);
    ...
    public function checkWorkshop(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user = User::where('id', $accessToken->user_id)->first();
            $role = $user->role;
            $workshopTask = WorkshopDoneTask::where('project_id', $request->project_id)->get();
