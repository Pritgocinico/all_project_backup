        ...
        if ($validator->fails()) {
            ...
            if (!blank($project)) {
                if ($project->add_work == 1) {
                    ...
                    $addWork->transport_cost = $request->transport_cost;
                ...
                $cost = $request->quotation_cost + $request->transport_cost + $request->laber_cost;
                return response()->json(['status' => 1, 'message' => 'Project Cost Calculation.', 'margin' => $project->project_cost - $cost], 200);
            } else {
                return response()->json(['status' => 0, 'error' => 'Project does not exist.'], 404);
            }
            return response()->json(['status' => 0, 'error' => 'Something went wrong.'], 404);
    ...
    public function viewProject(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!blank($project)) {
                ...
                $issueWork = AdditionalProjectDetail::where('project_id', $project->id)->where('status', 'idssue')->get();
                $wokshop_process = [];
                ...
                foreach ($subProjectData as $add) {
                    ...
                    $materialReceivedStatus['add_work'] = 0;
                ...
                $costArray['add_work'] = 1;
                $costData[] = $costArray;
                foreach ($subProjectData as $add) {
                    $cutting_status = [];
                    $cutting_status['id'] = $key1 + 1;
                    $cutting_status['cutting_selection'] = $add->quotation_cost;
                    $cutting_status['project_cost'] = $add->project_cost;
                    $cutting_status['transport_cost'] = $add->transport_cost;
                    $cutting_status['labor_cost'] = $add->laber_cost;
                    $cutting_status['margin_cost'] = $add->margin_cost;
                    $cutting_status['add_work'] = 0;
                    array_push($costData, $cutting_status);
                    $key1++;
                }
                foreach ($issueWork as $issue) {
                    $issue_status = [];
