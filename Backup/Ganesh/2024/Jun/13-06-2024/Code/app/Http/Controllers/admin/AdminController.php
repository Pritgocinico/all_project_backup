...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        if ($role == 1) {
            ...
            $completedProjects = Project::where('status', 2)->get();
            $leads = Project::where('type', 0)->get();
            $completedTasks = TaskManagement::where('task_status', 'completed')->where('deleted_at', null)->get();
            $pendingTasks = TaskManagement::where('task_status', 'pending')->where('deleted_at', null)->get();
            $sales = Project::where('deleted_at', null)->sum('margin_cost');
            $currentMonth = Carbon::now()->month;
            $currentYear = Carbon::now()->year;
            $cancelLead = Project::where('lead_cancel_status', '1')
