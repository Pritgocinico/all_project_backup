...
<div class="fieldset">
    <div class="d-flex">
        ...
        <button type="button" class="btn btn-primary btn-add ms-auto">Add Measurement</button>
    </div>
    <hr>
    <form class="" action="{{ route('quotation_storemeasurement') }}" enctype="multipart/form-data" method="POST">
        @csrf
        <div class="measurement_show_hide" style="display: none">
            <div class="form-group">
                ...
                <input class="form-control" type="file" id="files" name="measurementfile[]" multiple />
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="measurementdesc" rows="5"
                    name="description">@if (!blank($measurements))@if (!empty($measurements->description)){{ $measurements->description }}@endif @else{{ old('description') }}@endif</textarea>
            ...
            <div class="my-3">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        ...
                        <a href="javascript:void(0);" data-id="{{$file->id}}" class="btn btn-danger delete_file"
            ...
            <div class="my-3">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        ...
                        <a href="javascript:void(0);" data-id="{{$sfile->id}}" class="btn btn-danger delete_sfile"
                            download><i class="fa fa-trash"></i></a>
                    </div>
                </div>
                <hr>
    ...
    <div class="collapse" id="collapseExample{{ $i }}">
        <div class="card card-body">
            <form class="" action="{{ route('quotation_edit.measurement', $measure->id) }}"
                        ...
                        <div class="d-flex gap-4">
                            ...
                            <div class="">
                                ...
                                <a href="javascript:void(0);" data-id="{{ $file->id }}"
                                    class="btn btn-primary delect_btn mb-4">Remove</a>
                            </div>
                            @endif
                            @endforeach
                            @endforeach
                    ...
                    <div class=" measurement_div" name="{{ $measurementId ?? '' }}">
                        ...
                        <div class="d-flex gap-4">
                            ...
                            <div class="">
                                ...
                                <a href="javascript:void(0);" data-id="{{ $sfile->id }}"
                                    class="btn btn-primary delect_btn mb-4">Remove</a>
                            </div>
                            @endif
                            @endforeach
                ...
                <div class="form-group">
                    <label for="measurementdesc">Measurement Description </label>
                    <textarea class="form-control" id="measurementdesc" rows="5"
                        name="description">{{ $measure->description }} </textarea>
                    @if ($errors->has('description'))
                    <span class="text-danger">{{ $errors->first('description') }}</span>
                    @endif
                </div>
    ...
    @endif
    @if (!blank($quotations))
    <form class="" action="{{ route('quotation_update_material_status') }}" enctype="multipart/form-data" method="POST">
        @csrf
        <div class="form-group mt-4">
            ...
            <div class="d-flex gap-3">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="material_option" id="flexRadioDefault2" value="0"
                        @if ($projects->material_selection == 0) checked @endif>
                    <label class="form-check-label" for="flexRadioDefault2"> No </label>
                </div>
            </div>
        </div>
        <label for="flexRadioDefault">Selection Date</label>
        <div class="d-flex gap-3">
            <div class="">
                @php
    ...
    @endif
    <hr>
    <!-- <form class="" action="{{ route('quotation_store.project.materials') }}" enctype="multipart/form-data" method="POST">
            @csrf -->
    <input type="hidden" name="project_id" class="projectID" value="{{ $projects->id }}">
    <!-- <button type="submit" class="btn btn-primary mt-2">Submit</button>
        </form> -->
    ...
    <div class="d-flex align-items-center justify-content-end mt-3">
        <a href="{{ route('quotation_view.project', $projects->id) }}" class="btn btn-primary me-3"><i
                data-feather="arrow-left" class="me-2 fw-bold"></i>Previous</a>
        <a class="{{ !blank($measurements) ? 'active_btn' : 'disabled' }} btn btn-primary"
            href="{{ !blank($measurements) ? route('quotation_view.quotation', $projects->id) : '#' }}">Next<i
...
<script>
        ...
        $(document).on('click', '.delect_btn', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('quotation_delete.measurement', '') }}" + "/" +
                            employee_id,
                        type: 'GET',
                        dataType: 'json',
    ...
    $(".delete_file").click(function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('quotation_delete.measurement', '') }}" + "/" + task_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function (data) {
                        Swal.fire({
    ...
    $(".delete_sfile").click(function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('quotation_delete.measurement_pic', '') }}" + "/" + task_id,
                    type: 'GET',
                    dataType: 'json',
                    success: function (data) {
                        Swal.fire({
