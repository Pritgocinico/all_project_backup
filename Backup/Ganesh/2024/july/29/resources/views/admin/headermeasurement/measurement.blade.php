...
<div class="row">
    <div class="col-md-4">
        <div class="page-header">
            <div>
                <h3>Add New Measurement Role</h3>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12">
                                <form action="{{ route('admin.insert.measurement') }}" method="POST">
                                    @csrf
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($measurement->app_user_active == "0")
                                <a href="javascript:void(0);" data-id="{{ $measurement->id }}"
                                    class="ms-2 verify-btn"><i class="fa fa-check" aria-hidden="true"></i>
                                </a>
                                @endif
                                <a href="" class="editMeasurement" data-id="{{ $measurement->id }}"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <form action=""
                            method="POST" id="measurementForm" name="measurementForm">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="adminName">Measurement Name <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control" id="editmeasurementName"
                                    placeholder="Enter Name" value="{{ old('name') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="tel" name="mobile" class="form-control" id="editmeasurementMobile"
                                    placeholder="Enter Mobile Number" value="{{ old('mobile') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" name="email" class="form-control" id="editmeasurementEmail"
                                    placeholder="Enter Email" value="{{ old('email') }}">
                            </div>
...
@section('script')
    <script>
        $(document).on('click', '.editMeasurement', function() {
            var id = $(this).data('id');
            $.ajax({
                type: 'GET',
                url: "{{ route('get_measurement') }}",
                data: {
                    'id': id
                },
                success: function(data) {
                    var action = "{{route('admin.update.measurement','')}}"+"/"+data.id;
                    $('#measurementForm').attr('action',action);
                    $('#editmeasurementName').val(data.name);
                    $('#editmeasurementEmail').val(data.email);
                    $('#editmeasurementMobile').val(data.phone);
        ...
        $(document).on('click', '#updateCustomerBtn', function() {
            var name_value = document.measurementForm.name.value;
            var phone_value = document.measurementForm.mobile.value;
            ...
            if (name_value == "") {
                ...
                document.measurementForm.name.focus();
            }
            if (phone_value == 0) {
                i++;
                phoneError.innerHTML = "Phone must be filled out!";
                phoneError.style.color = "Red";
                document.measurementForm.role.focus();
            }
            var hiddenvalue = document.getElementById('customerid_hidden').value;
            if (i == 0) {
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.update.measurement', '') }}" + '/' + hiddenvalue,
        ...
        $(document).ready(function() {
            $(document).on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('delete.measurement.user', '') }}" + "/" + employee_id,
                            type: 'GET',
                            dataType: 'json',
                            success: function(data) {
                                Swal.fire({
                                    title: 'Deleted!',
                                    text: "Measurement User has been deleted.",
                                    icon: 'success',
                                    showCancelButton: false,
                                    confirmButtonColor: '#3085d6',
                                    cancelButtonColor: '#d33',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(data) {
                                Swal.fire({
                                    title: 'Success!',
                                    text: "Measurement User has been Verified.",
                                    icon: 'success',
                                    showCancelButton: false,
                                    confirmButtonColor: '#3085d6',
                                    cancelButtonColor: '#d33',
