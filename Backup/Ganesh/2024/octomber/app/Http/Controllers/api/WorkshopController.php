...
class WorkshopController extends Controller
{
    ...
    public function updateCutting(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $setting = Setting::first();
            $quotationUser = User::where('role', 5)->get();
            if (!blank($project)) {
                if ($request->cutting_selection == 1) {
                    ...
                    foreach ($quotationUser as $key => $quotation) {
            ...
            if (!blank($project)) {
                if ($request->glass_measurement == 1) {
                    ...
                    if ($setting->wa_message_sent == 1) {
                        ...
                        $isSent = SmsHelper::sendSmsWithTemplate($userDetail->phone, $message, true);
                        $quotationUser = User::where('role', '5')->get();
                        ...
                        $isSent = SmsHelper::sendSms($userDetail->phone, $message, $templateid, true);
                        $quotationUser = User::where('role', '5')->get();
                        foreach ($quotationUser as $quotation) {
                            $mobile = $quotation->phone;
    ...
    public function updateShutterReady(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $userDetail = Customer::where('id', $project->customer_id)->first();
            $mobileNumber = $userDetail->phone;
            $customerAddress = $userDetail->address . ', ' . $userDetail->city . ', ' . $userDetail->state . ', ' . $userDetail->pincode;
            if (!blank($project)) {
            ...
            if (!blank($project)) {
                if ($request->material_delivered == 1) {
                    ...
                    try {
                        ...
                        $setting = Setting::first();
                        if ($setting->wa_message_sent == 1) {
                            $message = "Dear " . $userDetail->name . ", your material has been dispatched from the workshop and will be delivered by " . $request->driver_numbers . ". You can call him for the status of the delivery on " . $request->deliver_date . " and address is " . $customerAddress . ". Shree Ganesh Aluminum";
                            $isSent = SmsHelper::sendSmsWithTemplate($mobileNumber, $message, true);
                            if ($project->driver_number) {
                                $isSent = SmsHelper::sendSmsWithTemplate($project->driver_number, $message, false);
                            }
                        } else {
                    ...
                    try {
                        ...
                        foreach ($filltingUser as $key => $fitting) {
                            ...
                            if ($setting->wa_message_sent == 1) {
                                $message = "Material delivery of the " . $project->name . " is not dispatched yet from workshop kindly complete the delivery within next 24 hours and address is " . $customerAddress . ". Shree Ganesh Aluminum";
                                $isSent = SmsHelper::sendSmsWithTemplate($mobile, $message, $templateid, $status);
                            } else {
                    ...
                    if ($setting->wa_message_sent == 1) {
                        $message = "Dear " . $userDetail->name . ", your material has been dispatched from the workshop and will be delivered by " . $request->driver_numbers . ". You can call him for the status of the delivery on " . $request->deliver_date . " and address is " . $customerAddress . ". Shree Ganesh Aluminum";
                        $isSent = SmsHelper::sendSmsWithTemplate($mobileNumber, $message, $templateid, true);
                    } else {
                        $message = "Dear " . $userDetail->name . ", your material has been dispatched from the workshop and will be delivered by " . $request->driver_numbers . ". You can call him for the status of the delivery on " . $request->deliver_date . ". Shree Ganesh Aluminum";
            ...
            if (!blank($project)) {
                if ($request->invoice_status == 1) {
                    ...
                    try {
                        ...
                        $setting = Setting::first();
                        if ($setting->wa_message_sent == 1) {
                            $isSent = SmsHelper::sendSmsWithFile($mobileNumber, $message, $projectId, true);
                        } else {
                            $isSent = SmsHelper::sendSms($mobileNumber, $message, $templateid, true);
                        }
                    } catch (Exception $e) {
