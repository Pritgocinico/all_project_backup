...
class WorkshopController extends Controller
{
    ...
    public function view_store_workshop(Request $request)
    {
        ...
        if (Auth::user()->role == 1) {
            ...
            return redirect()->route('purchase_view.workshop', $request->project_id)->with('success', 'Workshop created successfully.');
        } else if(Auth::user()->role == 10){
            return redirect()->route('qa_view.workshop', $request->project_id)->with('success', 'Workshop created successfully.');    
        }
    ...
    public function store_workshop_question(Request $request)
    {
        ...
        if (Auth::user()->role == 1) {
            ...
            return redirect()->route('purchase_view.workshop', $request->project_id)->with('success', 'Workshop question submitted successfully.');
        } else if(Auth::user()->role == 10){    
            return redirect()->route('qa_view.workshop', $request->project_id)->with('success', 'Workshop question submitted successfully.');
        }
        ...
        if ($request->material_delivered == 2) {
            ...
            try {
                ...
                if ($setting->wa_message_sent == 1) {
                    $message = "Dear " . $userDetail->name . ", your material has been dispatched from the workshop and will be delivered by " . $partialDetails->partial_deliver_by . " - " . $partialDetails->driver_number . ". You can call him for the status of the delivery on " . $project->deliver_date . " and address is ". $customerAddress.". Shree Ganesh Aluminum";
                    $isSent = SmsHelper::sendSmsWithTemplate($mobileNumber, $message, true);
                    if ($partialDetails->driver_number) {
                        $mobile = $partialDetails->driver_number;
            ...
            try {
                ...
                $templateid = '1407171594318987872';
                if ($setting->wa_message_sent == 1) {
                    $message = "Dear " . $userDetail->name . ", your material has been dispatched from the workshop and will be delivered by " . $project->delivered_by . " - " . $project->driver_number . ". You can call him for the status of the delivery on " . $project->deliver_date . " and address is ". $customerAddress.". Shree Ganesh Aluminum";
                    $isSent = SmsHelper::sendSmsWithTemplate($mobileNumber, $message, true);
                    if ($project->driver_number) {
                        $mobile = $project->driver_number;
                        $isSent = SmsHelper::sendSmsWithTemplate($mobile, $message, false);
                    }
                } else {
    ...
    public function storeInvoiceStatus(Request $request)
    {
        ...
        if ($request->hasFile('invoicefiles')) {
            ...
            $customerAddress = $userDetail->address . " " . $userDetail->cityDetail->name . " " . $userDetail->stateDetail->name . " " . $userDetail->zipcode;
            $mobileNumber = $userDetail->phone;
            $message = "Dear " . $userDetail->name . ", your invoice has been generated for your " . $project->project_generated_id . ", kindly check your project details to this link " . $route . " and address is ". $customerAddress.". Shree Ganesh Aluminum";
            $templateid = '1407171594311415919';
            $setting = Setting::first();
            if($setting->wa_message_sent == 1){
                $isSent = SmsHelper::sendSmsWithFile($mobileNumber, $message,$request->project_id, true);
            } else {
                ...
                $isSent = SmsHelper::sendSmsWithFile($mobileNumber, $message, $templateid, true);
            }
        }
