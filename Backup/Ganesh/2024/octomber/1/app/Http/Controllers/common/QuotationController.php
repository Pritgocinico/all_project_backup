...
class QuotationController extends Controller
{
    ...
    public function viewSelection(Request $request, $id)
    {
        ...
        if (Auth::user()->role == 1) {
            return view('admin.projects.view_selection', compact('issueProject', 'subPurchases', 'subProjectData', 'subPartialDeliverDatas', 'subInvoiceFiles', 'subquotations', 'subquotationfiles', 'sub_quotation_uploads', 'submeasurements', 'sumeasurementfiles', 'sumeasurementphotos', 'type', 'page', 'projects', 'quotations', 'quotationfiles', 'measurements', 'measurementfiles', 'workshops', 'fittings', 'quotation_uploads', 'purchases', 'measurementphotos', 'sitephotos', 'workshop_questions', 'workshop_doneTasks', 'invoiceFiles', 'fitting_questions', 'fitting_doneTasks', 'partialDeliverDatas'));
        } else if (Auth::user()->role == 8) {
            return view('purchase.projects.view_selection', compact('page', 'projects', 'users', 'type'));
        } else if (Auth::user()->role == 10) {
            return view('quality_analytic.projects.view_selection', compact('page', 'projects', 'users', 'type'));
        }
        return view('quotation.projects.view_selection', compact('issueProject', 'subPurchases', 'subProjectData', 'subPartialDeliverDatas', 'subInvoiceFiles', 'subquotations', 'subquotationfiles', 'sub_quotation_uploads', 'submeasurements', 'sumeasurementfiles', 'sumeasurementphotos', 'type', 'page', 'projects', 'quotations', 'quotationfiles', 'measurements', 'measurementfiles', 'workshops', 'fittings', 'quotation_uploads', 'purchases', 'measurementphotos', 'sitephotos', 'workshop_questions', 'workshop_doneTasks', 'invoiceFiles', 'fitting_questions', 'fitting_doneTasks', 'partialDeliverDatas'));
    }
    public function view_store_quotation(Request $request)
    {
                ...
                if (array_key_exists('id', $quotation_data)) {
                    ...
                    if ($request->hasFile('quotations_file')) {
                        ...
                        try {
                            ...
                            $message = "Dear " . $userDetail->name . ", your quotation has been uploaded in our system for the " . $project->lead_no . " and address is ".  $customerAddress ." To check, use the following " . $url . ". Shree Ganesh Aluminum";
                            $templateid = '1407171593880088737';
                            if($setting->wa_message_sent == 1){
                                $isSent = SmsHelper::sendSmsWithQuotationFile($mobileNumber, $message,$request->project_id, true);
                            } else {
        ...
        if (Auth::user()->role == 1) {
            ...
            return redirect()->route('purchase_view.quotation', $request->project_id)->with('created', 'Quotation created successfully.');
        } else if (Auth::user()->role == 10) {
            return redirect()->route('qa_view.quotation', $request->project_id)->with('created', 'Quotation created successfully.');
        }
