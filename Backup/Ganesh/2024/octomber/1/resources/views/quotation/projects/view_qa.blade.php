...
@section('pages')
    ...
    <div class="fieldset">
        ...
        <h2 class="fs-title">Quality Analytic</h2>

        <h3 class="fs-subtitle">Enter your Quality Analytic</h3>

        <div>

            <form class="" action="{{ route('quotation_store_qa_question') }}" enctype="multipart/form-data" method="POST">
                ...
                <div class="fitting_show_hide" style="display: none">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="qa_question" rows="5" name="qa_question"></textarea>

                        @if ($errors->has('qa_question'))
                            <span class="text-danger">{{ $errors->first('qa_question') }}</span>
                        @endif
        ...
        <div class="mt-3">
            ...
            <form class="fitting_form" action="{{ route('quotation_store_qa_data') }}" enctype="multipart/form-data" method="POST">
                ...
                <table class="table">
                    ...
                    @if (count($qa_doneTasks) != 0)
                        @foreach ($qa_doneTasks as $d_item)
                            <tr>

                                <th>{{ $d_item->question->question }}</th>
                        ...
                        @endforeach
                    @else
                        @foreach ($qa_questions as $w_item)
                            <tr>
            ...
            @else
                <form class="" action="{{ route('projectDone') }}" enctype="multipart/form-data" method="POST">
                    ...
                    <div id="project-end-container" class="form-group mt-4">
                        ...
                        <div class="d-flex gap-3">
                            <div class="">
                                <input class="form-control" type="file" name="project_done_photos[]" id="project_done_photos" multiple>
                            </div>
                        </div>
    ...
    <div>
        ...
        @if ( $projects->add_work == '0')
        <form action="{{ route('quotation.resume.work') }}" id="resume_work_form">
            <div class="page-title d-flex justify-content-start gap-1 me-3">
                <input type="hidden" name="resume_work_option" id="status" value="2">
                <input type="hidden" name="project_id" value="{{ $projects->id }}">
                <div
        ...
        <form action="{{ route('quotation.resume.work.complete') }}" id="resume_work_form">
            <div class="page-title d-flex justify-content-start gap-1 me-3">
                <input type="hidden" name="resume_work_option_complete" id="status" value="0">
                <input type="hidden" name="project_id" value="{{ $projects->id }}">
                <div
