...
@section('pages')
    ...
    <div class="fieldset">
        ...
        @endif
        <form class="" action="{{ route('purchase_store.quotations') }}" enctype="multipart/form-data" method="POST">
            @csrf
...
@endif
        ...
        <div class="d-flex align-items-center justify-content-end mt-3">
            <a href="{{ route('purchase_view.measurement', $projects->id) }}" class="btn btn-primary me-3"><i
                    data-feather="arrow-left" class="me-2 fw-bold"></i>Previous</a>
            @if ($projects->status == 1)
                <a href="{{ !empty($quotations->project_id) ? route('purchase_view.material', $projects->id) : '#' }}"
            ...
            @else
                <a href="{{ !empty($quotations->project_id) ? route('purchase_view.selection', $projects->id) : '#' }}"
                    class="{{ !empty($quotations->project_id) ? 'active_btn' : 'disabled' }} btn btn-primary">Next<i
                        data-feather="arrow-right" class="ms-2 fw-bold"></i></a>
            @endif
        </div>
    ...
    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('purchase.reject.quotation') }}" method="post">
                    @csrf
...
@section('script')
    ...
    <script>
        $(document).on('click', '.finalizeQuotation', function() {
            ...
            $.ajax({
                url: "{{ route('purchase_finalize.quotation', '') }}" + "/" + quotation_id,
                type: 'GET',
                dataType: 'json',
