...
class MeasurementController extends Controller
{
    ...
    public function view_store_measurement(Request $request)
    {
        ...
        try {
            ...
            $message = "Dear " . $customer_name . ", your measurement for the " . $project->lead_no . " has been taken on " . date('d-m-Y h:i:s A') . " and address is ".  $customerAddress ." Shree Ganesh Aluminum";
            $templateid = '1407171593796762936';
            $setting = Setting::first();
            if ($setting->wa_message_sent == 1) {
                $isSent = SmsHelper::sendSmsWithMeasurement($mobileNumber, $message,$request->project_id, true);
                ...
                foreach ($measurementList as $measurement) {
                    ...
                    $isSent = SmsHelper::sendSmsWithMeasurement($mobile, $message,$request->project_id, false);
                }
            } else {
        ...
        if (Auth::user()->role == 1) {
            return redirect()->route('view.material', $request->project_id)->with('success', 'Measurement created successfully.');
        } else if (Auth::user()->role == 8) {
            return redirect()->route('quotation_view.material', $request->project_id)->with('success', 'Measurement created successfully.');
        } else if (Auth::user()->role == 10) {
            return redirect()->route('purchase_view.material', $request->project_id)->with('success', 'Measurement created successfully.');
        }
        return redirect()->route('qa_view.material', $request->project_id)->with('success', 'Measurement created successfully.');
    }
    public function editMeasurement(Request $request, $id = NULL)
    {
        ...
        if (Auth::user()->role == 1) {
            ...
            return redirect()->route('purchase_view.measurement', $request->project_id)->with('success', 'Measurement created successfully.');
        } else if (Auth::user()->role == 10) {
            return redirect()->route('qa_view.measurement', $request->project_id)->with('success', 'Measurement created successfully.');
        }
    ...
    public function finalMeasurement(Request $request){
        $request->validate([
            'final_measurement_file' => 'required',
        ]);
        $project = Project::find($request->project_id);
        ...
        if ($request->hasFile('final_measurement_file')) {
            $allowedfileExtension = ['pdf', 'jpg', 'png', 'docx'];
            $files = $request->file('final_measurement_file');
