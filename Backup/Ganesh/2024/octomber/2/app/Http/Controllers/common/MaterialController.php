    ...
    public function storeMaterial(Request $request)
    {
        ...
        if ($request->has('purchase') && !blank($materials)) {
            ...
            if($setting->wa_message_sent == 1){
                ...
                $message = "Dear " . $customer_name . ", your purchase file for the project with ID " . $project->project_generated_id . " has been successfully uploaded on " . date('d-m-Y h:i:s A') . ". The address is " . $customerAddress . ". Shree Ganesh Aluminum.";
                $isSent = SmsHelper::sendSmsWithPurchaseFile($message,$request->project_id, true);
            }
            $log = new Log();
        ...
        if (Auth::user()->role == 1) {
            ...
            return redirect()->route('purchase_view.material', $request->project_id);
        } else if(Auth::user()->role == 10){
            return redirect()->route('qa_view.material', $request->project_id);
        }
