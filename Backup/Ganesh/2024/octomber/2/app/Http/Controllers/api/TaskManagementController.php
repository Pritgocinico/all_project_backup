            ...
            if ($accessToken->type == "customer") {
                ...
                if ($role[0]->id == 1) {
                    $tasks = $tasksQuery->when($task_status, function ($query) use ($task_status) {
                        $query->whereIn('task_status', [$task_status,'Site_installation']);
                    })->get();
                } else {
                    $roleNames = $role[0]->name;
                    if($roleNames == 'fitting'){
                        $tasks = $tasksQuery->whereIn('task_type', [$roleNames,'Site_installation'])->get();
                    } else if($roleNames == 'Quality Analytic'){
                        $tasks = $tasksQuery->where('task_type', 'qa')->get();
                    } else {
            ...
            if (!blank($tasks)) {
                ...
                foreach ($tasks as $task) {
                    if (!blank($task->project)) {
                        ...
                        $array['task_type'] = str_replace('_', ' ', ucfirst($task->task_type));
