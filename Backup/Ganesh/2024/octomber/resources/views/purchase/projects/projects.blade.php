    ...
    <div class="project">
        <div class="page-header align-items-center">
            <div class="row">
                ...
                <div class="col-md-4">
                    <form action="{{ route('purchase_projects') }}">
                        <div class="row">
                            <div class="col-md-10">
                        ...
                        @if (!blank($projects))
                            @foreach ($projects as $key => $project)
                                <tr>
                                    ...
                                    <td>{{ $key + 1 }}</td>
                                    <td><a
                                            href="{{ route('purchase_view.completed.project', $project->id) }}">{{ $project->project_generated_id }}</a>
                                    </td>
                                    <td>
                                        @if ($project->customer)
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        <a href="{{ route('purchase_view.project', $project->id) }}" class="editUser"><i
        ...
        function generateAdditionalProject(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('purchase_additional.project', '') }}" + "/" + id,
                        type: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            if (data.status == '1') {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.href = 'purchase/view-measurement/' +
                                            data.id;
                                    }
                                });
                            }
